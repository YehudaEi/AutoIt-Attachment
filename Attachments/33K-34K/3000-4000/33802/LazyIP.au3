...
Global $gui, $msg
Global $pTitle, $pName	;Preset name/Adapter name loaded from preset (if needed)
Global $aTitle, $aName
...
Func Config()
	Local $Presets, $PresetsDummy, $PresetsContext, $pNew, $pSave, $pSaveAs, $pDelete, $msg123
	Local $bAdapterOptions, $AdapterName
	Local $bViewAdvanced, $bApply, $bApplyTo
	...
	$gui = GUICreate("LazyIP Config", 325, 410, -1, -1)
	
	$Presets = GUICtrlCreateButton("Presets", 40, 12, 80, 22)
	...
	$bAdapterOptions = GUICtrlCreateButton("Adapter Options", 190, 12, 110, 22)
	...
	$bViewAdvanced = GUICtrlCreateButton("Advanced Properties", 15, 375, 150, 25)
	$bApply = GUICtrlCreateButton("Apply", 225, 375, 55, 25)
	$bApplyTo = GUICtrlCreateButton("+", 285, 375, 25, 25)
	GUISetState()
	
	$tPreset = TrayCreateMenu("Presets")
	TrayCreateItem("Apply", $tPreset)
	TrayCreateItem("", $tPreset)
	$tOptions = TrayCreateItem("Options")
	...
	$ALT_DNS_GET = _GUICtrlIPAddress_Get($ALTERNATE_DNS)
		...
		Switch $msg
			...
			Case $bApply
				...
				Else
					If $IP_AUTO_STATUS = $GUI_CHECKED Then
						Run("Netsh interface ip set address name='Local Area Connection' source=dhcp", @SystemDir, @SW_HIDE)
						If $DNS_AUTO_STATUS = $GUI_CHECKED Then
					...
					Else
						If $IP_GET = 0.0.0.0 Then
							Run("Netsh interface ip set address name='Local Area Connection' source=dhcp", @SystemDir, @SW_HIDE)
						Else
							Run("Netsh Interface ip set address name='Local Area Connect' source=static addr=" & $IP_GET & " mask=" & $SUB_GET & " gateway=" & $DEF_GET & " gwmetric=1", @SystemDir, @SW_HIDE)
						EndIf
						If $PREF_GET = 0.0.0.0 Then
							Run("Netsh Interface ip set dns name='Local Area Connection source=dhcp", @SystemDir, @SW_HIDE)
						Else
			...
			Case $bCreate
				...
				If $NewInstalledStatus = $GUI_CHECKED Then
					...
					;If $UseInstalledStatus = $GUI_CHECKED Then
						...
						;If the values are 0.0.0.0/DHCP Then > make a new If for IP/DNS - Then for each field if necaessary
...
Func AdapterOptions()
	...
	$CustomName = GUICtrlRead($aCustom)
		...
		Switch $msg
			Case $aEnable
				If $InstalledStatus = $GUI_CHECKED Then
					RunWait("ipconfig /renew " & $InstalledName, @SystemDir, @SW_HIDE)
					ProcessWaitClose("ipconfig.exe")
				Else
					;~Check if $CustomName Adapter exists
					If @error Then
						MsgBox(0, "Custom Adapter Read Error", "The adapter you specified was not found!" & @CRLF & "If the adapter does exist then you either don't have sufficient privileges or spelt it wrong.")
					Else
						RunWait("ipconfig /renew " & $CustomName, @SystemDir, @SW_HIDE)
						ProcessWaitClose("ipconfig.exe")
					EndIf
				EndIf
			Case $aDisable
				If $InstalledStatus = $GUI_CHECKED Then
					RunWait("ipconfig /release " & $InstalledName, @SystemDir, @SW_HIDE)
					ProcessWaitClose("ipconfig.exe")
				Else
					;Check if $CustomName Adapter exists
					If @error Then
						MsgBox(0, "Custom Adapter Read Error", "The adapter you specified was not found!" & @CRLF & "If the adapter does exist then you either don't have sufficient privileges or spelt it wrong.")
					Else
						RunWait("ipconfig /release " & "'" & $CustomName & "'", @SystemDir, @SW_HIDE)
						ProcessWaitClose("ipconfig.exe")
					EndIf
				EndIf
			Case $aReset
...
Func ApplyTo()
	...
	GUICtrlCreateList("", 20, 120, 210, 80)
...
Func About()
	...
	$cStatus = GUICtrlRead($cStartUp)
		Switch $msg
			Case $gLink
				ShellExecute("http://katoNkatoNk.deviantart.com")
			Case $bOK
