...
;~ 			FileRead($hFile,$OldTagSize) 			;read past Old Tag
	...
	EndSwitch
			FileClose($hFile)
			FileClose($hTagFile)
;~             If not FileDelete($Filename) Then MsgBox(0,'','无法删除源文件！')
			If not FileMove($TagFile,$Filename,1) Then Return MsgBox(0,'','无法覆盖原文件或覆盖时出现错误！')
	Return 1
EndFunc
			...
			Case "SYLT"
	            ...
	            While $i<$SynLength
	            	...
	            	While 1
	            		...
	            		$SynTemp &= $nByte
	            	WEnd
					$sFrameString&=BinaryMid(0x5b,1,1)
	            	$sFrameString&=StringToBinary(Dec(Hex(BinaryMid($SynBin,$i,4))),1)
	            	$sFrameString&=BinaryMid(0x5d,1,1)
	            	$sFrameString&=$SynTemp
	            	$sFrameString&=BinaryMid(0x0A0D,1,2)
	            	$i+=4
	            WEnd
...
Func _GetAlbumArt($hFile,$FrameLen)
	...
	Local $LengthToRead = $FrameLen, $AlbumArtFilename = @ScriptDir & "\temp\" & "AlbumArt", $bReturn
...
Func _GetSongLyrics($hFile,$FrameLen)
	...
	Local $LengthToRead = $FrameLen, $LyricsFilename = @ScriptDir & "\temp\" & "SongLyrics.txt", $bReturn
...
Func _GetSynLyrics($hFile,$FrameLen)
	Local $LengthToRead = $FrameLen, $_head, $LyricsFilename = @ScriptDir & "\temp\" & "SynLyrics.txt", $bReturn
	$ID3Filenames &= $LyricsFilename & "|"
