...
Func TreeDBTableClick()
	...
	For $Count = 0 To $ColumnCount - 1
		...
		If Not StringInStr($Type, "INDEX", 1) Then
			...
			$TempArray = StringSplit($Type, "(", 1)
			$TempArray = StringSplit($TempArray[2], ",", 1)
			$ColType = StringStripWS(StringReplace($TempArray[$Count + 1], $FieldNames[$Count][0], "",1), 3)
			If StringRight($ColType, 1) = ")" Then $ColType = StringLeft($ColType, StringLen($ColType) - 1)
			
			$FieldNames[$Count][1] = $ColType
			if stringinstr($Coltype,"PRIMARY KEY",1) Then
				$Data1 = $Data1 & "*"
			EndIf
		EndIf
	Next
	...
	;$Data1 = StringTrimLeft($Data1, 1) & "|RowID*"
	...
	$TableData = $oDb.Prepare ("SELECT rowid,* FROM " & $CurTable & " ORDER BY rowid"); & $FieldNames[0][0])
	
	If $BeginnerMode = True Then
		MsgBox(0, "Select all data, order by rowid","SELECT rowid,* FROM " & $CurTable & " ORDER BY rowid")
	EndIf
	...
	While $tabledata.step () = 0
		...
		For $Count = 0 To $ColumnCount ;- 1
			...
			;$RecordData = $RecordData & " AND " & $FieldNames[$count][0] & "='" & $tabledata.ColumnValue ($Count) & "'"
		Next
		$Data = StringTrimLeft($Data, 1)
		;$RecordData = StringTrimLeft($RecordData, 4)
		
		;msgbox(0,"","SELECT ROWID FROM " & $curtable & " WHERE " & $RecordData)
		;$RowIDobj = $oDb.prepare("SELECT rowid FROM " & $curtable & " WHERE " & $RecordData)
		
		;while $RowIDobj.step()=0
		;	$RowID = $RowIDobj.columnvalue(0)
		;WEnd
			...
			if isarray($TempArray) then
				...
				For $PCount = 1 To $Parameters[0]
					...
					ElseIf $Parameters[$PCount] = "4" and $param=False  Then
						...
						$param=True
					ElseIf $Parameters[$PCount] = "5" and $key = False Then
						$Parameters[$PCount] = "PRIMARY KEY"
						$Key=True
					ElseIf $Parameters[$PCount] = "6" and $key = False Then
						$Parameters[$PCount] = "UNIQUE"
						$key=True
					ElseIf $Parameters[$PCount] = "7" Then
						$Parameters[$PCount] = "COLLATE NOCASE"
					ElseIf $Parameters[$PCount] = "8"  and $Default=False Then
						$Parameters[$PCount] = "DEFAULT CURRENT_TIME"
						$Default = True
					ElseIf $Parameters[$PCount] = "9"  and $Default=False Then
						$Parameters[$PCount] = "DEFAULT CURRENT_DATE"
						$Default = True	
					ElseIf $Parameters[$PCount] = "0"  and $Default=False Then
...
Func EditRecord()
	...
	elseif _GUICtrlListViewGetSelectedCount($listvdb) < 1 Then
...
Func DeleteRecord()
	...
	Else
		$CurRecord = _GUICtrlListViewGetItemTextArray ($ListVDB)
		$DBIndex = $currecord[1]
		$ListIndex = _GUICtrlListViewGetSelectedIndices($ListVDB,1)
		$confirm = 0
		$Confirm = msgbox(1,"Delete Record?","Are you sure you want to delete Record# " & $dbindex & "?  All data will be lost!")
		
		if $Confirm = 1 Then
			$odb.execute("BEGIN TRANSACTION")
			$odb.execute("DELETE FROM " & $curtable & " WHERE rowid=" & $dbindex)
			$odb.execute("COMMIT TRANSACTION")
		
			If $BeginnerMode = True Then
				MsgBox(0, "Delete record", "DELETE FROM " & $curtable & " WHERE rowid=" & $dbindex)
			EndIf
...
Func DisplayRecGUI($rgRecord="")
	...
	For $FieldCount = 0 To UBound($FieldNames, 1) - 1
		...
		If IsArray($rgRecord) Then
			If $rgRecord[$FieldCount + 2] <> "" Then
				$Text = $rgRecord[$FieldCount + 2]
			Else
				$Text = ""
			EndIf
		Else
