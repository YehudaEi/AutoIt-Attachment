...
; Happy 2006 to all
;
; Modified by Onoitsu2 to allow saving and recovery of last region used,
; and auto loads Local Surface Weather if you have a region selected,
; (region not being US National that is...)
;========================================================================
...
Global $default_region = RegRead("HKEY_CURRENT_USER\Software\WeatherDesk","DefaultRegion")
If $default_region = 0 OR $default_region = "" Then
$default_region = 0
GUICtrlSetState($selectitem[0],$GUI_CHECKED)
Else
GUICtrlSetState($selectitem[$default_region],$GUI_CHECKED)
$loc = RegRead("HKEY_CURRENT_USER\Software\WeatherDesk","DefaultRegionAb")
$LastPic = 3
EndIf
...
;~ $region = GUICtrlCreateMenu("")
;~ $display = GUICtrlCreateMenu("")
...
Else
	$title = GUICtrlRead($selectitem[$default_region],1)
	$region = GUICtrlCreateMenu("Currently " & $title[0])
	$display = GUICtrlCreateMenu("Local Surface Weather")
InetGet("http://image.weather.com/images/maps/current/cur_" & $loc & "_720x486.jpg", @TempDir & "\WeatherMaps\pic1.jpg", 1, 0)
EndIf
$Button1 =  GUICtrlCreatePic(@TempDir & "\WeatherMaps\pic1.jpg", 0, 0, 600, 405)
...
While 1
   $get = GUIGetMsg()
;~    MsgBox(0,"","before of do")
   While $init
   Select
   Case $default_region = 0
	   $get = 4
   Case $default_region = 1
	   $get = 5
   Case $default_region = 2
	   $get = 6
   Case $default_region = 3
	   $get = 7
   Case $default_region = 4
	   $get = 8
   Case $default_region = 5
	   $get = 9
   Case $default_region = 6
	   $get = 10
   Case $default_region = 7
	   $get = 11
   Case $default_region = 8
	   $get = 12
   Case $default_region = 9
	   $get = 13
   EndSelect
;~    _GetEcho()
   $init = 0
...
Func _GetEcho()
   ...
   GUICtrlSetImage($Button1, @TempDir & "\WeatherMaps\" & $name)
EndFunc;==>_GetEcho

Func OnAutoItExit()
   AdlibDisable()
   If NOT ($default_region = "") Then
   RegWrite("HKEY_CURRENT_USER\Software\WeatherDesk","DefaultRegion","REG_SZ",$default_region)
   RegWrite("HKEY_CURRENT_USER\Software\WeatherDesk","DefaultRegionAb","REG_SZ",$loc)
   Else
   RegWrite("HKEY_CURRENT_USER\Software\WeatherDesk","DefaultRegion","REG_SZ",0)
   RegWrite("HKEY_CURRENT_USER\Software\WeatherDesk","DefaultRegionAb","REG_SZ","us")
   EndIf
   If $gui1 Then
   GUIDelete($gui1)
   EndIf
