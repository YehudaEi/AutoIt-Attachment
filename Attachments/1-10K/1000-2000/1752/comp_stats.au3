...
; combines the two for easy output
$cpu = $cpugen & $cpuat

; this was party of my revisions... some people dont have controlset001 in their registry some have ; 002 or 003 so this part just uses the first subkey in the section
global $crtset
...
_zounds("HKEY_LOCAL_MACHINE\SYSTEM\ControlSet001\Control\Class\{4D36E96C-E325-11CE-BFC1-08002BE10318}")
...
If $soundcardtwo <> $soundcardone then
$soundcard = "Sound device 1= " & $soundcardone & @crlf & "Sound device 2= " & $soundcardtwo
endif
...
$drtotalun = DriveSpaceTotal( "c:\" )
$drfreeun = DriveSpaceFree( "c:\" )

$drtotal = round($drtotalun, 0)
$drfree = round($drfreeun, 0)
...
$memtotal = round($memtotalun, 0)
	...
	If $drtotal >= 1000 then 
	$drtotals = ($drtotal / 1000) 
	$drtotalf = round($drtotals, 2) & " GB" 
	endif
...
; displays in msg box
msgbox(48, "PC Stats", $cpu & @crlf & "OS Version " & @osversion & "  build " & @OSBuild & "          "& @crlf & "Drive space free c:/ " & $drfreef & @crlf & "Total Drive space c:/  " & $drtotalf & @crlf & "Free physical memory " & $memfree & " MB" & @crlf & "Total physical memory " & $memtotal & " MB" & @crlf & $vidcard & @crlf & $soundcard & @crlf & $mouse & @crlf & $monitor)

; asks if you want to copy to clipboard
$answer = msgbox(4, "PC Stats", "Would you like to copy PC Stats to clipboard?")
If $answer = 6 Then
clipput ($cpu & @crlf & "OS Version " & @osversion & "  build " & @OSBuild & "          "& @crlf & "Drive space free c:/  " & $drfreef & @crlf & "Total Drive space c:/   " & $drtotalf & @crlf & "Free physical memory   " & $memfree & " MB" & @crlf & "Total physical memory   " & $memtotal & " MB" & @crlf & $vidcard & @crlf & $soundcard & @crlf & $mouse & @crlf & $monitor)
endif
