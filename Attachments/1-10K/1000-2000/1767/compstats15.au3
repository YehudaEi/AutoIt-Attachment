...
MsgBox(0, "PC Stats", "PC Stats will search your configuration" & @CRLF & "and display your basic computer hardware", 4)
...
$niccard = StringStripCR(StringStripWS(StringMid( $NICfull , StringInStr($NICfull ,"]")+1),3))
...
; displays in msg box
msgbox(48, "PC Stats", $cpu & @crlf & @osversion & " Build " & @OSBuild & " " & $servpackversion & @CRLF & $diskdrives & "Opticaldrives = " & $opticaldrives & @crlf & "Free physical memory = " & $memfree & " MB" & @crlf & "Total physical memory = " & $memtotal & " MB" & @crlf & $vidcard & @crlf & $soundcard & @crlf & "NIC card = " & $NICcard & @CRLF & $mouse & @crlf & $monitor)

; copies to clipboard
clipput ($cpu & @crlf & @osversion & " Build " & @OSBuild & " " & $servpackversion & @CRLF & $diskdrives & "Opticaldrives = " & $opticaldrives & @crlf & "Free physical memory = " & $memfree & " MB" & @crlf & "Total physical memory = " & $memtotal & " MB" & @crlf & $vidcard & @crlf & $soundcard & @crlf & "NIC card = " & $NICcard & @CRLF & $mouse & @crlf & $monitor)

$answer = MsgBox( 4, "PC Stats", "PC Stats has been copied to the clipboard" & @CRLF & "Do you want to have a file written to " &  $HDletters[1] &"pcstats.txt ?")
	If $answer = 6 Then
		$filename = FileOpen( $HDletters[1] & "pcstats.txt", 2)
		FileWrite( $filename, $cpu & @crlf & @osversion & " Build " & @OSBuild & " " & $servpackversion & @CRLF & $diskdrives & "Opticaldrives = " & $opticaldrives & @crlf & "Free physical memory = " & $memfree & " MB" & @crlf & "Total physical memory = " & $memtotal & " MB" & @crlf & $vidcard & @crlf & $soundcard & @crlf & "NIC card = " & $NICcard & @CRLF & $mouse & @crlf & $monitor)
		fileclose( $filename )
	EndIf
	If $answer = 7 Then
		msgbox(48, "PC Stats", "Bye and dont forget to thank WUS for this program!!!")
	EndIf
