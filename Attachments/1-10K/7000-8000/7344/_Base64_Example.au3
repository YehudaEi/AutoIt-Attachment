...
If MsgBox(260, "Warning", "This is a slow process! Please do not pick a file over 50k" & @CRLF & "This will encode then decode any file" & @CRLF & "THIS WILL OVERWRITE FILES FROM PREVIOUS RUNS OF THIS PROGRAM") = 6 Then
	$file = FileOpenDialog("Image To Html, Base64 Encoder", "", "All (*.*)", 1)
	If @error Then
	Else
		...
		$Wfilehand = FileOpen($file & '.base64', 2)
		
		$iTime = TimerInit()
		;Encode the file, with line breaks (Slower) and show progress (slower)
		$tmp = _Base64Encode (FileRead($filehand, $fsize), "", "File Size: " & StringTrimRight($fsize, 3) & "KB")
		...
		$filehand = FileOpen($file & '.base64', 0)
		$Wfilehand = FileOpen($file & '.original.' & $type, 2)
		$iTime = TimerInit()
		;Decode the file, show progress (slower)
		$tmp = _Base64Decode (FileRead($filehand, $fsize), "File Size: " & StringTrimRight($fsize, 3) & "KB")
		$TimeEnd = Round(TimerDiff($iTime) / 1000, 3)
		FileWrite($Wfilehand, $tmp)
		FileClose($Wfilehand)
		...
		Else
			$filehand = FileOpen($file, 0)
			$Wfilehand = FileOpen($file & '.original.' & $type, 0)
			While 1
...
If MsgBox(260, "Warning", "This is a slow process! Please do not pick a file over 50k" & @CRLF & "This will encode a picture into a html page" & @CRLF & "The html file will will be appened if it all ready exists") = 6 Then
	...
	$tmp = _Base64Encode (FileRead($filehand, $fsize), False)
