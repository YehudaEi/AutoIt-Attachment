...
$Stop = _StopService($strComputer, $strServiceName)
If $Stop = 0 Then MsgBox(0,"Error", "Could not stop service." & @CRLF & @CRLF & "Attempting to start service.")
Sleep(2000)
$Start = _StartService($strComputer, $strServiceName)
If $Start = 0 Then
	MsgBox(0,"Error", "Could not start service.")
Else
	MsgBox(0,"","The Service " & StringUpper($strServiceName) & " On " & StringUpper($strComputer) & " Has Been Restarted")
EndIf
