...
Updated 05-08-2008:
	- Fixed: Script broke on missing includes due to new AutoIt version. Added "#include <StaticConstants.au3>" & "#include <EditConstants.au3>"
		Thanks to: 2words4uready for bringing this breaking to my attention

Updated 05-05-2008:
	- Changed: Program doesn't closes on a pressed esc key anymore
	- Changed: Default program icon
...
Updated 14-04-2008:
 ...
 Updated 02-04-2008:
	...
	- Added function to set the ICMP packet size. Added 'Set ICMP packet size' button in the traymenu. Default set to 32. Also added -l option in DOS command in fastpong() function for this new function
...
$pongfrm = GUICreate("Pong", 174, 40, @DesktopWidth - 209, 35)
...
Func fastpong($j)
	...
	If IsArray($aresult) = 0 Or $pingresult = 0 Then ; If the result is not an array OR the result is '0'
