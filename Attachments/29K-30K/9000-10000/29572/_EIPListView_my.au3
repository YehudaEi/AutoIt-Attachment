...
ConsoleWrite("$bLVUPDATEONFOCUSCHANGE = "&$bLVUPDATEONFOCUSCHANGE &@LF)
	...
	;----------------------------------------------------------------------------------------------
	If $editFlag = 1 Then
		ConsoleWrite(" !!!!!!!! _ListView_Click -> ")
		If $bLVUPDATEONFOCUSCHANGE = True Then
			ConsoleWrite(" !!!!!!!! _ListView_Click -> True")
...
Func _InitEdit($LVINFO, $LVcolControl)
	...
	If $bCanceled  Then
		$bCanceled = False
		ConsoleWrite("_InitEdit:1111111"&@CRLF)
		Return
	EndIf
	if $bCALLBACK Then
		_CancelEdit()
		$bCALLBACK = False
		ConsoleWrite("_InitEdit:2222222"&@CRLF)
	EndIf
	ConsoleWrite("_InitEdit "&$editFlag&@CRLF)
	If $editFlag = 1 Then _CancelEdit()
	Local $CtrlType
	ConsoleWrite("$LVINFO[0] "&$LVINFO[0]&@CRLF)
	ConsoleWrite("$LVINFO[1] "&$LVINFO[1]&@CRLF)
	If $LVINFO[0] < 0 Or $LVINFO[1] < 0 Then Return 0
	ConsoleWrite("UBound($LVcolControl): "&UBound($LVcolControl)&@CRLF)
	...
	;----------------------------------------------------------------------------------------------
	ConsoleWrite("$CtrlType:" & $CtrlType)
	Switch $CtrlType
		Case 1
	...
	Else
		...
		ConsoleWrite("$editCtrlPos[0]: "&$editCtrlPos[0]&@CRLF)
		ConsoleWrite("$editCtrlPos[1]: "&$editCtrlPos[1]&@CRLF)
		ConsoleWrite("$editCtrlPos[2]: "&$editCtrlPos[2]&@CRLF)
		ConsoleWrite("$editCtrlPos[3]: "&$editCtrlPos[3]&@CRLF)
		Local $x1 , $y1
		ConsoleWrite("Classname="&_GetClassName($editCtrl))
...
;		GUICtrlSetPos($editCtrl, $editCtrlPos[0],$editCtrlPos[1], $editCtrlPos[2],$editCtrlPos[3])
		...
		ConsoleWrite("$__LISTVIEWCTRL="&$__LISTVIEWCTRL&@CRLF)
		...
		Switch $editCtrl
			...
			Case $lvCombo
				If $oldText <> "" Then
					...
					ConsoleWrite("$editCtrl="&$editCtrl&@CRLF)
				EndIf
			Case Else	
		...
		;ConsoleWrite("GUI_SHOW: " & GUISetState(@SW_SHOW,$lvControlGui))
		GUISetState(@SW_SHOW,$lvControlGui)
;~ 	GUICtrlSetPos($editCtrl, $editCtrlPos[0],$editCtrlPos[1], $editCtrlPos[2],$editCtrlPos[3])
;~ 	GUICtrlSetState($editCtrl, $GUI_SHOW)
		GUICtrlSetPos($editCtrl, 0,0, $editCtrlPos[2],$editCtrlPos[3])
		ConsoleWrite("GUICtrlSetState($editCtrl, $GUI_SHOW): "&GUICtrlSetState($editCtrl, $GUI_SHOW))
...
Func _CalcEditPos($nLvCtrl,$aINFO)
	   Local $pos[4]
		...
		ConsoleWrite("$ctrlSize[0]: "& $ctrlSize[0]&@CRLF)
		ConsoleWrite("$ctrlSize[1]: "& $ctrlSize[1]&@CRLF)
		ConsoleWrite("$ctrlSize[2]: "& $ctrlSize[2]&@CRLF)
		ConsoleWrite("$ctrlSize[3]: "& $ctrlSize[3]&@CRLF)
		
		ConsoleWrite("$aINFO[2]: "& $aINFO[2]&@CRLF)
		ConsoleWrite("$aINFO[3]: "& $aINFO[3]&@CRLF)
		ConsoleWrite("$aINFO[4]: "& $aINFO[4]&@CRLF)
		ConsoleWrite("$aINFO[5]: "& $aINFO[5]&@CRLF)
		Local $ERR = @error 
		$pos[0] = $aINFO[2]
		$pos[1] = $aINFO[3]+3
		$pos[2] = $aINFO[4]
...
Func WM_Notify_Events($hWndGUI, $MsgID, $wParam, $lParam)
	...
	ConsoleWrite("WM_Notify_Events1!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"&@CRLF)
...
Func WM_MOVE_EVENT($hWndGUI, $MsgID, $wParam, $lParam)
	#forceref $hWndGuI,$MsgID,$wParam,$lParam
	ConsoleWrite("WM_MOVE_EVENT!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"&@CRLF)
...
Func WM_Command_Events($hWndGUI, $MsgID, $wParam, $lParam)
	#forceref $hWndGUI, $MsgID, $wParam
	ConsoleWrite("WM_Command_Events!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"&@CRLF)
