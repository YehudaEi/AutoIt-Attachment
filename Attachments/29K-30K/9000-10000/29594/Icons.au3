...
#Region Function by Aris
Func _Icons_MergeToBitmap($hBack, $hFront, $iX, $iY, $iWidth = -1, $iHeight = -1)
	Local $Size
	...
	_WinAPI_ReleaseDC(0, $hDC)
	If $hBack Then
		_WinAPI_DrawIconEx($hMemDC, 0, 0, $hBack, 0, 0, 0, 0, 3);============>Change from $DI_NORMAL -> 3
	EndIf
	If $hFront Then
		_WinAPI_DrawIconEx($hMemDC, $iX, $iY, $hFront, 0, 0, 0, 0, 3);============>Change from $DI_NORMAL -> 3
	EndIf

	_WinAPI_DeleteDC($hMemDC)

	Return $hBitmap
EndFunc   ;==>_Icons_MergeToBitmap
#EndRegion Function by Aris
