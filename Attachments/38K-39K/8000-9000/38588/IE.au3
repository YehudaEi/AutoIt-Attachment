...
Func _IELoadWait(ByRef $o_object, $i_delay = 0, $i_timeout = -1)
	...
	Local $oTemp, $i_ErrorStatusCode = $_IEStatus_Success
	...
	Select
		Case __IEIsObjType($o_object, "browser"); InternetExplorer
			...
			While Not ($f_Abort Or $o_object.readyState = 4 Or String($o_object.readyState) = "complete")
				...
				Sleep(100)
			WEnd
			While Not ($f_Abort Or String($o_object.document.readyState) = "complete" Or $o_object.document.readyState = 4)
				If (TimerDiff($IELoadWaitTimer) > $i_timeout) Then
					$i_ErrorStatusCode = $_IEStatus_LoadWaitTimeout
					$f_Abort = True
				EndIf
