...
#AutoIt3Wrapper_Res_Comment=Update Application via Internet
#AutoIt3Wrapper_Res_Description=Update demo
...
MsgBox(0,"Update demo", "This is demo version " & $sVersion )

; check for updates
CheckUpdate("UpdateTest" & (@Compiled ? ".exe" : ".au3"), $sVersion, "                                                               ")
If @error = 1 Then
	MsgBox(0,"Update demo", "No newer version available than " & $sVersion )
ElseIf @error = 2 Then
	MsgBox(0,"Update demo", "User refused update." )
EndIf
...
;~ 				If MsgBox(36, "Update " & $sFileToUpdate & " ver " & $sCurrentVersion, "A new version of " & $sFileToUpdate & " is available since " & $sDate & "." & @CRLF & "Download version " & $sNewVersion & " now? ") = 6 Then ;  No is Default
				While "loop view History"
					$Return = MsgBoxEx("|No, Continue||||View Changes|Download", 32 + 3, "Update " & $sFileToUpdate & " ver " & $sCurrentVersion, "A new version of " & $sFileToUpdate & " is available since " & $sDate & "." & @CRLF & "Download version " & $sNewVersion & " now? ")
