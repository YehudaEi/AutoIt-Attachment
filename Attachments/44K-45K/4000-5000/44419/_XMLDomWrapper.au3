...
Func _XMLGetField($strXPath)
	...
	EndIf
	Local $objNodeList, $objChild
	#forceref $objChild
	Local $arrResponse[1], $aRet, $xmlerr, $szNodePath
...
Func _XMLCreateAttrib($strXPath, $strAttrName, $strAttrValue = "")
	...
	EndIf
	Local $objNode, $objAttr, $objAttrVal
	#forceref $objAttr, $objAttrVal
	Local $err
	$objNode = $objDoc.selectSingleNode($strXPath)
	If IsObj($objNode) Then
...
Func _XMLGetPath($strXPath)
	...
	If $DOMVERSION < 4 Then
		...
		Return SetError(1, 0, -1)
	EndIf
	Local $objNodeList, $objNodeChild, $objParent
	#forceref $objNodeChild
	Local $arrResponse[1], $xmlerr, $nodepath, $ns, $nodepathtag
	#forceref $nodepathtag
	$objNodeList = $objDoc.selectNodes($strXPath)
	If $objNodeList.length > 0 Then
