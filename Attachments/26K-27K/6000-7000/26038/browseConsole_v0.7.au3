...
$console_tra = 0
$consoleOnTop = 1
$title = "BrowseConsole v0.7"
$elementGlobal=""

If ProcessExists("cmd.exe") = 0 Then
	Run(@ComSpec & " /k """ & @ScriptFullPath & """",@ScriptDir)
	ConsoleWrite("The best is start program from cmd.exe console. You can set a better width and height of console from option console")
	Sleep(3000)
EndIf
...
;~ $consoleHandle = WinGetHandle(@ComSpec & " - " & @ScriptName)

$consoleHandle = WinGetHandle(@ComSpec & " - " & @ScriptName)
if $consoleHandle = "" Then
	$consoleHandle = WinGetHandle(@ComSpec & " - """ & @ScriptName & """")
	if $consoleHandle = "" Then
		$consoleHandle = WinGetHandle(@ScriptFullPath)
	EndIf
EndIf
...
;~ 	$Pic1 = GUICtrlCreatePic($localize, 10, 10, $w, $h, BitOR($SS_NOTIFY,$WS_GROUP))
	$Pic1 = GUICtrlCreatePic($localize, 10, 10, $w, $h)
...
Func ConsoleWinWrite($ConsoleID, $Text)
    ConsoleWrite($text)
EndFunc

Func ConsoleWinInput($ConsoleID, $Text, $Progress=-1)
    Return _consoleread($text)
EndFunc
