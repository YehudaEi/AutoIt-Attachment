...
;
;	Authors
;		Annelinn, Tammelinn (JLab)
;	Acknowledgements
;		Thanks to Skruge and Spiff59 at the AutoIT forums for their invaluable
;		bug-squishing advice
; 	Known issues
;		1. Captured image disappears if window is minimised or moved partially
;			off the desktop
; 		2. 'Load image' button loads the image, but won't display it
;		3. RefreshLocator() function doesn't display a rectangular locator on
;			a scraped image
;		4. No way to save images with unique filenames, without image display
;			being disabled
;		5. GUIGetMsg sends multiple button pushes, instead of just one
;			(eg $Flag_imagecapture_current)
;		6. When all scraped characters are marked "known", some input controls
;			can't be blanked (and show zero)
...
Global $Script_version = "0.4"

; Window variables
; ----------------

; Main window
Global $Mwin_width = 800 ; To increase the maximum width of the image, increase $Mwin_width and
Global $Mwin_height = 550 ; $Mwin_imagebox_maxwidth by the same number
Global $Mwin_handle
; Image box - the control in which the image is displayed
Global $Mwin_imageboxD
...
; Char box - the control in which an ASCII representation of a scraped character is displayed
Global $Mwin_charbox
; Buttons
Global $Mwin_maxbuttons = 32
Global $Mwin_button[$Mwin_maxbuttons + 1]
; Boxes - $Mwin_info[n] is the input control, and $Mwin_label[n] is its label
...
; The handle of the displayed image
Global $Image_handle
; How many images have been captured - used to give each image a different file name
Global $Image_filecount = 0
; Which file holds the currently captured image
Global $Image_currentfile
; What name the user has given to the currently captured image
Global $Image_currentname
; The captured image's checksum
Global $Image_checksum
; The captured image's original coordinates on the desktop; in the active window; in the active window's client area
...
; Scraped characters are stored here for processing
; The maximum number of characters in the array, and their maximum sizes
Global $Char_maxchars = 1024
Global $Char_maxwidth = 16
Global $Char_maxheight = 16
; An ASCII representation of each scraped character; $Char_array[n][1..x] contains x strings representing
; each line of character n
Global $Char_array[$Char_maxchars + 1][$Char_maxheight + 1]
; The symbol the user has assigned to the scraped character. ^ means "still unknown" and | means "deleted"
Global $Char_array_chr[$Char_maxchars + 1]
; The scraped character's size
Global $Char_array_width[$Char_maxchars + 1]
Global $Char_array_height[$Char_maxchars + 1]
; Where in the captured image the scraped character was found
Global $Char_array_xpos[$Char_maxchars + 1]
Global $Char_array_ypos[$Char_maxchars + 1]

; Variables for the undo buttons, which can undo all changes of character from 'unknown' to 'known'/'deleted',
...
; For each character in the char array, if it was marked 'known' or 'deleted', what value $Char_undo_changecount
; 		had at the time
Global $Char_changenumber[$Char_maxchars + 1]
; The undo button can undo only one character split, this variable holds the value $Char_undo_changecount had
;		the last time a character was split
Global $Char_splitnumber
; Which character was split, and which new character was created (the characters' number in the character array)
...
; A list of all the characters still unknown, and which the user has marked as 'known', displayed at the bottom of
;		the main window
Global $Char_foundstring, $Char_notfoundstring
; What $Char_foundstring will be immediately after the first scrape
Global $Misc_standardstring = "#$,.0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz£€"

; How many characters were scraped during the scrape operation
Global $Char_scrapecharcount
; The number of the character currently shown in the char box
Global $Char_current = 0
; The locator rectangle
Global $Char_locatorhandle

; Target pixel colours
; --------------------

; Up to three colours can be chosen, and they are stored here (the fourth element is needed during the choose routine)
Global $Pixel_target[4 + 1]
; The pixel colour boxes are grey before any colours are chosen
...
; How many colours the user has chosen (maximum 3)
Global $Pixel_number = 0

; Misc variables
; --------------

; Set to 1 when the script should exit
Global $Misc_exitscriptflag = 0
; The file in which the user can store the location of a bitmap image file, its checksum and three of
;		its important pixel colours
Global $Misc_detailfile = "details.txt"

; Flags to mark things that have been done, thereby enabling some other operations 
...
; Set the hotkey for taking photos and exiting the script
...
; The main loop
Do
	; Check the buttons, and react if they have been pressed
	CheckButtons()
	; Check the location of the cursor, and display its coordinates if it's over the captured image
	CheckCursor()
...
Func CreateMainWindow() 
	...
	;$Mwin_imageboxD = GUICtrlCreatePic("", 10, 10, 0, 0)
...
Func CaptureImage() 
	...
	; Capture an image from somewhere on the desktop, save it as a file and 
	; display it in the image box
...
Func RefreshImage() 
	...
	; Display the captured image in the image box
	...
	GUICtrlSetPos($Mwin_imageboxD, $Mwin_imagebox_xposition, $Mwin_imagebox_yposition)
	GUICtrlSetImage($Mwin_imageboxD, $Image_currentfile)
	GUICtrlCreateLabel("",$Mwin_imagebox_xposition+20, $Mwin_imagebox_yposition+20, 50,50,BitOr($GUI_SS_DEFAULT_LABEL,$SS_BLACKFRAME, $WS_THICKFRAME)); $WS_CLIPSIBLINGS));
...
Func RefreshLocator()
ConsoleWrite("in refreshlocator" & @CRLF)
	; Display a rectangle over the captured image, to show the location of the scraped character
	; currently display in the char box
	...
	;$graphics = _GDIPlus_GraphicsCreateFromHWND(ControlGetHandle($Mwin_handle, "", $Mwin_imageboxD))
...
Func CaptureColour() 
	...
	; Alow the user to choose the colours of the text that will be scraped
...
; The scraping algorithm goes through the following stages
;
;	ScrapeText()				Define the search region, which is the location in the main window where the captured
;								image is displayed
;	ScrapeText_SearchStage1		If necessary, lower the top and raise the bottom of the search region to remove any 
;								characters touching the top and bottom, which may be partially outside the captured image
;	ScrapeText_SearchStage2		Separate the search region into horizontal bands, each line of which contains at least one of 
;								the target pixel colours
;		(for each band)
; 	ScrapeText_SearchStage3		If necessary, move the left- and right-hand boundaries of the band inwards to remove
;								characters touching the boundaries, which may be partially outside the captured image
;	ScrapeText_SearchStage4		Separate the band into vertical strips, which should contain one character (or a few
;								touching ones)
;		(for each strip)
;	ScrapeText_SearchStage5		If necessary, narrow the strip to contain only the top and bottom of a character
; 	ScrapeText_SearchStage6		If the character is too big, reduce its size
; 	ScrapeText_ScrapeChar		Scrape the character and store it as a series of ASCII strings, one for each line of pixels,
;								in the character array

Func ScrapeText() 

	; Scrape text from the captured image, and store one copy of each character in the character array
...
Func ScrapeText_SearchStage2($topx, $topy, $bottomx, $bottomy) 
...
Func ScrapeText_SearchStage4($topx, $topy, $bottomx, $bottomy) 
...
Func ScrapeText_SearchStage5($topx, $topy, $bottomx, $bottomy) 
...
EndFunc   ;==>ScrapeText_SearchStage6

Func ScrapeText_ScrapeChar($topx, $topy, $bottomx, $bottomy) 
	...
	Else
		...
		$Char_scrapecharcount = $Char_scrapecharcount + 1
...
Func ScrapeText_RemoveDuplicates() 
...
Func ScrapeText_RemoveBlanks()
	...
	; Having deleted duplicates, sort the character array so that all characters appear at the top of the array, 
...
Func ScrapeText_ResetUndo()
	...
	; Scraping for the second or subsequent times removes the ability to undo changes from before the latest scrape
...
Func DisplayChar($number) 

	; Display the character with char array $number in the char box
...
Func MoveUpChar()
	...
	; Display the previous character marked as 'not found' in the char array, moving upwards from $Char_current
...
Func MoveDownChar()
	...
	; Display the next character marked as 'not found' in the char array, moving downwards from $Char_current
...
Func EnterChar($string)
	...
	; When the user marks a character as 'found', change variables and display the next unfound char
...
Func CharNotfoundToFound($string)
	...
	; Move the character or characters in $string from the not found to the found list
...
Func CharFoundToNotFound($string)
	...
	; Move the character or characters in $string from the found to the not found list
...
Func DeleteChar()
	...
	; Marks the character in the char array, number $Char_current, as 'deleted', then displays the next character
...
Func UndoAllChanges()
	...
	; Find all characters marked as 'found' or 'deleted', and mark them as 'unknown', thereby undoing all changes
...
Func SplitChar()
	...
	; Split a character in the char array into two new characters
...
Func RejigChar($element, $oldwidth, $oldheight, $oldypos)
	...
	; Remove any blank lines from the top and bottom of a newly-split character in the char array
...
Func Hotkey_TakePhoto()
	...
	; Whenever the hotkey is pressed, sets the global variable $Hotkey_press, which is checked by 
	; CaptureImage() and CaptureColour()
...
Func SaveDetails()
	...
	; For the current captured image, appends information about it to the text file $Misc_detailfile,
	;	namely, the name the user has given the image, its filename, the image size, position and checksum 
...
Func SaveData()
	...
	; Open two dialogue windows before writing the contents the character array to files
	; The user can opt not to save one or both of the files by clicking "cancel" in the dialogue windows
	; The first file is a .txt file, in which the array is saved as raw data,
	; the second file is a .au3 file, in which the array is hard-coded in an AutoIT function
	;	(required by other JLab scripts)

	Local $file, $result, $count

	$count = 0

	; .txt file
	$file = FileOpenDialog("Choose a file to save", @ScriptDir, "Text files (*.txt)", 2)
	If @error <> 1 Then
		$result = SaveData_Raw($file)
		If $result = "fail" Then
			Return "fail"
		Else
			$count = $count + 1
	...
	; .au3 file
	$file = FileOpenDialog("Choose a file to save", @ScriptDir, "AutoIT files (*.au3)", 2)
	If @error <> 1 Then
		$result = SaveData_AutoIT($file)
		If $result = "fail" Then
			Return "fail"
		Else
			$count = $count + 1
	...
	; Display confirmation
	If $count = 1 Then
		MsgBox(1, $Script_name, "1 file saved")
	Else
		MsgBox(1, $Script_name, $count & " files saved")
	EndIf
...
Func SaveData_Raw($whichfile)

	; Saves the contents of the char array as raw data to a text file
...
EndFunc   ;==>SaveData_Raw

Func SaveData_AutoIT($whichfile)

	; Saves the contents of the char array as AutoIT code 
	...
	If $result = 0 Then
		...
		Return "fail"
	EndIf
	$line = "	If $Scrape_charbankname = ""something"" Then 	"
	$result = FileWriteLine($file, $line)
	If $result = 0 Then
		MsgBox(1, $Script_name, "Error writing the AutoIT code file (FileWriteLine error)")
		Return "fail"
