...
;================================================================================================================================
;================================================================================================================================
$Client = WinGetClientSize($GUI0)
$Pic = GUICtrlCreatePic("", ($Client[0] - ($Size - ($Size / 3 + $Padding))) / 2, _
		($Client[1] - ($Size - ($Size / 3 + $Padding))) / 2, $Size - ($Size / 3 + $Padding), $Size - ($Size / 3 + $Padding))
...
$Background = PixelGetColor(($Client[0] - ($Size - ($Size / 3 + $Padding))) / 2, ($Client[0] - ($Size - ($Size / 3 + $Padding))) / 2, $GUI0)
$Background = Hex($Background, 6)
GUISetBkColor($Background, $GUI0)
...
_CombineRgn($a, $b)
_CombineRgn($a, $c)
_SetWindowRgn($GUI0, $a)

GUISetState()

For $x = 1 To $Outer_Items
	Assign($x & "Angle", -(($x - 1) * 360 / $Outer_Items))
	Assign("GUI" & $x, GUICreate("Top", $Width, $Width, $cx + $wrad * Sin((0 * 360 / $Outer_Items) * ATan(1) / 45) - ($Width / 2), _
			$cy - $wrad * Cos((0 * 360 / $Outer_Items) * ATan(1) / 45) - ($Width / 2), $WS_CHILD, -1, $GUI0))
	Assign("Icon" & $x, GUICtrlCreateIcon("", -1, 0, 0, $Width, $Width))
	If IniRead(@ScriptDir & "\Settings.ini", "Icon", $x, "") <> "" And IniRead(@ScriptDir & "\Settings.ini", "Run", $x, "") <> "" Then
		...
		If StringInStr($String, "\") Then
			GUICtrlSetImage(Eval("Icon" & $x), IniRead(@ScriptDir & "\Settings.ini", "Icon", $x, ""))
		Else
			$Reg = _GetRegDefIcon("." & $String)
			_SetIcon(Eval("Icon" & $x), $Reg[0], $Reg[1], 48, 48)
		EndIf
		$Icons += 1
		GUICtrlSetTip(Eval("Icon" & $x), IniRead(@ScriptDir & "\Settings.ini", "Icon", $x, ""))
	EndIf
	GUISetBkColor(0x00FF00, Eval("GUI" & $x))
	GUISetState(@SW_SHOW, Eval("GUI" & $x))
	$iFlags = BitOR($SWP_SHOWWINDOW, $SWP_NOSIZE, $SWP_NOMOVE)
	_WinAPI_SetWindowPos(Eval("GUI" & $x), $HWND_TOP, 0, 0, 0, 0, $iFlags)
Next
	...
	Switch $iwParam
		Case $WM_MOUSEWHEEL
			...
			Else
				...
				Next
			EndIf
		Case Eval(IniRead(@ScriptDir & "\Settings.ini", "Options", "Hide Key", "WM_MBUTTONDOWN"))
			If BitAND(WinGetState($GUI0), 2) Then
				WinSetState($GUI0, "", @SW_HIDE)
			Else
				WinSetState($GUI0, "", @SW_SHOW)
			EndIf
	EndSwitch
