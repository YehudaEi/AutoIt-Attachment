...
For $i=0 to Ubound($titles)-1
	...
	Else
        ...
        $ret = DLLCall("user32.dll","long","GetWindowThreadProcessId", "long", _SysTrayIconHandle($i), "ptr", DllStructGetPtr($stPID))
...
;=========================================================
    if(@OSArch = "X64") Then
      Local $str        = "int;int;byte;byte;byte[6];dword;int"
    Else
...
$stPID = DllStructCreate("int")
$ret = DLLCall("user32.dll","long","GetWindowThreadProcessId", "long", $trayHwnd, "ptr", DllStructGetPtr($stPID))
If Not @error Then
		$pId = $ret[2]
Else
...
$stPID = DllStructCreate("int")
Local $ret = DLLCall("user32.dll","long","GetWindowThreadProcessId", "long", $trayHwnd, "ptr", DllStructGetPtr($stPID))
If Not @error Then
		$pId = $ret[2]
Else
	ConsoleWrite("Error: Could not find toolbar process id, " & @error & @LF)
...
Func _SysTrayIconVisible($flag, $index)
	...
	$stPID = DllStructCreate("int")
    Local    $ret = DLLCall("user32.dll","long","GetWindowThreadProcessId", "long", $hWnd, "ptr", DllStructGetPtr($stPID))
