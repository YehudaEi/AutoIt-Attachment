...
$cost = GUICtrlCreateInput ("", 60,  15, 70, 20, $ES_RIGHT) 
$retail = GUICtrlCreateInput ("", 60,  38, 70, 20, $ES_RIGHT)
...
$marginbox = GUICtrlCreateInput ("", 60,  115, 70, 20, $ES_RIGHT) 
$markupbox = GUICtrlCreateInput ("", 60,  138, 70, 20, $ES_RIGHT)
...
While 1
    ...
    Select
        ...
        Case $msg = $calculate
            ...
            $marginvalue = GUICtrlRead($marginbox)
            $markupvalue = GUICtrlRead($markupbox)
            ;---calculate COST from MARGIN & RETAIL
            If $costvalue = "" AND $markupvalue = "" AND $retailvalue <> "" AND $marginvalue <> "" Then
                $costvalue = Round(($retailvalue - ($retailvalue * ($marginvalue/100))), 2)
                GUICtrlSetData ($cost, $costvalue)
            ;---calculate COST from MARKUP & RETAIL
            ElseIf $costvalue = "" AND $marginvalue = "" AND $retailvalue <> "" AND $markupvalue <> "" Then
                $costvalue = Round(($retailvalue / (1+($markupvalue/100))), 2)
                GUICtrlSetData ($cost, $costvalue)
            ;---calculate RETAIL from MARKUP & COST
            ElseIf $retailvalue = "" AND $markupvalue <> "" AND $costvalue <> "" AND $marginvalue = "" Then
                $retailvalue = Round(($costvalue + ($costvalue * ($markupvalue/100))), 2)
                GUICtrlSetData ($retail, $retailvalue)
            ;---calculate RETAIL from MARGIN & COST
            ElseIf $retailvalue = "" AND $marginvalue <> ""  AND $costvalue <> "" AND $markupvalue = "" Then
                $retailvalue = Round(($costvalue / (1-($marginvalue/100))), 2)
                GUICtrlSetData ($retail, $retailvalue)     
            ;---calculate MARGIN & MARKUP from COST & RETAIL
            ElseIf $costvalue <> "" AND $retailvalue <> "" Then
