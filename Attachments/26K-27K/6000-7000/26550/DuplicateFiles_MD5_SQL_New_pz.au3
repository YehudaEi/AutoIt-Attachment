...
_SQLite_Exec (-1, "CREATE INDEX ix1 on Duplicates (md5);")
_SQLite_Exec (-1, "CREATE INDEX ix2 on Duplicates (File);")
_SQLite_Exec (-1, "CREATE INDEX ix3 on Bookmarks (ShortPointer);")
...
;~ 					  '("' &  $aFilesList[$n] & '","' & $sDateStamp & '","' &  $sFileSize & '","' & $oDict1_key & '");')
		...
		$sql_cmd &= 'INSERT INTO Duplicates(File,Date,Size,md5) VALUES' & _ 
					  '("' &  $aFilesList[$n] & '","' & $sDateStamp & '","' &  $sFileSize & '","' & $oDict1_key & '");'
		$num_sql += 1
		
		If $num_sql = 50 Then ; call it once for every 50 collected INSERTs
...
;~ 	Sleep (25)
	...
	If $Time <> $Time_Prev Then ; refresh only if it changes to avoid flicker
		GuiCtrlSetData($TimeLabel,"Time Elapsed : " & $Time )
		$Time_Prev = $Time 
	EndIf
	
	$percent = ($n/$aFilesList[0])*100
