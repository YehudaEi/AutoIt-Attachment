...
#region ; Functions
	...
	Func _Toast_Show($vIcon, $sTitle, $sMessage, $iDelay = 0, $fWait = True, $fRaw = False)
		...
		;$PackagesRegKey = Regread("HKLM\Software\LSL", "PKGS")
		;$ArrayOfPkgs = StringSplit($PackagesRegKey, @LF)
		;$NumberOfPackages = Ubound($ArrayOfPkgs) -1
		;$HeightOfGui = $NumberOfPackages * 10 + 20

		Local $iToast_Height = $iLabelheight + $iTitle_Height + $HeightOfGui
	...
	Func _Toast_Locate($iToast_Width, $iToast_Height)
		...
		Else
			...
			;ElseIf $aWorkArea[1] <> 0 Then
				$iToast_Move = 0x00050008 ; $AW_SLIDE_OUT_TOP
				$aToast_Data[0] = @DesktopWidth / 2 - $iToast_Width /2
