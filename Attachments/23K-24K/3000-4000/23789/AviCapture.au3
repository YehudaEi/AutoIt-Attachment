...
$win = GUICreate("AVICapture  v1.3", 305, 180, -1, -1, BitOR($WS_CAPTION, $WS_SYSMENU), $WS_EX_CONTEXTHELP)
...
Func _Capture($param = 'full screen')
	...
	If $param = 'region' Then
		...
		$AVI = _CreateAvi(@ScriptDir & "\temp.avi", GUICtrlRead($comboframe), @TempDir & "\frame.bmp")
		...
		While $Capture = True
			...
			$AviFileSize = FileGetSize(@ScriptDir & "\temp.avi") / 1048576
			GUICtrlSetData($AviSize, $AviFileSize & " Mo")
			If $timer <> 0 Then
