...
$Window=GuiCreate("Window Property Manager", 556+30, 310,-1, -1 , BitOR($WS_OVERLAPPEDWINDOW, $WS_CLIPSIBLINGS))

$List_Window = GuiCtrlCreateListView("Enabled|Visible|Status|Window Name   |Handle ", 10, 10, 386+80, 279)
...
$Button_Refresh = GuiCtrlCreateButton("Refresh", 406+80, 10, 90, 20)
$Button_Reset = GuiCtrlCreateButton("Reset", 406+80, 222, 90, 20)
$Button_Apply = GuiCtrlCreateButton("Apply settings", 406+80, 244, 90, 20)
$Button_Exit = GuiCtrlCreateButton("Exit", 406+80, 266, 90, 20)

$Group_Attributes = GuiCtrlCreateGroup("Attributes", 406+80, 30, 90, 182)

$Checkbox_Visible = GuiCtrlCreateCheckbox("Visible", 416+80, 45, 70, 20)
GUICtrlSetState (-1,$GUI_CHECKED)
$Checkbox_Enabled = GuiCtrlCreateCheckbox("Enabled", 416+80, 65, 70, 20)
GUICtrlSetState (-1,$GUI_CHECKED)
$Checkbox_OnTop = GuiCtrlCreateCheckbox("On top", 416+80, 85, 70, 20)

$Radio_Max = GuiCtrlCreateRadio("Maximized", 416+80, 111, 70, 20)
GUICtrlSetState (-1,$GUI_CHECKED)
$Radio_Min = GuiCtrlCreateRadio("Minimized", 416+80, 131, 70, 20)
$Radio_Res = GuiCtrlCreateRadio("Restored", 416+80, 151, 70, 20)
$Label_1=GUICtrlCreateLabel("Transparency",416+80,176,70,20)
$Label_2=GUICtrlCreateLabel("—",505-94+80,191,10,20)
$Label_3=GUICtrlCreateLabel("+",578-94+80,191,10,20)
$Slider_Trans = GUICtrlCreateSlider(418+80, 191, 66,20,$TBS_NOTICKS)
GUICtrlSetLimit(-1,255,0)
GUICtrlSEtData(-1, 255)

$Label_status=GUICtrlCreateLabel (" Ready",0,292,346-80,18,BitOr($SS_SIMPLE,$SS_SUNKEN))
GUICtrlSetColor(-1,0x666666)
GUICtrlCreateLabel ("",540-94-180,292,76+100+84,18,BitOr($SS_SIMPLE,$SS_SUNKEN))
GUICtrlSetColor(-1,0x666666)
$Progress_status=GUICtrlCreateProgress (540-92-180,294,76+100+80,15)
GUICtrlSetColor(-1,0x666666)
$Label_color=GUICtrlCreateLabel (" Ready",540-94+80,292,60,18,BitOr($SS_SIMPLE,$SS_SUNKEN))
GUICtrlSetColor(-1,0x666666)

GUICtrlCreateLabel("",500-94+80,108,90,1)
GUICtrlSetBkColor(-1,0xD0D0BF)
GUICtrlCreateLabel("",500-94+80,172,90,1)
GUICtrlSetBkColor(-1,0xD0D0BF)
...
While 1
	...
	If $msg = $MenuItem_Help Then
		...
		Cntrlstate()
	Endif
	If GUICtrlRead($List_Window) <> 0 Then GUICtrlSetData($Label_color,$windows[GUICtrlRead($List_Window)-33][1])
	If $msg = $Button_Refresh OR $msg = $MenuItem_Refresh Then
		ReloadWinList()
	Endif
	If $msg = $Button_Apply AND $refreshstate = 1 Then																;
		$state = WinGetState($windows[GUICtrlRead($List_Window)-33][0], "")											;
		If GUICtrlRead($Checkbox_Enabled) = $GUI_CHECKED Then 													;
			WinSetState($windows[GUICtrlRead($List_Window)-33][0],"",@SW_ENABLE)										;
		Else																										;
			WinSetState($windows[GUICtrlRead($List_Window)-33][0],"",@SW_DISABLE)									;
		Endif																										;
		If GUICtrlRead($Checkbox_Visible) = $GUI_CHECKED Then 													;
			WinSetState($windows[GUICtrlRead($List_Window)-33][0],"",@SW_SHOW)										;
		Else																										;
			WinSetState($windows[GUICtrlRead($List_Window)-33][0],"",@SW_HIDE)										;
		Endif																										;
		If GUICtrlRead($Checkbox_OnTop) = $GUI_CHECKED Then 													;
			WinSetOnTop($windows[GUICtrlRead($List_Window)-33][0],"",1)												;
		Else																										;
			WinSetOnTop($windows[GUICtrlRead($List_Window)-33][0],"",0)												;
		Endif																										;	
		If GUICtrlRead($Slider_Trans) Then 																;
			WinSetTrans($windows[GUICtrlRead($List_Window)-33][0],"",GUICtrlRead($Slider_Trans))					;
		Endif																										;
		If GUICtrlRead($Radio_Max) = $GUI_CHECKED Then 															;
			WinSetState($windows[GUICtrlRead($List_Window)-33][0],"",@SW_MAXIMIZE)									;
		Else																										;
			If GUICtrlRead($Radio_Min) = $GUI_CHECKED Then 														;
				WinSetState($windows[GUICtrlRead($List_Window)-33][0],"",@SW_MINIMIZE)								;
			Else																									;
				WinSetState($windows[GUICtrlRead($List_Window)-33][0],"",@SW_RESTORE)								;
			Endif																									;
		Endif																										;
	Endif	
	If GUICtrlRead($List_Window) <> $cursel AND GUICtrlRead($List_Window) <> 0 AND $refreshstate = 1 Then 
		GUICtrlSetData($Label_Status,$windows[GUICtrlRead($List_Window)-33][0] & ", " & GUICtrlRead($List_Window)-33 & ", " & GUICtrlRead($List_Window))
		$cursel = GUICtrlRead($List_Window)
	Endif
...
Func ReloadWinList()
	...
	For $i=1 to $windows[0][0]
		;If $i < 30 Then Msgbox(0,"Test",$temp2[$i] & ", " & $i)
		sleep(random(5,26))
		...
		$state = WinGetState($windows[$i][0], "")
			...
			Else
				...
				EndIf
			EndIf
		If $windows[$i][0] <> "0" Then 
			$winitem[$i]=GUICtrlCreateListViewItem($WinAttrib[$i-1][0] &"|"& $WinAttrib[$i-1][1] &"|"& $WinAttrib[$i-1][2] &"|"& $windows[$i][0] &"|"& $windows[$i][1],$List_Window)
		EndIf
