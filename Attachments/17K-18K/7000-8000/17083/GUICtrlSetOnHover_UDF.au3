...
Func CALLBACKPROC($hWnd, $uiMsg, $idEvent, $dwTime)
	...
	Local $CursorInfo = GUIGetCursorInfo($hWnd)
	
	If Not IsArray($CursorInfo) Or $CursorInfo[4] = 0 Then
	...
	Else
		For $i = 1 To $HOVER_CONTROLS_ARRAY[0][0]
			If $HOVER_CONTROLS_ARRAY[$i][0] = GUICtrlGetHandle($CursorInfo[4]) Then
				Call($HOVER_CONTROLS_ARRAY[$i][1], $CursorInfo[4])
				If $HOVER_CONTROLS_ARRAY[$i][2] <> -1 Then
					
					$LAST_HOVERED_ELEMENT[0] = $HOVER_CONTROLS_ARRAY[$i][2]
					$LAST_HOVERED_ELEMENT[1] = $CursorInfo[4]
				EndIf
				Return
			EndIf
		Next
