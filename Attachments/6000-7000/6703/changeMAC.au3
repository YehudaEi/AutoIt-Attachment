...
While 1
	...
	If $returnval = $NicId Then
		...
		RegWrite($NicKey & RegEnumKey($NicKey, $i), "NetworkAddress", "REG_SZ", RandomMAC())
		ExitLoop
	ElseIf @error = 1 Then
...
; generates a sorta-random mac address starting with 00 (not really a good rand gen, but it works fine for me)
Func RandomMAC()
	Local $newMAC = "00"
	For $i = 1 To 5
