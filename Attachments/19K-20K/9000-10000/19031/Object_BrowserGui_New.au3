...
Local $sFile, $TVItem
	...
	GuiSetState(@SW_LOCK)
		_GUICtrlTreeView_DeleteAll($MemberNames)
	
	$tliTypeLibInfo.ContainingFile = $sPath ;"tlbinf32.dll" ; $sPath The file to expose its TLB info. 
...
For $tliTypeInfo In $tliTypeLibInfo.TypeInfos
	...
	; Now we have to search each member to find the correct value.
		For $oTLMember In $tliTypeInfo.Members 
                $GetEnumName = $oTLMember.Name & " " & $oTLMember.InvokeKind
...
Func _MemberClick()
	If _GUICtrlListView_GetItemText($ListView1,0) = 0 then ; Check this New Function PTREX
