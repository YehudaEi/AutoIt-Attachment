		...
		Else
			For $i = 1 To $Window_List_Sizes[0][0] Step 1
				If BitAnd(WinGetState($Window_List_Sizes[$i][0], ""), 16) = 0 Then
					If ($Ball_Pos[0] + $Ball_Size) > $Window_List_Sizes[$i][1] And $Ball_Pos[0] < ($Window_List_Sizes[$i][1] + StringReplace($Xslope, "-", "")) And $Ball_Pos[1] > $Window_List_Sizes[$i][2] And $Ball_Pos[1] < ($Window_List_Sizes[$i][2] + $Window_List_Sizes[$i][4]) And StringInStr($Xslope, "-") = 0 Then
						$Xslope = $Xslope * -1
					ElseIf $Ball_Pos[0] < ($Window_List_Sizes[$i][1] + $Window_List_Sizes[$i][3]) And $Ball_Pos[0] >= (($Window_List_Sizes[$i][1] + $Window_List_Sizes[$i][3]) - StringReplace($Xslope, "-", "")) And $Ball_Pos[1] > $Window_List_Sizes[$i][2] And $Ball_Pos[1] < ($Window_List_Sizes[$i][2] + $Window_List_Sizes[$i][4]) And StringInStr($Xslope, "-") Then
						$Xslope = $Xslope * -1
					ElseIf ($Ball_Pos[1] + $Ball_Size) > $Window_List_Sizes[$i][2] And $Ball_Pos[1] < ($Window_List_Sizes[$i][2] + StringReplace($Yslope, "-", "")) And $Ball_Pos[0] > $Window_List_Sizes[$i][1] And $Ball_Pos[0] < ($Window_List_Sizes[$i][1] + $Window_List_Sizes[$i][3]) And StringInStr($Yslope, "-") = 0 Then
						$Yslope = $Yslope * -1
