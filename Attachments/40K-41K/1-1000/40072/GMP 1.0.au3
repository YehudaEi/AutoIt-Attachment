...
; Remarks .......: Requires expressed operations. Also doesn't always handle order of operations properly. For example 1-5+3-2+3=0,
;                  but this returns -12 because it does all of one arithmetic operation first before doing the next operation.
;                  Furthermore the larger the result the slower it becomes as it has to parse the result multiple times.
; ===============================================================================================================================
Func _GMP_Parse($sLine)
	If StringRegExp($sLine, "\^-") Then
		$sLine = StringStripWS($sLine, 8)
		$aExp = StringRegExp($sLine, "\d+\^-\d+", 3)
		For $x = 0 To UBound($aExp) - 1
			$aNum = StringRegExp($aExp[$x], "\d+", 3)
			$sLine = StringReplace($sLine, $aExp[$x], _GMP_PowFloat($aNum[0], "-" & $aNum[1]))
		Next
		Return _GMP_ParseFloat($sLine)
	ElseIf StringRegExp($sLine, "\.") Then
		Return _GMP_ParseFloat($sLine)
	Else
...
; Parameters ....: $sLine - e.g. "3^5*2*(128/2)"
;                  ^ : Power
;                  * : Muliply
;                  % : Remainder
;                  / : Quotient
...
Func _GMP_ParseInt($sLine)
	...
	If IsArray($aReg) Then
		...
		For $i = 0 To UBound($aReg) - 1
			$sLine = StringReplace($sLine, Chr(1), _GMP_ParseInt($aReg[$i]), 1)
		Next
		$sLine = _GMP_ParseInt($sLine)
	EndIf
	Local $aOp[6][2] = [["\^", "_GMP_PowInteger"],["%", "_GMP_DivIntegerr"],["/", "_GMP_DivIntegerq"],["\*", "_GMP_MulInteger"],["\+", "_GMP_AddInteger"],["-", "_GMP_SubInteger"]]
	Local $iCnt = 0
	While $iCnt < 6
...
Func _GMP_ParseFloat($sLine)
	...
	If IsArray($aReg) Then
		$sLine = StringRegExpReplace($sLine, "\([^()]+\)", Chr(1))
		For $i = 0 To UBound($aReg) - 1
			$sLine = StringReplace($sLine, Chr(1), _GMP_ParseFloat($aReg[$i]), 1)
		Next
		$sLine = _GMP_ParseFloat($sLine)
	EndIf
	Local $aOp[5][2] = [["\^", "_GMP_PowFloat"],["/", "_GMP_DivFloat"],["\*", "_GMP_MulFloat"],["-", "_GMP_SubFloat"],["\+", "_GMP_AddFloat"]]
	Local $iCnt = 0
	While $iCnt < 5
		$aReg = StringRegExp($sLine, "(-*[0-9.]+)\#*" & $aOp[$iCnt][0] & "\#*(-*[0-9.]+)", 1)
		If IsArray($aReg) Then
