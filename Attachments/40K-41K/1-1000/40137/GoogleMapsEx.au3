...
#cs
	...
	Changelog:
	              ...
	              ------------08/04/13--------  v0.5
				      Drawing manager added (just Polygon Form), Also a in/out form that contains the last drawing
				      coordenates to store in a DB.
					---------10/06/10---------- v0.4
...
func _GUICtrlGoogleMap_Create(ByRef $gmap, $left, $top, $width, $height, $address, $zoom = 8, $marker = True, $map_type = 0, $navigation_style = 0, $scale_style = 0, $map_type_style = 0)
	...
	Local Const $html1 = _
		...
		"<script src=""http://maps.google.com/maps/api/js?sensor=false&libraries=drawing&dummy=.js""></script>" & @CRLF & _
		"<script src=""http://ajax.googleapis.com/ajax/libs/jquery/1/jquery.min.js""></script>" & @CRLF & _
		"<script type=""text/javascript"">" & @CRLF & _
		"  var directionDisplay;" & @CRLF & _
		"  var directionsService = new google.maps.DirectionsService();" & @CRLF & _
		"  var map;" & @CRLF & _
		"  var iw = new google.maps.InfoWindow();" & @CRLF & _
		"  var lat_longs = new Array();" & @CRLF & _
		"  var drawingManager;" & @CRLF & _
		"  var markers = new Array()" & @CRLF & _
  		"  var markersArray = [];" & @CRLF & _
		"  var vertices = new Array ()" & @CRLF & _
		"  var verarray = [];" & @CRLF & _
		...
		"		drawingManager = new google.maps.drawing.DrawingManager({" & @CRLF & _
		"		drawingMode: google.maps.drawing.OverlayType.POLYGON," & @CRLF & _
  		"		drawingControl: true," & @CRLF & _
  		"		drawingControlOptions: {" & @CRLF & _
  		"			position: google.maps.ControlPosition.TOP_CENTER," & @CRLF & _
		"			drawingModes: [google.maps.drawing.OverlayType.POLYGON]" & @CRLF & _
		"		}," & @CRLF & _
		"				polygonOptions: {" & @CRLF & _
		"					editable: true" & @CRLF & _
		"				}" & @CRLF & _
		"	});" & @CRLF & _
		"	drawingManager.setMap(map);" & @CRLF & _
		"	google.maps.event.addListener(drawingManager, ""overlaycomplete"", function(event) {" & @CRLF & _
		"		var newShape = event.overlay;" & @CRLF & _
		"		newShape.type = event.type;" & @CRLF & _
		"	});" & @CRLF & _
        "    google.maps.event.addListener(drawingManager, ""overlaycomplete"", function(event){" & @CRLF & _
        "        overlayClickListener(event.overlay);" & @CRLF & _
        "        $('#vertices').val(event.overlay.getPath().getArray());" & @CRLF & _
        "    });" & @CRLF & _
		...
		"    }" & @CRLF & _
		"  }" & @CRLF & _
		"	function overlayClickListener(overlay) {" & @CRLF & _
		"   google.maps.event.addListener(overlay, ""mouseup"", function(event){" & @CRLF & _
        "   $('#vertices').val(overlay.getPath().getArray());" & @CRLF & _
		"	});" & @CRLF & _
		"   };" & @CRLF & _
		"   google.maps.event.addDomListener(window, 'load', initialize);" & @CRLF & _
        "  $(function(){" & @CRLF & _
		"    $('#save').click(function(){" & @CRLF & _
        "  });"& @CRLF & _
        "  });"& @CRLF & _
	...
	Local Const $html2 = _
		...
		"  }" & @CRLF & _
		"  function drawmap(verarray) {" & @CRLF & _
		"   var vertices = [(verarray)]; " & @CRLF & _
	    "    newdraw = new google.maps.Polygon({ " & @CRLF & _
		"    paths: vertices,"  & @CRLF & _
		"	strokeColor: ""#FF0000"", " & @CRLF & _
		"   strokeOpacity: 0.8," & @CRLF & _
        "    strokeWeight: 2," & @CRLF & _
        "    fillColor: ""#FD852D""," & @CRLF & _
        "    fillOpacity: 0.35" & @CRLF & _
		"});" & @CRLF & _
		"    newdraw.setMap(map)}" & @CRLF & _
		...
		"<form method=""post"" accept-charset=""utf-8"" id=""map_form"">"  & @CRLF & _
		"  <input type=""text"" name=""vertices"" value="""" id=""vertices""  />"  & @CRLF & _
        "  <input type=""button"" name=""save"" value=""Save!"" id=""save""/>"  & @CRLF & _
        " </form>" & @CRLF & _
		"</body>" & @CRLF & _
		"</html>"
#cs
...
; #FUNCTION# ;===============================================================================
;
; Name...........:	_GUICtrlGoogleMap_DrawPolygon()
; Description ...:	Draws a Overlayed Polygon in the current Map.
; Syntax.........:	 _GUICtrlGoogleMap_DrawPolygon(ByRef $gmap, $VerArray)
; Parameters ....:	$gmap			- The embedded Google Map Object
;                   $VerArray       - the array on LatLng Mode
; Return values .: 	On Success		- Shows the Overlayed Map
;                 	On Failure		- Returns False.

; Author ........:	Pionner
; Modified.......:
; Remarks .......:	An MCVArrray must be provided by yourself, otherwise you will get an
;                   Error, this function was desinged to work with DB's. it is too
;                   hard to insert them manually, do you?
;
; Related .......:
; Link ..........:
; Example .......:	no
;
; ;==========================================================================================
Func _GUICtrlGoogleMap_DrawPolygon(ByRef $gmap, $VerArray)

local $MCVArray = " new google.maps.LatLng "
local $i, $coordenates, $CountVer
...
$CountVer = StringSplit($VerArray,")",0)
for $i= 1 to $CountVer[0] - 1
	$Truearray = StringSplit($VerArray,")",0)
	   if $i = 1 then
		     $ei = 0
	     Else
		     $ei=1
	    EndIf
	$f = _StringInsert($TrueArray[$i] & ")", $MCVArray,$ei)
                $coordenates = $coordenates & $f
next

$gmap.document.parentWindow.execScript("drawmap([" & $coordenates & "]);")
