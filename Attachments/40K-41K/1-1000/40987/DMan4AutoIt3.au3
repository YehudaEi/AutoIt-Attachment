...
Local $iVersion = Number(StringReplace(@AutoItVersion, ".", ""))
If Not $iVersion >= 3396 Then
	MsgBox($MB_ICONHAND, "Disk Manager", "Disk Manager requires at least AutoIt v3.3.9.6")
	Exit
EndIf
...
$Tips[2] = "Please visit                        for more programs."
...
Global $Parts[] = [175, 150]
; Allow multi control creation of controls ~ Nahuel
Global $Labels[$_AllD[0] + 1]
Global $Progress[$_AllD[0] + 1]
; Dll Structures
...
$GUI = GUICreate("Disk Manager", 520, 350 + $_AllD[0] * 6)
...
$Console = GUICtrlCreateInput("", 190, 335, 310, 17)
GUICtrlSetFont(-1, 8.5, 400, Default, "Courier New")
...
;~ 	MsgBox(0, "Disk Manager", "When saving a file please remember to add the extension .DMA to the end!", 10)
	...
	ConsoleWrite($Export_File & @CRLF)
	ConsoleWrite(StringRight(StringLower($Export_File), 4) & @LF)
	...
	FileWrite($io_file, "// Exported using Disk Manager by James Brooks, "&@YEAR&"!" & @CRLF & @CRLF)
