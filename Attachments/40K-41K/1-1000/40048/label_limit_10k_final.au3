...
Global $intervalRowCount = 48, $intervalColumnCount = 1, $skillsetRowCount = 1, $skillsetColumnCount = 80
...
;~ 			$gridCellsContent[$r - 1][$c - 1] = $r & " " & Random(-9, 9, 1) ; enumerate rows
			$gridCellsContent[$r - 1][$c - 1] = Random(-9, 9, 1) & " " & $c ; enumerate columns
...
Func _CreateSkillsetRow()
	...
	Local $tempAssignSkillset = "ABCDEF123456", $aSizeSkillset
	Local $textBuilder, $intBuilder, $stringBuilder, $letterBuilder
	Local $textSize = 13
	ReDim $tempGrid[$tempRC][$tempCC]
...
;~ 	For $c = 1 To 1 ; use this to show the first column only
	For $c = 1 To $skillsetColumnCount
		$textBuilder = ""
		$intBuilder = ""
		$stringBuilder = ""
		For $x = 1 To Random(1, 8, 1)
			$letterBuilder = Chr(Random(Asc("A"), Asc("Z"), 1))
			$textBuilder = $textBuilder & $letterBuilder
		Next
		For $x = 1 To Random(1, 8, 1)
			$intBuilder = $intBuilder & $x
		Next
		$stringBuilder = $textBuilder & $intBuilder

		For $r = 1 To $skillsetRowCount
;~ Reverse the comments in here if you want to play with random length text labels rather than ABCDEF123456
;~ 			If ($c = 4 Or $c = 5 Or $c = 6 Or $c = 44 Or $c = 46) Then
;~ 			$tempAssignSkillset = $stringBuilder
;~ 			Else
;~ 				$tempAssignSkillset = "ABCDEF123456"
;~ 			EndIf
			$tempAssignSkillset = "ABCDEF123456"
;~ Reverse the comments here if you want to play with random length text labels rather than ABCDEF123456

			$aSizeSkillset = _StringSize($tempAssignSkillset, Default, Default, Default, Default, 0)
			$tempGrid[$r - 1][$c - 1] = $tempAssignSkillset
			_CreateBorderLabel("", (($c - 1) * ($cellWidth + $cellLeftOffset)) + $cellLeftOffset, (($r - 1) * ($cellHeight + $cellTopOffset)) + $cellTopOffset, _
					$cellWidth, _
					(($cellHeight - 3) * 4), _
					$penColor, $penWeight, BitOR($SS_CENTER, $SS_CENTERIMAGE))

			______DebugPrint______("Container:" & $c & " --- String(array0):" & $aSizeSkillset[0] & " WidthRect(array2):" & $aSizeSkillset[2] & " --- " & _
					" Width: " & $cellWidth & _
					" ContHeight:" & (($cellHeight - 3) * 4) & _
					" TopOffset: " & (($r - 1) * ($cellHeight + $cellTopOffset)) + $cellTopOffset)

			$tempSkillsetContent[$r - 1][$c - 1] = _
					_CreateBorderLabel($tempGrid[$r - 1][$c - 1], (($c - 1) * ($cellWidth + $cellLeftOffset)) + $cellLeftOffset - 20, $cellTopOffset, _
					($aSizeSkillset[2] * 2), _
					($aSizeSkillset[2] * 2), _
					$COLOR_WHITE, $penWeight, BitOR($SS_CENTER, $SS_CENTERIMAGE), $WS_EX_TRANSPARENT, 1)

			______DebugPrint______("Text:     " & $c & " --- String(array0):" & $aSizeSkillset[0] & " WidthRect(array2):" & $aSizeSkillset[2] & " --- " & _
					" Width: " & ($aSizeSkillset[2] * 2) & _
					" TextHeight:" & ($aSizeSkillset[2] * 2) & _
					" TopOffset: " & $cellTopOffset & _
					" TextSize: " & $textSize)

			If $c <> 40 Then
				GUICtrlSetData($tempSkillsetContent[$r - 1][$c - 1], $tempAssignSkillset)
;~ 				_GuiCtrlSetFont($tempSkillsetContent[$r - 1][$c - 1], $textSize, 200, 0, ($c * 2)) ; use this to see the angle change from 0 to $skillsetColumnCount
				_GuiCtrlSetFont($tempSkillsetContent[$r - 1][$c - 1], $textSize, 200, 0, 90)
			EndIf
		Next
	Next
...
Func _CreateBorderLabel($sText, $iX, $iY, $iW, $iH, $iColor, $iPenSize = 1, $iStyle = -1, $iStyleEx = 0, $iType = 0)
	Switch $iType
		...
		Case 1
			$internalLabelID2 = GUICtrlCreateLabel($sText, $iX, $iY, $iW, $iH, BitOR($SS_CENTER, $SS_CENTERIMAGE), $iStyleEx)
			GUICtrlSetBkColor(-1, $GUI_BKCOLOR_TRANSPARENT) ; swap these to see the transparent label's background area
;~ 			GUICtrlSetBkColor(-1, $COLOR_BLACK) ; swap these to see the transparent label's background area
			$totalLabels += 1
	EndSwitch
...
;~ 	;;; Uncomment this and increase the number of $skillsetColumnCount to 100+ to exhaust resources
;~ 	$skillsetColumnCount = 100
