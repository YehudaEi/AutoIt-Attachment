...
LoadSettings()
$pos= WinGetPos("","")
if $pos[2]<10 and $pos[3] <10 then WinMove("","",$pos[0],$pos[1],400,400)
GUISetState()
; GUI MESSAGE LOOP
...
Func LoadSettings()
	...
	If FileExists($sFile) Then
		While @error = 0
			...
			WinSetState("Sample GUI", "",_XMLGetValue ("Settings/State"))
			GUICtrlSetData($h_Edit, _XMLGetValue ("Settings/Edit"))
			GUICtrlSendMsg($h_tabcontrol, 0x1300 + 12, Int(_XMLGetValue ("Settings/Tab")), 0)
			GUICtrlSendMsg($h_List, 0x0186, Int(_XMLGetValue ("Settings/List")), 0)
			ControlListView("Sample GUI", "", $listView, "Select", Int(_XMLGetValue ("Settings/ListView")), Int(_XMLGetValue ("Settings/ListView")))
			GUICtrlSetState($h_Radio1, _XMLGetValue ("Settings/Radio1"))
			GUICtrlSetState($h_Radio2, _XMLGetValue ("Settings/Radio2"))
			GUICtrlSetData($h_UpDown, _XMLGetValue ("Settings/UpDown"))
			GUICtrlSetData($h_Slider, _XMLGetValue ("Settings/Slider"))
			GUICtrlSetData($h_Input, _XMLGetValue ("Settings/Input"))
			GUICtrlSetState($h_Check, _XMLGetValue ("Settings/Check"))
			GUICtrlSetState( _XMLGetValue ("Settings/Tv1"), $GUI_FOCUS)
			GUICtrlSetState(_XMLGetValue ("Settings/Tv2"), $GUI_FOCUS)
; ????? restore the check boxes ????????
;~ 			dim $aNames[1], $aValues[1]
;~ 			_XMLGetAllAttrib("Settings/Tv2/Items",$aNames,$aValues)
