...
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	Switch $snmpVER
		Case 1,2
			Local $_SNMP_Req_Varbind 	= _Build_Varbind	($snmpOID, $dataTYPE, $dataVALUE)
			If @error Then
			...
			If $PDUType = "A5" And $snmpVER = 1 Then
				...
				Return SetError(1)
			EndIf
			Local $_SNMP_Req_PDU 		= _Build_PDU		($snmpReqID, $PDUType, $GetBulk, $_SNMP_Req_Varbind)
			Local $_SNMP_Req_Message	= _Build_Message	($snmpVER, $snmpCOMM, $_SNMP_Req_PDU)
			_WriteArrayValues($SNMP_Received, 1, "SNMP Command", $_SNMP_Req_Message)
			ConsoleWrite("_SNMPBuildPacket:" & $_SNMP_Req_Message&@CRLF)
...
;------------- Error Block --------------------------------------------------------
	...
	If $_sErr_ <> "00" Then
		ConsoleWrite("_ShowSNMPReceived:" & $rcvDATA & @CRLF)
...
Func _Build_Varbind($p_OID, $p_dTYPE, $p_dVALUE)
	...
	Switch $p_dTYPE
		...
		Case $SNMP_data_COUNTER, $SNMP_data_GAUGE, $SNMP_data_TIME, $SNMP_data_STR		;(COUNTER, GAUGE, TIME, STR)
...
Func _SNMPExtractOID($strOID)
	Local $extractedOIDarray [Int(StringLen($strOID)/2) + 1]
	Local $extractedOID = "1.3."
	For $i = 2 To Int(StringLen($strOID)/2)
		...
		If Dec($extractedOIDarray[$i]) > 128 Then
			...
			Else
				...
				$i += 1
			EndIf
		ElseIf $i = Int(StringLen($strOID)/2) Then
...
Func _ThrowError($sErr, $spkt)
	Switch $sErr
		...
		Case Else
			ClipPut($spkt)
			MsgBox(16, "SNMP Error Code: "&$spkt, "Error Message:   No Error message for this one ("&$sErr&")."&@CRLF&@CRLF&"                (packet received placed in clipboard)")
	EndSwitch
	Exit
EndFunc
