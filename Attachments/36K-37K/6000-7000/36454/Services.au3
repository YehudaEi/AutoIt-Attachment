...
;			_Service_Create($sServiceName, $sServiceName, BitAND($SERVICE_WIN32_SHARE_PROCESS,$SERVICE_INTERACTIVE_PROCESS), $SERVICE_AUTO_START, $SERVICE_ERROR_SEVERE, '"' & @WindowsDir & "\" & $sServiceName & ".exe" &  '"')
...
Func _Service_init($sServiceName)
	...
	If $ret[0] = 0 Then
		...
		If $iError = 1063 Then
			MsgBox(48, "Script not compiled", "Please run this script once compiled" & @CRLF & "and start it from SCM")
		Else
			MsgBox(0, "", " Error " & $iError & @CRLF)
		EndIf
	EndIf
...
Func _Service_ServiceMain($iArg, $sArgs)
	...
	If $ret[0] = 0 Then
		MsgBox(0, "Error", _WinAPI_GetLastError_2())
		Exit
	EndIf
