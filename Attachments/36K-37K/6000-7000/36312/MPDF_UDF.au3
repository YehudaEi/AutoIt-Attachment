...
Func _ClosePDFFile()
	...
	If $_bOpen Then
		...
		If Not @error Then
			...
			$oPDF.src = $PDF_NAME
			$hGUI_UPDF = GUICreate("Visualizzatore PDF", 500, 550)
			$btnClose = GUICtrlCreateButton("&Close", 200, 500, 113, 33, 0)
			$GUI_ActiveX = GUICtrlCreateObj($oPDF, 30, 30, 450, 450)
			GUISetState(@SW_SHOW)
		Else
			MsgBox(0, "Attenzione", "Errore COM - Acrobat Reader")
		EndIf
		While 1
			$nMsg = GUIGetMsg()
			Switch $nMsg
				Case $GUI_EVENT_CLOSE
					GUIDelete($GUI_ActiveX)
;~ 					If FileExists($PDF_NAME) Then
					FileDelete($PDF_NAME)
;~ 					EndIf
					Return (False)
				Case $btnClose
...
Func _InitPDF($sFileName = "")
	...
	$_iMaxObject = 0
	__InitObj(1)
	__ToBuffer("<< /Title(" & __ToPdfStr($PDF_TITLE) & ")" & @CRLF & "/Author(" & __ToPdfStr($PDF_AUTHOR) & ")" & @CRLF & "/Creator(" & __ToPdfStr($PDF_CREATOR) & ")" & @CRLF & "/Producer(" & __ToPdfStr($PDF_COPYRIGHT) & ")" & @CRLF & "/Keywords(" & __ToPdfStr($PDF_KEYWORDS) & ")" & @CRLF & "/Subject(" & __ToPdfStr($PDF_SUBJECT) & ")" & @CRLF & "/CreationDate(D:" & @YEAR & @MON & @MDAY & @HOUR & @MIN & @SEC & "+02'00')" & @CRLF & "/ModDate()>>")
	__EndObj()
	__InitObj(2)
