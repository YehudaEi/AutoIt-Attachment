...
EndIf
	...
	;Does not allow to resize the app
	Opt("GUIResizeMode",1)
	;Allows the user to minimize the application
...
;@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	...
	WinMove("RF Attenuation Tool", "", 569, 1)
	...
	GUICtrlSetOnEvent(-1, "_exitPressed")
	;Help menu creation
	...
	GUICtrlCreateLabel("RF Attenuator ", 150, 0)
...
Func fontsize()
		...
		GUISetFont(9, 600, 2, $Msgfont)
		GUICtrlCreateLabel("Application Controls :", 25, 232)
EndFunc
...
Func Helptext()
		...
		Local $Msgfonts,$Buttonmsg = String(50),$closeButton
		$Msgfonts = "Comic Sans MS"
		GUISetFont(8, 500, 0, $Msgfonts)
		
		GUICtrlCreateLabel("Press Start button to start the Application", 20, 265)
		GUICtrlCreateLabel("Press Pause button to pause the Application", 20, 286)
		GUICtrlCreateLabel("Press Break key on the keyboard to Resume", 20, 305)
		GUICtrlCreateLabel("Press Exit button to stop the Application", 20, 325)
		GUICtrlCreateLabel("Press Lock button to lock this Application", 20, 345)
		...
		GUICtrlCreateButton("Start",20,380,80,30)
		;On key press control jumps to start function
		$startpress = GUICtrlSetOnEvent(-1, "start")
EndFunc
...
;Locking the application from being moved by the user
	...
	Local $lockbutton, $Lockfont
	
	$Lockfont = "Comic Sans MS"
	GUISetFont(8.5, 400, 0, $Lockfont)

	$lockbutton = GUICtrlCreateButton("Lock ",320,380,80,30)
	;On key press control jumps to Terminate function
	$keypress = GUICtrlSetOnEvent(-1, "detectkeypress")
...
Func detectkeypress()
	...
	;Disables the lock button aftre pressing once
...
Func TogglePause()
    ...
    While $Paused
        ...
        sleep(100)
		ToolTip('"Application is in Pause state - Press Break key to Resume"',684,9)
	WEnd
...
Func RFSeqincr()
	...
	if $chk == 1 Then
		MsgBox(64, "Delay value for sequential decrement is", $getuserdelayincr, 5)
	Elseif $chk == 0 Then
...
Func RFSeqdecr()
	...
	;Checking if $chk returned 1 or 0
	if $chk1 = 1 Then
		MsgBox(64, "Delay value for sequential decrement is", $getuserdelaydecr, 5)
	Elseif $chk1 = 0 Then
		MsgBox(16,"","Please enter numeric value if not delay will be set to default value(4sec)",10)
		;Clearing the text field
...
while(1)
	...
	;Printing the current Date and Time with RF output value
	MemoWrite("RF value is " & $rfrand & "" & @CRLF & "Date and Time after changing the RF is: " & _Now() & @CRLF )
	MemoWrite("*****************************************************")
