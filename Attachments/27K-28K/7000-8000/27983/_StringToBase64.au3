...
;=================================================================================
; Description:  converts any picture into an HTML supported base64 encryption.
; Params:  $FileFullPath	- 	IN	-  Full file path to the picture.
; Returns:  HTML encoded base 64 compatible picture.
; Author:  Armand.
;=================================================================================
Func _HTMLBase64PictureEncode($FileFullPath)
	Local $type, $filehand, $FileEncoded
	;Collect required info from path.
	$type = StringLeft($FileFullPath, 3)
	;Open the file for reading.
	$filehand = FileOpen($FileFullPath, 16)
	;Encode the file with no line breaks (Faster) and no progess bar (faster)
	$FileEncoded = _Base64Encode(BinaryToString(FileRead($filehand)), False, False)
	FileClose($filehand)
	;Return.
	Return '<img src="data:image/' & $type & ';base64,' & @CRLF & $FileEncoded & @CRLF & '"/>'
EndFunc
;===============================================================================
;
