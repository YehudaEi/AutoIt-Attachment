...
; This is needed to properly handle WM_MOUSEMOVE
Global $aCurInitPos[2] = [-1, -1]


; Make a GUI
Global $iWidthGUI = @DesktopWidth
Global $iHeightGUI = @DesktopHeight
Global $hGUI = GUICreate("OpenGL Space", $iWidthGUI, $iHeightGUI, 0, 0, $WS_POPUP, $WS_EX_TOPMOST)
...
DllStructSetData($tZoffset, 1, 0.85)
...
If Not $pRtlRandomEx Then
	$aRtlRandomEx = DllCall("kernel32.dll", "ptr", "GetProcAddress", "ptr", _WinAPI_GetModuleHandle("ntdll.dll"), "str", "RtlRandom")
...
GUIRegisterMsg(512, "_MouseMoveExit") ; WM_MOUSEMOVE

; Some other
GUIRegisterMsg(256, "_Quit") ; WM_KEYDOWN
GUIRegisterMsg(260, "_Quit") ; WM_SYSKEYDOWN
GUIRegisterMsg(513, "_Quit") ; WM_LBUTTONDOWN
GUIRegisterMsg(516, "_Quit") ; WM_RBUTTONDOWN
GUIRegisterMsg(519, "_Quit") ; WM_MBUTTONDOWN
GUIRegisterMsg(522, "_Quit") ; WM_MOUSEWHEEL
...
Func _MouseMoveExit($hWnd, $iMsg, $wParam, $lParam)
	...
	If $aCurInitPos[0] = -1 And $aCurInitPos[1] = -1 Then
		$aCurInitPos[0] = $aCursorPos[0]
		$aCurInitPos[1] = $aCursorPos[1]
	Else
		If $aCursorPos[0] <> $aCurInitPos[0] And $aCursorPos[1] <> $aCurInitPos[1] Then
			_Quit()
		EndIf
	EndIf

EndFunc   ;==>_MouseMoveExit



; OpenGL functions used with DllCall()
