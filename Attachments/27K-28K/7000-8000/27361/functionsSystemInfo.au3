	...
	For $element In $asDatabase
		...
		For $toolsFile In $asItem
			If $toolsFile = "" Or $toolsFile = $toolsDirName Then ContinueLoop
				...
				GUICtrlSetOnEvent($toolsFile, "_ItemFunction")
				Func _ItemFunction()
				While @InetGetActive
					InetGet ($toolsDir & $toolsFile, $toolsFile, 1, 1)
					$filesize = InetGetSize($toolsDir & $toolsDirName & "/" & $toolsFile)
					$currentsize = @InetGetBytesRead
					$percentDownloaded = Round((($currentsize / $filesize) * 100), 1) & "% Complete"
					TrayTip("Downloading " & $toolsFile, "File: " & $toolsFile & " " & $percentDownloaded, 10, 16)
				Wend
				TrayTip("Download Complete", "You have successfully downloaded " & $toolsFile,2,16)	
				ShellExecute($toolsFile, $tempDir, "run")
				EndFunc
		Next
	Next
EndFunc
