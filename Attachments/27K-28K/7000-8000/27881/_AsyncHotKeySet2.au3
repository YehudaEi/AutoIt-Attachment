...
Global Const $ahks_kCallOn    = 4 ; set when the func is called. 1 for keydown (default), 2 for keyup, 0 for both and an additional param will be sent for keydown/keyup.
	...
	For $index=0 To $ahks_CurrentKeyCount-1
		...
		if __IsPressed($iVKey, $hDLL) then
			...
			if $iKeyLastState=$iAllHotKeyBits then
				...
				if $ahks_keyMap[$index][$ahks_kCallOn] = 0 then ; calls func when keydown and keyup, send true for keydown
				...
				elseif $ahks_keyMap[$index][$ahks_kCallOn] = 1 then ; calls func when keydown
					if $ahks_keyMap[$index][$ahks_kSendParam] = 1 then
		...
		; Main Key *not* pressed if got here
			...
			if $iKeyLastState=$ahks_HotKeyInvokedBit then
				...
				elseif $ahks_keyMap[$index][$ahks_kCallOn] = 2 then ; calls func when keyup
					if $ahks_keyMap[$index][$ahks_kSendParam] = 1 then
						Call($ahks_keyMap[$index][$ahks_kUserFunc], $iKeyCode)
					else
						Call($ahks_keyMap[$index][$ahks_kUserFunc])
