...
global Const $cI_CompName = @ComputerName
...
For $i = 1 To $BootConfig[0][0] Step 1
		...
		_ArrayAdd($DATA,	 $BootConfig[$i][0] )
		_ArrayAdd($DATA,	"Boot Directory: " & $BootConfig[$i][1] )
		_ArrayAdd($DATA,	"Configuration Path: " & $BootConfig[$i][2] )
		_ArrayAdd($DATA,	"Description: " & $BootConfig[$i][4] )
		_ArrayAdd($DATA,	"Scratch Directory: " & $BootConfig[$i][5] )
		_ArrayAdd($DATA,	"Temp Directory: " & $BootConfig[$i][7])
		_ArrayAdd($DATA,	"--------------------------------------")
	Next
...
If @error Then
	...
	EndSwitch
EndIf
_ArrayAdd($DATA,"ComputerGetLoggedOnUsers")
For $i = 1 To $LoggedOnUser[0][0] Step 1



			if $LoggedOnUser[$i][1] <> "system" or "Local system" then
			_ArrayAdd($DATA,"UserName: " & $LoggedOnUser[$i][1])
			_ArrayAdd($DATA,"LogonID: " & $LoggedOnUser[$i][2])
	EndIf
		Next
		_ArrayAdd($DATA,	"--------------------------------------")
...
If @error Then
	...
	EndSwitch
EndIf
_ArrayAdd($DATA,"ComputerGetOS config")
For $i = 1 To $OSs[0][0] Step 1

		_ArrayAdd($DATA	,"Boot Device: " & $OSs[$i][1] )
		_ArrayAdd($DATA	,"Build Number: " & $OSs[$i][2] )
		_ArrayAdd($DATA	,"Build Type: " & $OSs[$i][3] )
		_ArrayAdd($DATA	,"Country Code: " & $OSs[$i][6])
		_ArrayAdd($DATA	,"Creation Class Name: " & $OSs[$i][7] )
		_ArrayAdd($DATA	,"CS Name: " & $OSs[$i][10] )
		_ArrayAdd($DATA	,"Current Time Zone: " & $OSs[$i][11] )
		_ArrayAdd($DATA	,"Data Execution Prevention_32BitApplications: " & $OSs[$i][12] )
		_ArrayAdd($DATA	,"Data Execution Prevention_Available: " & $OSs[$i][13] )
		_ArrayAdd($DATA	,"Data Execution Prevention_Drivers: " & $OSs[$i][14] )
		_ArrayAdd($DATA	,"Data Execution Prevention_SupportPolicy: " & $OSs[$i][15] )
		_ArrayAdd($DATA	,"Encryption Level: " & $OSs[$i][18])
		_ArrayAdd($DATA	,"Foreground Application Boost: " & $OSs[$i][19] )
		_ArrayAdd($DATA	,"Free Physical Memory: " & $OSs[$i][20] )
		_ArrayAdd($DATA	,"Free Space In Paging Files: " & $OSs[$i][21] )
		_ArrayAdd($DATA	,"Free Virtual Memory: " & $OSs[$i][22] )
		_ArrayAdd($DATA	,"Install Date: " & $OSs[$i][23] )
		_ArrayAdd($DATA	,"Large System Cache: " & $OSs[$i][24] )
		_ArrayAdd($DATA	,"Last Boot Up Time: " & $OSs[$i][25] )
		_ArrayAdd($DATA	,"Local Date Time: " & $OSs[$i][26] )
		_ArrayAdd($DATA	,"Locale: " & $OSs[$i][27] )
		_ArrayAdd($DATA	,"Manufacturer: " & $OSs[$i][28] )
		_ArrayAdd($DATA	,"Max Process Memory Size: " & $OSs[$i][30] )
		_ArrayAdd($DATA	,"Number Of Licensed Users: " & $OSs[$i][31] )
		_ArrayAdd($DATA	,"Number Of Processes: " & $OSs[$i][32] )
		_ArrayAdd($DATA	,"Number Of Users: " & $OSs[$i][33] )
		_ArrayAdd($DATA	,"Organization: " & $OSs[$i][34] )
		_ArrayAdd($DATA	,"OS Language: " & $OSs[$i][35] )
		_ArrayAdd($DATA	,"OS Product Suite: " & $OSs[$i][36])
		_ArrayAdd($DATA	,"OS Type: " & $OSs[$i][37] )
		_ArrayAdd($DATA	,"Primary: " & $OSs[$i][41])
		_ArrayAdd($DATA	,"Registered User: " & $OSs[$i][45] )
		_ArrayAdd($DATA	,"Serial Number: " & $OSs[$i][46] )
		_ArrayAdd($DATA	,"Service Pack Major Version: " & $OSs[$i][47] )
		_ArrayAdd($DATA	,"Service Pack Minor Version: " & $OSs[$i][48] )
		_ArrayAdd($DATA	,"Size Stored In Paging Files: " & $OSs[$i][49] )
		_ArrayAdd($DATA	,"Status: " & $OSs[$i][50] )
		_ArrayAdd($DATA	,"System Device: " & $OSs[$i][52] )
		_ArrayAdd($DATA	,"System Directory: " & $OSs[$i][53] )
		_ArrayAdd($DATA	,"System Drive: " & $OSs[$i][54] )
		_ArrayAdd($DATA	,"Total Swap Space Size: " & $OSs[$i][55] )
		_ArrayAdd($DATA	,"Total Virtual Memory Size: " & $OSs[$i][56] )
		_ArrayAdd($DATA	,"Total Visible Memory Size: " & $OSs[$i][57] )
		_ArrayAdd($DATA	,"Version: " & $OSs[$i][58] )
		_ArrayAdd($DATA	,"Windows Directory: " & $OSs[$i][59])
...
If @error Then
	...
	EndSwitch
EndIf
_ArrayAdd($DATA,"ComputerGetShares")
For $i = 1 To $Shares[0][0] Step 1

			_ArrayAdd($DATA,"Name: " & $Shares[$i][0])
			_ArrayAdd($DATA,"Description: " & $Shares[$i][4] )
			_ArrayAdd($DATA,"Path: " & $Shares[$i][5] )
			_ArrayAdd($DATA,"Status: " & $Shares[$i][6] )
...
For $i = 1 To $Software[0][0] Step 1
		...
		_ArrayAdd($DATA,	"ComputerGetSoftware")
		_ArrayAdd($DATA,	"Name: " & $Software[$i][0])
		_ArrayAdd($DATA,	"Version: " & $Software[$i][1] )
		_ArrayAdd($DATA,	"Publisher: " & $Software[$i][2] )
		_ArrayAdd($DATA,	"Uninstall Path: " & $Software[$i][3])
Next
#endregion Software
_ArrayAdd($DATA,	"--------------------------------------")
...
If @error Then
	...
	EndSwitch
EndIf
_ArrayAdd($DATA,"ComputerGetBIOS")
For $i = 1 To $BIOS[0][0] Step 1


		_ArrayAdd($DATA,"Name: " & $BIOS[$i][0])
		_ArrayAdd($DATA,"Status: " & $BIOS[$i][1] )
		_ArrayAdd($DATA,	"BIOS Version: " & $BIOS[$i][3] )
		_ArrayAdd($DATA,	"Description: " & $BIOS[$i][4] )
		_ArrayAdd($DATA,	"Manufacturer: " & $BIOS[$i][12] )
		_ArrayAdd($DATA,	"Release Date: " & $BIOS[$i][15] )
		_ArrayAdd($DATA,	"Serial Number: " & $BIOS[$i][16] )
...
_ArrayAdd($DATA,"ComputerGetMemory")
For $i = 1 To $Memory[0][0] Step 1

		_ArrayAdd($DATA,	"Capacity: " & $Memory[$i][2])
		_ArrayAdd($DATA,	"DeviceLocator: " & $Memory[$i][6])
		_ArrayAdd($DATA,	"Manufacturer: " & $Memory[$i][11] )
		_ArrayAdd($DATA,	"PartNumber: " & $Memory[$i][15] )
		_ArrayAdd($DATA,	"SerialNumber: " & $Memory[$i][20] )
		_ArrayAdd($DATA,	"Speed: " & $Memory[$i][22] )
...
If @error Then
	...
	EndSwitch
EndIf
_ArrayAdd($DATA,"ComputerGetPrinters")
For $i = 1 To $Print[0][0] Step 1

		_ArrayAdd($DATA, "Name: " & $Print[$i][0] )
		_ArrayAdd($DATA,	"DeviceID: " & $Print[$i][28] )
		_ArrayAdd($DATA,	"DoCompleteFirst: " & $Print[$i][30])
		_ArrayAdd($DATA,	"DriverName: " & $Print[$i][31] )
		_ArrayAdd($DATA,	"Location: " & $Print[$i][47] )
		_ArrayAdd($DATA,	"PortName: " & $Print[$i][59] )
...
If @error Then
	...
	EndSwitch
EndIf
_ArrayADD($DATA,"ComputerGetProcessors")
For $i = 1 To $Processors[0][0] Step 1


		_ArrayAdd($DATA,	"Name: " & $Processors[$i][0] )
		_ArrayAdd($DATA,	"Current Clock Speed: " & $Processors[$i][9] )
		_ArrayAdd($DATA,	"Manufacturer: " & $Processors[$i][22] )
		_ArrayAdd($DATA,	"Max Clock Speed: " & $Processors[$i][23] )
...
If @error Then
	...
	EndSwitch
EndIf
_ArrayAdd($DATA,"ComputerGetSystem")
For $i = 1 To $System[0][0] Step 1


			_ArrayAdd($DATA,	"Name: " & $System[$i][0] )
			_ArrayAdd($DATA,	"Domain: " & $System[$i][13] )
			_ArrayAdd($DATA,	"Manufacturer: " & $System[$i][21] )
			_ArrayAdd($DATA,	"Model: " & $System[$i][22] )
			_ArrayAdd($DATA,	"Number Of Processors: " & $System[$i][25] )
			_ArrayAdd($DATA,	"Total Physical Memory: " & $System[$i][48] )
			_ArrayAdd($DATA,	"User Name: " & $System[$i][49] )
...
If @error Then
	...
	EndSwitch
EndIf
_ArrayAdd($DATA,"ComputerGetVideoCards")
For $i = 1 To $VideoCards[0][0] Step 1

_ArrayAdd($DATA, "Name: " & $VideoCards[$i][0])
Next
_ArrayAdd($DATA,	"--------------------------------------")
...
_FileWriteFromArray($hFile, $data, 1)
FileClose($hFile)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
#include <Excel.au3>
_ArrayDisplay($DATA, "$avArray as a list classes in window")

$oExcel = _ExcelBookNew(0)
_ExcelWriteSheetFromArray($oExcel, $aRecords, 1, 1, 0, 0) ;0-Base Array parameters
_ExcelBookSaveAs($oExcel, @ScriptDir &"\tmp\"& $cI_CompName, "xls", 0, 1) ; Now we save it into the temp directory; overwrite existing file if necessary
