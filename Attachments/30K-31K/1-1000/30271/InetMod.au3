...
Func _INetSmtpMail ( $s_SmtpServer, $s_FromName, $s_FromAddress, $s_ToAddress, $s_Subject = "", $as_Body = "", $s_Username = "", $s_Password = "", $s_helo = "", $s_first = " ", $b_trace = 0)
	...
	Local $s_Send[9], $s_ReplyCode[9]	; Return code from SMTP server indicating success
	;$s_Send[0] = "HELO " & $s_helo & @CRLF
	;If StringLeft($s_helo,5) = "EHLO " Then $s_Send[0] = $s_helo & @CRLF
	$s_Send[0] = "EHLO " & @ComputerName & @CRLF ;override for testing
	$s_ReplyCode[0] = "250"

;Encode username and password and write to array ======================================================
	If $s_Username <> "" AND $s_Password <> "" Then
		$s_Send[1] = "AUTH LOGIN" & @CRLF
		$s_Send[2] = _Base64Encode($s_Username) & @CRLF
		$s_Send[3] = _Base64Encode($s_Password) & @CRLF
		$s_ReplyCode[1] = "335"
		$s_ReplyCode[2] = "335"
		$s_ReplyCode[3] = "235"
	EndIf
	...
	;Remove authentication rows if not in use =============================================
	If $s_Send[1] = "" OR $s_Send[2] = "" OR $s_Send[3] = "" Then
		_ArrayDelete($s_Send, 3)
		_ArrayDelete($s_Send, 2)
		_ArrayDelete($s_Send, 1)
		_ArrayDelete($s_ReplyCode, 3)
		_ArrayDelete($s_ReplyCode, 2)
		_ArrayDelete($s_ReplyCode, 1)
	EndIf
...
; ===============================================================================
;		*************NOTHING HAS BEEN MODIFIED BELOW THIS LINE*************
; ===============================================================================
