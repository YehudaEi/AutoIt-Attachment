...
; ==============================================================================
; UDF ...........: FF.au3
Global Const $_FF_AU3VERSION = "0.6.0.0b-5"
...
; Date ..........: Fri Nov 13 18:32:04 CET 2009 @772 /Internet Time/
; FireFox Version: Firefox/3.5.3 (required 3.x.x)
...
#Region Global Variables
	...
	Global $_FF_LOADWAIT_TIMEOUT = 600000;120000
...
; Date ..........: Fri Nov 06 22:12:29 CET 2009
...
;                  @ERROR       -
; Author(s) .....: Thorsten Willert
; Date ..........: Fri Sep 25 12:01:48 CEST 2009 @459 /Internet Time/
; Link ..........:
; Related .......: _FFImageClick, _FFLinkClick
; Example .......: Yes
; ==============================================================================
...
;                  @ERROR       -
; Author(s) .....: Thorsten Willert
; Date ..........: Mon Nov 09 19:02:40 CET 2009
; Link ..........:
...
;                  @ERROR       -
; Author(s) .....: Thorsten Willert
; Date ..........: Thu Sep 24 17:27:40 CEST 2009 @685 /Internet Time/
; Link ..........:
...
Func _FFLinkClick($vSearch, $sMode = "href", $bLoadWait = True)
	...
	If $sMode <> "index" Then
		If _FFClick( _FFXPath($sX, "", 9) ) Then
			If $bLoadWait Then Return _FFLoadWait()
			Return 1
		EndIf
	Else
...
;                  @ERROR       -
; Author(s) .....: Thorsten Willert
; Date ..........: Fri Nov 06 18:12:43 CET 2009
; Link ..........:
...
; AutoIt Version : V3.3.0.0
; Syntax ........: _FFDispatchEvent($sElement[, $sEventType = "change"[, $iKeyCode = 0]])
...
;                  @ERROR       -
; Author(s) .....: Thorsten Willert
; Date ..........: Fri Nov 06 19:35:04 CET 2009
...
Func _FFDispatchEvent($sElement, $sEventType = "change", $iKeyCode = 13)
	...
	Switch $sEventType
		...
		Case "keydown", "keypress", "keyup"
			$sType = "KeyboardEvent"
		Case Else
			SetError(__FFError($sFuncName, $_FF_ERROR_InvalidValue, "(change|select|focus|blur|resizekeydown|keypress|keyup) $sEventType: " & $sEventType))
			Return 0
	EndSwitch
...
; AutoIt Version : V3.3.0.0
; Syntax ........: _FFLoadWait([$iTimeOut = $_FF_LOADWAIT_TIMEOUT])
...
;                  @EXTENDED    - loading time in ms
; Author(s) .....: Thorsten Willert
; Date ..........: Thu Nov 05 18:27:55 CET 2009
; Link ..........: https://developer.mozilla.org/en/nsIWebProgress
; Related .......: _FFDialogWait
; Example .......: Yes
; ==============================================================================
Func _FFLoadWait($iTimeOut = $_FF_LOADWAIT_TIMEOUT)
	...
	While TimerDiff($TimeOutTimer) < $iTimeOut
		...
		If Not _FFCmd("window.getBrowser().webProgress.isLoadingDocument") And _
			...
			$_FF_COM_TRACE = $bTrace
			Return SetError(0, $iLoadingTime - $_FF_CON_DELAY - 25, 1)
		EndIf
		Sleep(250)
	WEnd
	$_FF_COM_TRACE = $bTrace
...
;                  @ERROR       -
; Author(s) .....: Thorsten Willert
; Date ..........: Wed Nov 04 18:17:25 CET 2009
; Link ..........:
; Related .......:
; Example .......: Yes
; ==============================================================================
Func _FFGetPosition($sElement = "")
	...
	If @error Then
		SetError(__FFError($sFuncName, $_FF_ERROR_NoMatch, "Cant find 'MozillaContentWindowClass'-control"))
		Return
	EndIf
...
;                  @ERROR       -
; Author(s) .....: Thorsten Willert
; Date ..........: Fri Sep 18 06:06:26 CEST 2009 @212 /Internet Time/
...
;                  @ERROR       -
; Author(s) .....: Thorsten Willert
; Date ..........: Fri Nov 06 17:53:58 CET 2009
; Link ..........:
; Related .......:
; Example .......: Yes
; ==============================================================================
Func _FFAu3Option($sOption, $vValue="")
	...
	Switch $sOption
		...
		Case Else
			SetError(__FFError($sFuncName, $_FF_ERROR_InvalidValue, "(LoadWaitTimeOut|ComTrace|ErrorMsgBox) $sOption" & $sOption))
	EndSwitch
...
Func __FFValue2JavaScript(ByRef $sValue)
	...
	If StringRegExp($sValue, "[^\x00-\x7F]", 0) Then
		;$sValue = BinaryToString(StringToBinary($sValue,1),4)
...
; AutoIt Version : V3.3.0.0
; Syntax ........: __FFSendJavaScripts([$bRefresh = False[, $bTrace = False]])
; Parameter(s): .: $bRefresh    - Optional: (Default = false) : Send again to FF
; Return Value ..: Success      - 1
;                  Failure      - 0
; Author(s) .....: Thorsten Willert
; Date ..........: Wed Oct 28 15:33:16 CET 2009 @648 /Internet Time/
...
Func __FFSendJavaScripts($bRefresh = true, $bTrace = False)
	...
	Local $sJavaScript = 'FFau3=new Object();FFau3.tmp="";FFau3.obj=null;FFau3.WCD=window.content.top.document;'
	...
	; X = clientX
	; Y = clientY
	$sJavaScript = 'FFau3.simulateEvent=function simulateEvent(oObject,sEvent,sType){'
	$sJavaScript &= 'try{ var evt=document.createEvent(sEvent);'
	$sJavaScript &= 'switch(sEvent){'
	$sJavaScript &= ' case "MouseEvents":'
	$sJavaScript &= '  evt.initMouseEvent(sType,true,true,window,0,0,0,simulateEvent.arguments[4],simulateEvent.arguments[5],false,false,false,false,0,null);break;'
	$sJavaScript &= ' case "KeyboardEvent":'
	$sJavaScript &= '  evt.initKeyEvent(sType,true,true,null,false,false,false,false,simulateEvent.arguments[3],0);break;'
	$sJavaScript &= ' case "Event":'
	$sJavaScript &= '  evt.initEvent(sType,true,true);break; }'
	$sJavaScript &= 'oObject.dispatchEvent(evt);'
	$sJavaScript &= 'return 1;'
	$sJavaScript &= '}catch(e){return -3;}return 0;};'

	_FFCmd($sJavaScript)
	ConsoleWrite(".")

	; GetLinkInfo(): JavaScript to get all informations about a link
	$sJavaScript = 'FFau3.GetLinkInfo = function GetLinkInfo(i,sLimiter)'
	$sJavaScript &= '{ var info=""; with(FFau3.WCD) {'
	$sJavaScript &= 'try {'
	...
	$sJavaScript = 'FFau3.GetLinks = function GetLinks(sLimiter)'
	$sJavaScript &= '{ var res=""; with(FFau3.WCD) {try{'
	$sJavaScript &= 'for (var i=0;i<links.length;i++){res += this.GetLinkInfo(i,sLimiter) + "\n";}'
	...
	$sJavaScript = 'FFau3.SearchImageBySize = function SearchImageBySize(iH, iW, sMode, iP){'
	$sJavaScript &= 'var sRet=""; with(FFau3.WCD) {'
	...
	$sJavaScript &= 'case "eq": if ((iWidth<=iW+iW/100*iP && iHeight<=iH+iH/100*iP && iWidth>=iW-iW/100*iP && iHeight>=iH-iH/100*iP)) sRet += " " + i; break;'
	$sJavaScript &= 'case "lt": if ((iWidth<=iW && iHeight<=iH) && (iWidth>=iW-iW/100*iP && iHeight>=iH-iH/100*iP)) sRet += " " + i; break;'
	$sJavaScript &= 'case "gt": if ((iWidth>=iW && iHeight>=iH) && (iWidth<=iW+iW/100*iP && iHeight<=iH+iH/100*iP)) sRet += " " + i; break;'
	...
	$sJavaScript = "FFau3.findPos = function findPos(obj){try{"
	$sJavaScript &= "var curleft=curtop=0;"
	$sJavaScript &= "if (obj.offsetParent) {"
	$sJavaScript &= "do {curleft+=obj.offsetLeft;"
	$sJavaScript &= "  curtop+=obj.offsetTop;"
	$sJavaScript &= "} while (obj=obj.offsetParent);"
	$sJavaScript &= "return curleft+' '+curtop;"
	$sJavaScript &= "}}catch(e){return -3}}"
...
Func __FFWaitForRepl($iTimeOut)
	Local Const $sFuncName = "__FFWaitForRepl"
