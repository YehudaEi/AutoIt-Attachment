...
;
;Version 1.2a  ControlSend is not reliable, it sends 'sometimes' lower case and transforms : to ., A to a 
;              changed all back to send() 
		...
		while $found < 3 and secs() < $timeout
			...
			While 1
				...
				if stringinstr($Message,$wexpr[$found]) then ;wait for the first expression in the array and select the next array entry
				    ;we have definitely a log in window, somebody might have clicked to somewhere else and activated another window
					;in between, so we'll return to PuTTY and try to keep it active
					winactivate($putties[$putties[0]])
					winwaitactive($putties[$putties[0]])
					SendKeepActive($putties[$putties[0]])
