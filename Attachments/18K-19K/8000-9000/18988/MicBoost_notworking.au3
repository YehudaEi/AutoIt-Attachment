...
GUICtrlSetFont(-1, 10, 400, 0, "MS Sans Serif")
$RAD_WaveOut = GUICtrlCreateRadio("TEST", 248, 128, 105, 17)
...
;might be more accurate if use steps from control but this suits my needs
	...
	Case $msg=$RAD_WaveOut
		...
		GetMXWaveoutID2($mixers,$curmixer,$MIXERLINE_COMPONENTTYPE_SRC_MICROPHONE)
		$savevolume=GetSetOutVolume($mixers,$curmixer,0,0)
		$mxvol=$savevolume
		GUICtrlSetData($SL_WaveOut,BitAND($mxvol/ 0xFFFF * 100,0xffff))
		GUICtrlSetData($LB_vol,BitAND($mxvol/ 0xFFFF * 100,0xffff))
...
Func GetMXWaveoutID2(ByRef $mixers,$index,$linetype)
...
"dword;" & _ ; DWORD cbStruct; 								;1
"dword;" & _ ;   DWORD dwControlID; 						;2
"dword;" & _ ;   DWORD dwControlType;  						;3
"dword;" & _ ;   DWORD fdwControl;    						;4
"dword;" & _ ;   DWORD cMultipleItems; 						;5
"char[16];" & _ ;szShortName[MIXER_SHORT_NAME_CHARS]; 		;6
"char[64];" & _ ;szName[MIXER_LONG_NAME_CHARS]; 			;7
"dword;" & _	;lMinimum									;8
"dword;" & _	;lMaximum									;9
"dword[4];" & _ ;dwReserved[4];								;10
"dword;"  & _ 	;cSteps										;11
"dword[5]") ;   DWORD dwReserved[6];						;12
   If @error Then return False
Local $mixercontrol2=DllStructCreate( _ 
"dword;" & _ ; DWORD cbStruct; 								;1
"dword;" & _ ;   DWORD dwControlID; 						;2
"dword;" & _ ;   DWORD dwControlType;  						;3
"dword;" & _ ;   DWORD fdwControl;    						;4
...
"ptr") ;    	  LPMIXERCONTROL pamxctrl; 
	...
	DllStructSetData($mixerlinecontrols,3,$mxline[$cControls] )
	...
	$ret = DLLCall("winmm.dll","long","mixerGetLineControls","hwnd",$hmxobj,"ptr",DllStructGetPtr($mixerlinecontrols),"long",$MIXER_GETLINECONTROLSF_ALL)
	If $ret[0]<>$MMSYSERR_NOERROR Then Return False	
	for $k = 1 to 12
		ConsoleWrite("DllStructGetData($mixerlinecontrols,"&$k&"] : " & DllStructGetData($mixercontrol,$k) & @CRLF)
	next
	
	DllStructSetData($mixerlinecontrols,$cbStruct,DllStructGetSize($mixerlinecontrols))
	DllStructSetData($mixerlinecontrols,2,$lineid)
	DllStructSetData($mixerlinecontrols,3, DllStructGetData($mixercontrol,3))
	DllStructSetData($mixerlinecontrols,4,1)
	DllStructSetData($mixerlinecontrols,5,DllStructGetSize($mixercontrol2))
	DllStructSetData($mixerlinecontrols,6,DllStructGetPtr($mixercontrol2))
	$ret = DLLCall("winmm.dll","long","mixerGetControlDetails","hwnd",$hmxobj,"ptr",DllStructGetPtr($mixerlinecontrols),"long",0)
	ConsoleWrite($ret[0] & @CRLF)
	If $ret[0]<>$MMSYSERR_NOERROR Then Return False	
	for $k = 1 to 12
		ConsoleWrite("DllStructGetData($mixerlinecontrols,"&$k&"] : " & DllStructGetData($mixercontrol2,$k) & @CRLF)
	next
...
;~ 	for $k = 1 to 6
;~ 		ConsoleWrite("DllStructGetData($mixerlinecontrols,"&$k&"] : " & DllStructGetData($mxcd,$k) & @CRLF)
;~ 	next
...
;~ 	for $k = 1 to 12
;~ 		ConsoleWrite("DllStructGetData($mixercontrol,"&$k&"] : " & DllStructGetData($plistbool,$k) & @CRLF)
;~ 	next
	If @error Then Return False
