...
;
; Script Function:
;	IconFileScanner 1.3.2 (Modifications by G.Sandler (MsCreatoR)/ptrex)
;
; ----------------------------------------------------------------------------
...
$Drag_Seperator = GUICtrlCreateSeperator(1, 392, 23, 2, 280)
...
Func MainEvents()
	Switch @GUI_CtrlId
		...
		Case $Drag_Seperator
			Local $GetCurInfo = GUIGetCursorInfo($hGUI)
			If @error Then Return
			
			Local $X_MPos = $GetCurInfo[0]
			Local $X_Check_MPos = $X_MPos
			Local $Move_Diff
			Local $nDirList_Pos, $nIconList_Pos, $nFileInfo_Pos, $nIconInfo_Pos, $Drag_Sep_Pos
			
			While $GetCurInfo[2] = 1
				$X_Check_MPos = $GetCurInfo[0]
				
				If $X_MPos <> $X_Check_MPos Then
					$nDirList_Pos = ControlGetPos($hGUI, "", $nDirList)
					$nIconList_Pos = ControlGetPos($hGUI, "", $nIconList)
					$nFileInfo_Pos = ControlGetPos($hGUI, "", $nFileInfo)
					$nIconInfo_Pos = ControlGetPos($hGUI, "", $nIconInfo)
					$Drag_Sep_Pos = ControlGetPos($hGUI, "", $Drag_Seperator)
				EndIf
				
				If $X_MPos > $X_Check_MPos And $nDirList_Pos[2] >= 100 Then
					$Move_Diff = $X_MPos - $X_Check_MPos
					MoveSeperator($nDirList_Pos, $nIconList_Pos, $nFileInfo_Pos, $nIconInfo_Pos, $Drag_Sep_Pos, $Move_Diff, 1)
					$nOldItem = -1
					RefreshItem(GUICtrlRead($nDirList))
				ElseIf $X_MPos < $X_Check_MPos And $nIconList_Pos[2] > 100 Then
					$Move_Diff = $X_Check_MPos - $X_MPos
					MoveSeperator($nDirList_Pos, $nIconList_Pos, $nFileInfo_Pos, $nIconInfo_Pos, $Drag_Sep_Pos, $Move_Diff, 0)
					$nOldItem = -1
					RefreshItem(GUICtrlRead($nDirList))
				EndIf
				
				$X_MPos = $X_Check_MPos
				$GetCurInfo = GUIGetCursorInfo($hGUI)
				Sleep(10)
			WEnd
		Case $OpenWithItem
...
Func MoveSeperator($nDirList_Pos, $nIconList_Pos, $nFileInfo_Pos, $nIconInfo_Pos, $Drag_Sep_Pos, $Move_Diff, $Direction=0)
	$nDirList_W = $nDirList_Pos[2] + $Move_Diff
	$nIconList_L = $nIconList_Pos[0] + $Move_Diff
	$nIconList_W = $nIconList_Pos[2] - $Move_Diff
	$nFileInfo_W = $nFileInfo_Pos[2] + $Move_Diff
	$nIconInfo_L = $nIconInfo_Pos[0] + $Move_Diff
	$nIconInfo_W = $nIconInfo_Pos[2] - $Move_Diff
	$Drag_Seperator_L = $Drag_Sep_Pos[0] + $Move_Diff
	
	If $Direction = 1 Then
		$nDirList_W -= ($Move_Diff * 2)
		$nIconList_L -= ($Move_Diff * 2)
		$nIconList_W += ($Move_Diff * 2)
		$nFileInfo_W -= ($Move_Diff * 2)
		$nIconInfo_L -= ($Move_Diff * 2)
		$nIconInfo_W += ($Move_Diff * 2)
		$Drag_Seperator_L -= ($Move_Diff * 2)
	EndIf
	
	GUICtrlSetPos($nDirList, 0, $nDirList_Pos[1], $nDirList_W)
	GUICtrlSetPos($nIconList, $nIconList_L, $nIconList_Pos[1], $nIconList_W)
	
	GUICtrlSetPos($nFileInfo, 0, $nFileInfo_Pos[1], $nFileInfo_W)
	GUICtrlSetPos($nIconInfo, $nIconInfo_L, $nIconInfo_Pos[1], $nIconInfo_W)
	
	GUICtrlSetPos($Drag_Seperator, $Drag_Seperator_L, $Drag_Sep_Pos[1])
EndFunc
...
Func GUICtrlCreateSeperator($Direction, $Left, $Top, $Width=3, $Lenght=25)
	Switch $Direction
		...
		Case 1
			Return GUICtrlCreateLabel("", $Left, $Top, $Width, $Lenght, $SS_SUNKEN)
	EndSwitch
EndFunc
