...
Global $VisVillages,$aResult,$iRows,$iCols,$Villages,$ContextMenu,$ContextMenuItem,$CurrX,$CurrY,$File,$FileP
Global $BkColors[6]=["",0xBBBBBB,0xFFBBBB,0x99BBFF,0xBBFFBB,0x000000]
Global $TribeNames[6]=["","Romans","Teutons","Gauls","Nature","Natar"]

$FindFile=FileFindFirstFile(@Scriptdir&"\*.sql")
FileClose($FindFile)
If $FindFile=-1 Then
	Switch MsgBox(35,"SQL file not found","There doesn't appear to be any SQL dump files in your Documents; would you like to download one now?"&@CRLF&@CRLF&"(Clicking no will allow you to browse for another location)")
		Case 2
			Exit
		Case 7
			$FileP=FileOpenDialog("Select File","","SQL files (*.sql)")
			If @error Then Exit
			$File=FileOpen($FileP,0)
		Case 6
			DlOpenFile()
	EndSwitch
Else
...
Func DlOpenFile()
	While 1
		$FileP=InputBox("Server address","Please enter the server address you wish to download:","http://s1.travian.com")
		If @error OR $FileP="" Then Exit
		If StringInStr($FileP,".",0,3)=0 AND StringInStr($FileP,".",0,2) <> 0 Then ExitLoop
		MsgBox(16,"Error","Please just provide the server address (for instance, nothing after .com)")
	WEnd
	If StringTrimLeft($FileP,StringInStr($FileP,".",0,2))="de" OR StringTrimLeft($FileP,StringInStr($FileP,".",0,2))="org" Then
		Local $SQLname="/karte.sql"
	Else
		Local $SQLname="/map.sql"
	EndIf
	GUICreate("Doanloading Travian dump",250,100,-1,-1,0x80800000)
	GUISetBkColor(0xE1EED3)
	GUICtrlCreateLabel("Downloading "&$FileP&$SQLname,0,10,250,25,0x1)
	GUICtrlCreateLabel("Please wait...",0,35,250,30,0x1)
	GUISetState()
	If InetGet($FileP&$SQLname,@Scriptdir&"\"&StringReplace(StringReplace($FileP,"http://",""),".","")&".sql",1)=0 Then
		MsgBox(16,"Error","There was a problem downloading "&$FileP&$SQLname&@CRLF&@CRLF&"This program will now exit.")
		Exit
	EndIf
	GUIDelete()
	Sleep(500)
	$FileP=@Scriptdir&"\"&StringReplace(StringReplace($FileP,"http://",""),".","")&".sql"
	$File=FileOpen($FileP,0)
	If @error Then
		MsgBox(16,"Error","There was a problem downloading "&$FileP&$SQLname&@CRLF&@CRLF&"This program will now exit.")
		Exit
...
_SQLite_Startup()
$PersistentDB=_SQLite_Open(StringReplace($FileP,".sql",".db"))
...
GUICreate("Travian 13x13 map",600,680)
GUISetBkColor(0xE1EED3)
...
$PlayerCombo=GUICtrlCreateCombo("Populating...",5,630,125,20)
...
If $iRows < 2 Then
	SetMap()
Else
	SetMap($aResult[1][1],$aResult[2][1])
EndIf
PopulateFavorites()
...
HotKeySet("{UP}","GoNorth")
HotKeySet("{DOWN}","GoSouth")
HotKeySet("{LEFT}","GoWest")
HotKeySet("{Right}","GoEast")
...
While 1
	If _GUICtrlEdit_GetModify($XInput) OR _GUICtrlEdit_GetModify($YInput) AND GUICtrlRead($PlayerCombo) <> "Populating..." Then SetMap(GUICtrlRead($XInput)-6,GUICtrlRead($YInput)-6)
	Sleep(150)
WEnd

#Region navigation
Func GoNorth()
	If GUICtrlRead($PlayerCombo) = "Populating..." Then Return
...
#ce
Func SetMap($_x=-6,$_y=-6)
	Global $CurrX=$_x
	Global $CurrY=$_y
	_GUICtrlEdit_SetModify($XInput,0)
	_GUICtrlEdit_SetModify($YInput,0)
...
Func ListPlayerVillages()
	...
	_SQLite_GetTable2d($MemDB,"SELECT `village`,`player` from `x_world` where `player` = "&_SQLite_Escape(GUICtrlRead($PlayerCombo)),$lResult,$lRows,$lCols)
	Local $VillageString="|"
	For $i=1 To $lRows
...
Func ShowCenteredPlayer()
	...
	If $iRows > 0 Then
		_GUICtrlComboBox_SelectString ($PlayerCombo,-1,$aResult[1][0])
		ListPlayerVillages()
		GUICtrlSetData($VillageList,$aResult[1][1])
	Else
		_GUICtrlComboBox_SelectString ($PlayerCombo,-1," ")
		GUICtrlSetData($VillageList,"")
	EndIf
EndFunc
