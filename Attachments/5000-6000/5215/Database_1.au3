	...
	if $dbpath <> "" Then
		...
		While $GetTableNames.step () = 0
			...
			if $data = "z_SQL_Log" Then
				$HaveLog = True	
			elseif $data = "z_SQL_VIEWS" then
				$HaveVIEWS = true
			elseif $data = "z_SQL_SELECT" Then
				$HaveSELECT = true	
			EndIf
		...
		EndIf
		if $HaveVIEWS = false Then
			$SQL = "CREATE TABLE z_SQL_VIEWS(TimeStamp TEXT DEFAULT CURRENT_TIMESTAMP , Comment TEXT , SQLEntry TEXT)"
			$odb.execute("BEGIN TRANSACTION")
			$odb.execute($SQL)
			$SQL = stringreplace($SQL,"'","''")
			$odb.execute("INSERT INTO z_SQL_Log(SQLEntry, Comment) VALUES('" & $SQL & "','Initialize VIEW Table')")
			$ODB.execute("COMMIT TRANSACTION")
		EndIf
		if $HaveSELECT = false Then
			$SQL = "CREATE TABLE z_SQL_SELECT(TimeStamp TEXT DEFAULT CURRENT_TIMESTAMP , Comment TEXT , SQLEntry TEXT)"
			$odb.execute("BEGIN TRANSACTION")
			$odb.execute($SQL)
			$SQL = stringreplace($SQL,"'","''")
			$odb.execute("INSERT INTO z_SQL_Log(SQLEntry, Comment) VALUES('" & $SQL & "','Initialize SELECT Table')")
			$ODB.execute("COMMIT TRANSACTION")
		EndIf		
	...
	Else
		...
		If $Proceed = True Then
			...
			if _GUICtrlListViewGetItemCount($NewFieldsLV) > 0 Then
				...
				$oDb.Execute ($SQL)
				$SQL = stringreplace($SQL,"'","''")
				$odb.execute("INSERT INTO z_SQL_Log(SQLEntry, Comment) VALUES('" & $SQL & "','Table Create')")
				$odb.execute("COMMIT TRANSACTION")
...
Func CommitSQL()
	...
	If GUICtrlRead($InputDBPath) <> "" Then
		If StringInStr(GUICtrlRead($SQLInsert), "SELECT") Then
			...
			$odb.execute("INSERT INTO z_SQL_SELECT(SQLEntry, Comment) VALUES('" & stringreplace(guictrlread($SQLInsert),"'","''") & "','User SELECT')")
		...
		ELSEIF StringInStr(GUICtrlRead($SQLInsert), "CREATE VIEW") Then
			...
			$odb.execute("INSERT INTO z_SQL_VIEW(SQLEntry, Comment) VALUES('" & stringreplace(guictrlread($SQLInsert),"'","''") & "','User VIEW')")
			$odb.execute ("COMMIT TRANSACTION")
		Else
			$odb.execute ("BEGIN TRANSACTION")
			$odb.execute (GUICtrlRead($SQLInsert))
			$odb.execute("INSERT INTO z_SQL_Log(SQLEntry, Comment) VALUES('" & stringreplace(guictrlread($SQLInsert),"'","''") & "','User Change')")
			$odb.execute ("COMMIT TRANSACTION")
		EndIf
	Else
