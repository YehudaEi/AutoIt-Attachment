...
Func CommitSQL()
	...
	If GUICtrlRead($InputDBPath) <> "" Then
		...
		Else
			...
			$odb.execute("INSERT INTO z_SQL_Log(SQLEntry, Comment) VALUES('" & guictrlread($SQLInsert) & "','User Change')")
			$odb.execute ("COMMIT TRANSACTION")
...
Func CSVImport()
	...
	Else
		$NewTableName = StringStripWS(inputbox("Table Name","Give the New Table a Name"),8)
		...
		While $Proceed = False
			For $Check = 0 To $CheckEnd
				If $TableNames[$Check] = $NewTableName Then
					$NewTableName = StringStripWS(InputBox("Duplicate Table", "You need to choose a different table name... the one you used is already used.", $NewTableName),8)
					$Proceed = False
					ExitLoop
				ElseIf $Check = $CheckEnd Then
					$Proceed = True
		...
		if $proceed = True Then	
			...
			if fileexists($path) Then	
				...
				with $oexcel
					...
					$SQL = "CREATE TABLE " & $NewTableName & "(" & $data & ")"
					$odb.execute("BEGIN TRANSACTION")
					$odb.execute($SQL)
					$SQL=stringreplace($SQL,"'","''")
					...
					for $SRow = 2 to $rEnd
						...
						$ValueData= stringtrimleft($ValueData,2)
						;msgbox(0,"Tempdata",$ValueData)
						$SQL= "INSERT INTO " & $NewTableName & "(" & $ColumnData & ") VALUES( " & $ValueData & ")"
						;msgbox(0,"",$sql)
						$odb.execute ($SQL)
				...
				OpenDatabase($dbpath)
			Else
				msgbox(0,"File Does not exist","Specified file does not exist.")
			EndIf
		EndIf
	EndIf
EndFunc
