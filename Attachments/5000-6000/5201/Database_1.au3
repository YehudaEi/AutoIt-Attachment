...
$GUIHeight = 775
$GUIText = "SQLite GUI"
$Main = GUICreate($GUIText, $GUIWidth, $guiheight, (@DesktopWidth - $GUIWidth) / 2, (@DesktopHeight - $GUIHeight) / 2,$WS_OVERLAPPEDWINDOW + $WS_VISIBLE + $WS_CLIPSIBLINGS) ;$WS_CLIPCHILDREN+$WS_OVERLAPPEDWINDOW
	...
	Else
		...
		If $NewName <> "" And @error = 0 Then
			...
			If $Proceed = True Then
				...
				$odb.execute ("COMMIT TRANSACTION") ;$odb.execute ("END TRANSACTION")
...
func _GUICtrlCreateListViewEnhanced($col_names, $x_pos,$y_pos,$width,$height)	
	...
	;EndIf
	if not isobj($oExcel) then
		$oExcel = ObjCreate("OWC10.spreadsheet"); Office 2003
	EndIf
