...
$yBorderTier2 = 380 ;Tier 2 distance from top of gui
...
;Tier 1------------------------------------------
$Tab = GUICtrlCreateTab(5, $ybordertier2, $GUIWidth - 5 - 5, $GUIHeight - $yBorderTier2 - 25)
...
;	function to create the edit GUI.  When you click 'edit' in the GUI you run RecordEdit.
	...
	Else
		...
		Else
			...
			;msgbox(0,"",ubound($CurRecord) & "|" & $LastNumOfHeadings)
			
			if $currow < 1 then
				msgbox(0, "Select Record", "You need to select a record before choosing this action.")
			Else
				for $Count = 1 to $LastNumOfHeadings-1
					$CurRecord[$count] = $oexcel.cells($CurRow,$count+1).value
				Next
			EndIf
		EndIf
...
func _GUICtrlCreateListViewEnhanced($col_names, $x_pos,$y_pos,$width,$height)	
	...
	If IsObj($oExcel) Then
		...
		$GUI_ActiveX = GUICtrlCreateObj($oExcel, $ListVDBxPos, $yBorderTier1, $GUIWidth - $ListVDBxPos - $xBorderR, $TreeDBheight + $ySpacing + $InputDBheight)	
		...
		;dim $hdrcolheadings
		;Set hdrColHeadings = activewindow.ColumnHeadings
