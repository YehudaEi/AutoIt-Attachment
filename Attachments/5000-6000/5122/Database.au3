...
Func TreeDBTableClick()
	...
	;_GUICtrlListViewDeleteAllItems($listvdb)
	GUICtrlDelete($ListVDB)
	;for $Col = _GUICtrlListViewGetSubItemsCount($listvdb) to 1 step -1
	;	_GUICtrlListViewDeleteColumn($listvdb,$Col)
	;next

	If StringLeft($Data1, 1) = "|" Then $Data1 = StringTrimLeft($Data1, 1)
	
	;$NTempArray = stringsplit($data1,"|")
	
	;for $AddCol = 2 to $ntemparray[0]
	;	_GUICtrlListViewInsertColumn($listvdb,$addcol,"")
	;Next
	...
	;guictrlsetdata($listvdb,$data1)
	
	;for $Col = 0 to $ntemparray[0]
	;	_GUICtrlListViewSetColumnWidth($Listvdb, $col, $LVSCW_AUTOSIZE_USEHEADER)
	;Next
