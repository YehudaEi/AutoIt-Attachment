...
$GUIWidth = 1000
$GUIHeight = 710
$GUIText = "Parts Database - 04 NOV 05"
$Main = GUICreate($GUIText, $GUIWidth, $guiheight, (@DesktopWidth - $GUIWidth) / 2, (@DesktopHeight - $GUIHeight) / 2, $WS_OVERLAPPEDWINDOW + $WS_VISIBLE + $WS_CLIPSIBLINGS)
...
Func CSVImport()
	if filexists($dbpath) = 0 Then
		msgbox(0,"Open a Database","You need to open a database.")
	Else
		$CSVSPath = FileOpenDialog("Choose a CSV file to import...", "C:\Kevin\AutoItScripts\PartsDatabase", "CSV (*.csv)")
		
		if $csvpath = ""  or @error <> 0 Then			
			msgbox(0,"Error Opening File","Not a valid file.")
		Elseif fileexists($CSVSPath) Then
			
			$CSVFile = fileopen($CSVSPath,0)
