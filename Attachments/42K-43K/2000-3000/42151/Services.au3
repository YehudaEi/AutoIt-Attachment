...
;~ 	DllStructSetData($tdispatchTable, 1, DllCallbackGetPtr($tServiceMain), 2)
	...
	;v4 :
	Local $tSERVICE_TABLE_ENTRY = "STRUCT;ptr lpServiceName;ptr lpServiceProc;ENDSTRUCT"
;~ 	logprint($tSERVICE_TABLE_ENTRY & ";" & $tSERVICE_TABLE_ENTRY)
	Local $tdispatchTable = DllStructCreate($tSERVICE_TABLE_ENTRY & ";" & $tSERVICE_TABLE_ENTRY)
;~ 	If @error Then
...
;~ 			DllStructSetData($tService_Status, "dwControlsAccepted", BitOR($SERVICE_ACCEPT_STOP, $SERVICE_ACCEPT_SESSIONCHANGE))
		...
		$rc = _Service_SetServiceStatus($hService_Status_handle, DllStructGetPtr($tService_Status))
	EndIf
	Return $rc
