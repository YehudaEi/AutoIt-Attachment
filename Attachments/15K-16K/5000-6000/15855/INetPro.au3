...
; Information Changed version see  _INetGetSourcePro info for more info (important it returns a array instead of a string)
; Changes made for better support to automate actions on websites (ie voting logging on posting stuff etc using sessionid cookies things like that)
; Additions made By MrSpacely the rest see autoit form search for the udf "inetpro"
...
Global Const $HTTP_ADDREQ_FLAG_REPLACE    						= 0x80000000  ;Force replace headers
Global Const $HTTP_ADDREQ_FLAG_ADD        						= 0x20000000  ;related to the one above 
Global Const $HTTP_ADDREQ_FLAG_COALESCE_WITH_COMMA      				= 0x40000000 ;related to the one above 
...
Global Const $HTTP_QUERY_RAW_HEADERS 							= 21
Global Const $HTTP_QUERY_RAW_HEADERS_CRLF  						= 22
...
#region - _INetGetSourcePro($s_URL, $s_Verb = 'GET', $s_Header = '', $s_Submits = '', $i_ReadFile = 1, $s_UrlUsername = '', $s_UrlPassword = '', $s_Proxy = '', $s_ProxyUsername = '', $s_ProxyPassword = '')
;===============================================================================
;
...
;					$s_Header 			= [Optional]The Custom headers (ie. sending cookies content type)
...
;					Return Value is a Array [0] is the source [1] is a string like the way its send over http [2] are the cookie values
; 					On Failure - Returns 0 and sets
...
;
;===============================================================================
Func _INetGetSourcePro($s_URL, $s_Verb = 'GET', $s_Header = '', $s_Submits = '', $i_ReadFile = 1, $s_UrlUsername = '', $s_UrlPassword = '', $s_Proxy = '', $s_ProxyUsername = '', $s_ProxyPassword = '')
    Local $h_DLL = DllOpen('wininet.dll'), $i
	If $h_DLL = -1 Then		;error handler of DllOpen('wininet.dll')
		SetError(1)
		Return 0
	EndIf
	Local $ai_IO, $i_Type = $INTERNET_OPEN_TYPE_PRECONFIG, $ai_IC, $ai_HOR, $ai_HSR, $ai_HQI, $s_StatusCode, $strc_Length, $ai_IRF, $s_Buf = Binary('')
	...
	Local Const $TMP_BUFFER_SIZE2 = 65535 ;262144 ;256kb
	Local $i_Err = 0, $i_ErrExt = 0, $ReturnVal = ''	;error code, extended code & returned value
	Local $s_Returned_Headers = ''
	Local $UserAgent = "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1; .NET CLR 1.1.4322)"
	;To simply send a differend header or change this var or use the header parameter and send something like this "User-Agent: Name here(comments)"
    ...
    $ai_IO = DllCall($h_DLL, _
				...
				'str', $UserAgent, _ 		;specifies the name of the application or entity calling the functions, used as the user agent in HTTP
    ...
    If @error = 0 And $ai_IO[0] <> 0 Then
		...
		If @error = 0 And $ai_IC[0] <> 0 Then
			...
			$ai_HOR = DllCall($h_DLL, _
						...
						'int', $INTERNET_FLAG_KEEP_CONNECTION + $INTERNET_FLAG_NO_COOKIES, _	;internet options (see constants). keeping connection is neccessary for authentication
			...
			If @error = 0 And $ai_HOR[0] <> 0 Then
				...
				EndIf
				;calls HttpAddRequestHeaders() here
				If $s_Header <> '' Then	
					DllCall($h_Dll, _								
								'int', 'HttpAddRequestHeaders', _		;Sets up all the headers (is not completely needed but you can force replacement so I kept it)
								'int', $ai_HOR[0], _				;the HTTP request handle
								'str', $s_Header, _				;a string that contains the additional headers to be appended to the request, can be NULL if none
								'int', StringLen($s_Header), _		;size of the additional headers, in TCHARs
								'int', $HTTP_ADDREQ_FLAG_ADD + $HTTP_ADDREQ_FLAG_REPLACE + $HTTP_ADDREQ_FLAG_COALESCE_WITH_COMMA)
				EndIf
				;calls HttpSendRequest() here
				$s_Header='' ;Simply disabling the header sending here (to lazy)
				...
				If @error = 0 And $ai_HSR[0] = 1 Then
					;calls HttpQueryInfo() here (for getting all headers
					$strc_Length2 = DllStructCreate('udword')
					DllStructSetData($strc_Length2, 1, $TMP_BUFFER_SIZE2) 		;set maximum buffer length (64kBytes)
					$ai_HQI = DllCall($h_DLL, _
								'int', 'HttpQueryInfo', _						;retrieves custom HTTP headers
								'int', $ai_HOR[0], _							;the HTTP request handle
								'int', $HTTP_QUERY_RAW_HEADERS_CRLF, _				;specify which header to fetch
								'str', '', _ 					;buffer for receiving the header content. Also being passed as a parameter if needed.
								'ptr', DllStructGetPtr($strc_Length2, 1), _		;DWORD which must be assigned the maximum length of the buffer. Also receive actual content length.
								'int', 0)										;DWORD used as a zero-based index passed to the function to enumerate multiple headers with the same name.
					$s_Returned_Headers = StringLeft($ai_HQI[3], DllStructGetData($strc_Length2, 1))
							...
							If $i_ReadFile = 1 Then		;the read-file flag is true
								...
								Do 
									$ai_IRF = DllCall($h_DLL, _
												...
												'int', $ai_HOR[0], _						;the HTTP request handle
												'ptr', DllStructGetPtr($BinaryBuffer), _	;a buffer that receives the data
									...
									If @error = 0 And $ai_IRF[0] = 1 Then
										...
										$s_Buf &= BinaryMid(DllStructGetData($BinaryBuffer,1),1,$ai_IRF[4])
									Else	;error handler of InternetReadFile()
										$BinaryBuffer=0
										$i_ErrExt = @error
										$i_Err = 8
										;$ReturnVal = BinaryMid($s_Buf,1,BinaryLen($s_Buf)-($TMP_BUFFER_SIZE - $ai_IRF[4])) ;apparently not necesary
										$ReturnVal = BinaryToString($s_Buf)
										ExitLoop
									EndIf
								Until $ai_IRF[4] = 0
								$ReturnVal = BinaryToString($s_Buf)		;successfully returns
	...
	DllClose($h_DLL)
	SetError($i_Err, $i_ErrExt)
	$ReturnValT = $ReturnVal
	Dim $Returnval[3]
	$ReturnVal[0] = $ReturnValT
	$ReturnValT= ""
	$ReturnVal[1] = $s_Returned_Headers
	$ReturnVal[2] = ""
	$HeadersArray = StringSplit ($s_Returned_Headers, @CRLF, 1)
	for $i = 1 to $HeadersArray[0]
		if(StringRegExp ($HeadersArray[$i], "\ASet-Cookie:")) then 
			$ReturnValT = StringRegExp ($HeadersArray[$i],"\ASet-Cookie: (.*?)\z",3)
			$ReturnVal[2] = $ReturnValT[0] 
		endif
	next
	Return $ReturnVal
