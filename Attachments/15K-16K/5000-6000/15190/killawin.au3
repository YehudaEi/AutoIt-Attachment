...
;~ 		MsgBox(0,$title,"Window selected:" & @CRLF & $select)
		...
		HotKeySet("{ESC}")
		if $triggerAction= "shutdown" AND IsAdmin() = 0 Then
			$ret=MsgBox(4,$title, "You don't have administrator privileges. The program can't Shutdown+autodestroy exe file" & @CRLF & "Can you active shutdown trigger without autodestroy file?")
			if $ret = 6 Then
				$autodestroy = 0
			Else
				_GUIMainWin()
