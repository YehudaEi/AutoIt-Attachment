...
; _CSVRecordToString:	Converts a given Record (single csv line) to a string delimited by passed.
; 						Probably redundant.
; Author:			IVAN PEREZ
;===========================================================================================
...
;===========================================================================================
; Date:					04/01/07
; Fixed:				_CSVConvertDelimiter	Bug: $lFields array not assigned conversion.
; Date:					04/01/07
; Fixed:				_CSVConvertEnclose		Bug: $lFields array not assigned conversion.
; Date:					04/01/07
; Fixed:				_CSVConvertEnclose		Bug: $lFields array not assigned conversion.
; Date:					04/01/07
; Fixed:				_CSVStringToField		Bug: Conversion error if no delimiter in Str
;												but enclose char in str.
; Date:					28/12/06
; Added:				_CSVRecordToString		Converts a given Record (single csv line)
; 						 						to a string delimited by passed delimiter.
; 						 						Probably redundant.
; Date:					28/12/06
...
;												string or array.
; Date:					23/12/06
; Added:				_CSVGetField.			Returns the content of a field.
; Date:					23/12/06
; Added:				_CSVGetRecords			Obsolete and changed to _CSVReadRecords.
; Date:					23/12/06
; Added:				_CSVGetColumn			Returns a 1Dim array with column contents.
; Date:					23/12/06
; Added:				_CSVRecordToFields		Retreives the contents of a record to a 1Dim
;												fields array.
; Date:					23/12/06
; Added:				_CSVStringToField		Converts a string to csv value.
; Date:					23/12/06
; Added:				_CSVFieldToString		Converts a csv value to a string.
;===========================================================================================
...
;===========================================================================================
; Function Name: 	_CSVRecordToString
...
; Author:			IVAN PEREZ
;===========================================================================================
Func _CSVRecordToString($pRecord, $pDelimiter = -1, $pEnclose = -1, $pGUIDelimiter = -1)
	If $pEnclose = -1 Then $pEnclose = '"'
	If $pDelimiter = -1 Then $pDelimiter = ','
	If $pGUIDelimiter = -1 Then $pGUIDelimiter = Opt('GUIDataSeparatorChar')
	Dim $lFields
	Local $lGuiDisplayRecord
	; Populate the text list
	$lFields = _CSVRecordToFields($pRecord, $pDelimiter, $pEnclose)
	If @error Then
		SetError(1)
		Return 0
	EndIf
	For $i = 1 To UBound($lFields) - 1 Step 1
		If $i < UBound($lFields) - 1 Then $lGuiDisplayRecord &= _CSVFieldToString($lFields[$i], $pEnclose) & $pGUIDelimiter
		If $i = UBound($lFields) - 1 Then $lGuiDisplayRecord &= _CSVFieldToString($lFields[$i], $pEnclose)
	Next
	Return $lGuiDisplayRecord
EndFunc   ;==>_CSVRecordToString
	...
	If IsArray($pConvert) Then
		For $i = 1 To UBound($pConvert) - 1 Step 1
			...
			For $j = 1 To $lRecord[0] Step 1
				...
				$lFields[UBound($lFields) - 1] = _CSVStringToField($lFields[UBound($lFields) - 1], $pNewDelimiter, $pEnclose)
				; new delimited
	...
	If IsArray($pConvert) Then
		For $i = 1 To UBound($pConvert) - 1 Step 1
			...
			For $j = 1 To $lRecord[0] Step 1
				...
				$lFields[UBound($lFields) - 1] = _CSVStringToField($lFields[UBound($lFields) - 1], $pDelimiter, $pNewEnclose)
				; field concatenation in record
				If $j = $lRecord[0] Then
					$lConverted[UBound($lConverted) - 1] &= $lFields[UBound($lFields) - 1]
