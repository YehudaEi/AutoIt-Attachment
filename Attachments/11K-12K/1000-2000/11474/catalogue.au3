#cs ----------------------------------------------------------------------------
 ...
 Script Function:
	A Disk/CD/DVD Catalogue tool including search & Keywords facility
	...
	Search - Searches existing Catalogues for a String (a single cata logue or all)
	        enables you to search only the filename, the full path, Keywords of a combination
...
Use:
	...
	To change a files keywords, edit them in the textbox and click anywhere in the file list
...
Global $FoundNames [1]		; Files that meet the criteria
Global $KeyWrdsFound [1]	; Exactly what it says on the tin in the same positions as the listview
Global $idx = 1
Global $FilePos = 1
Global $KeyPos [1]			; where in the file is this record?
...
$Cats = GUICtrlCreateList("", 10, 45, 240, 150)
GUICtrlSetOnEvent(-1, "CatsClick")
$Dirs = GUICtrlCreateList("", 10, 190, 240, 266)
GUICtrlSetOnEvent(-1, "DirsClick")
$Files = GUICtrlCreateList("", 260, 70, 340, 380, $WS_BORDER+ $WS_VSCROLL)	; no Autosort
GUICtrlSetOnEvent(-1, "FilesClick")
$FullPath = GUICtrlCreateLabel("", 10, 2, 590, 20)
$Keywords = GUICtrlCreateInput("some great Text",10,22,590,20)
$ScanButton = GUICtrlCreateButton("Scan", 500, 45, 97, 25, 0)
GUICtrlSetOnEvent(-1, "ScanButtonClick")
$SearchButton = GUICtrlCreateButton("Search", 260, 45, 81, 25, 0)
...
GUISetOnEvent($GUI_EVENT_CLOSE, "CLOSEClicked")
$SearchOK = GUICtrlCreateButton("&OK", 25, 62, 75, 25, 0)
GUICtrlSetOnEvent(-1, "SearchOKClick")
GUICtrlSetState($SearchOK ,$GUI_DEFBUTTON)
$SearchCancel = GUICtrlCreateButton("&Cancel", 214, 62, 75, 25, 0)
GUICtrlSetOnEvent(-1, "CLOSEClicked")
...
GUICtrlSetState($AndPath ,$GUI_UNCHECKED)
$AndKeyWords = GUICtrlCreateCheckbox("Search Keywords",150,17,150,14,$BS_AUTOCHECKBOX)
GUICtrlSetState($AndKeyWords ,$GUI_CHECKED)
$SearchStr = GUICtrlCreateInput("", 24, 35, 265, 21)
...
While 1	; Just While some time away
	Sleep(1000)
WEnd
...
Func AddFiles()	
	...
	While 1
		...
		If $array[0] > 3 Then 	; in case somebody puts a pipe in the keywords text and we end up with more than 4 strings
			if $array[2] <> $dirclicked Then ExitLoop	
			_GUICtrlListAddItem ($Files,$array[3])
			_ArrayAdd($KeyWrdsFound,"")
			_ArrayAdd($KeyPos,0)
			$KeyPos[$CNT] = $FilePos
			$KeyWrdsFound[$CNT] = $array[4]
			$CNT += 1
	...
	WEnd
	FileClose($filelist)
	_GUICtrlListSelectIndex($files,0)
	GUICtrlSetData($Keywords,$KeyWrdsFound[0])
	$idx = 1
;~ 	_ArrayDisplay($KeyWrdsFound,"Keywords")
EndFunc

Func FilesClick()
	$NewKeys = GUICtrlRead($Keywords)
	If $NewKeys <> $KeyWrdsFound[$idx-1] Then
		$KeyWrdsFound[$idx-1] = $NewKeys
		ChangeKeywords($NewKeys)
	EndIf
	$idx = _GUICtrlListSelectedIndex($Files)  + 1	; index of selected item
	GUICtrlSetData($Keywords,$KeyWrdsFound[$idx-1])
	If $searching = 1 Then	
...
Func DriveChange()	;  Yup exactly that
...
Func SearchOKClick()	; Start the search
	...
	If GUICtrlRead($AndPath) = $GUI_CHECKED Then
		$SearchPath = 1
	EndIf
	If GUICtrlRead($AndKeyWords) = $GUI_CHECKED Then
		$SearchKeys = 1
	EndIf
			...
			While 1		; Loop thru index files
				...
				While 2 = 2							; loop thru records in the file
					...
					If $SearchPath = 1 Then
						$SearchLine = $array[2] & "\" & $array[3]
					Else
						If $SearchKeys = 1 Then
							$SearchLine = $array[3] & $array[4]
						Else
							$SearchLine = $array[3]
						EndIf
					EndIf
...
Func ChangeKeywords($newval)
	$catalogue = GUICtrlRead($Cats)
	$file = FileOpen($catalogue & "-TitleList.index",0)
	$line = FileReadLine($file,$KeyPos[$idx-1])
	FileClose ($file)
	$array = StringSplit($line,"|")
;~ 	_ArrayDisplay($array,$catalogue & $KeyPos[$idx-1])
	$line = $array[1] & "|" & $array[2] & "|" & $array[3] & "|" & $newval
	$file = $catalogue & "-TitleList.index"
	_FileWriteToLine($file,$KeyPos[$idx-1],$line,1)
EndFunc
