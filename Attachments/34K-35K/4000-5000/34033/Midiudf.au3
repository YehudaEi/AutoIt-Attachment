...
;Library : Microsoft winmm.dll
;=======================================================
Func _MidiOutCacheDrumPatches ($hmidiout, $Patch, $patches, $flags = 0)
   $struct = DllStructCreate("short[128]")	; "short KEYARRAY[MIDIPATCHSIZE]" (MIDIPATCHSIZE=128]
   If Not IsArray($patches) Then Dim $patches[0]=[$patches]
   For $i=0 To UBound($patches)-1
      DllStructSetData($struct, 1, $patches[$i],$i+1)
   Next
...
;Library : Microsoft winmm.dll
;=======================================================
Func _MidiOutCachePatches ($hmidiout, $bank, $patches, $flags = 0)
   $struct = DllStructCreate("short[128]")	; "short PATCHARRAY[MIDIPATCHSIZE]" (MIDIPATCHSIZE=128)
   If Not IsArray($patches) Then Dim $patches[0]=[$patches]
   For $i=0 To UBound($patches)-1
      DllStructSetData($struct, 1, $patches[$i],$i+1)
   Next
...
;=======================================================
; Func _MidiOutLongMsg($hmidiout, ByRef $preparedMidiHdr)
;
; parameters: handle to Midi Out, structure (from _MidiOutPrepareHeader)
;=======================================================

Func _MidiOutLongMsg($hmidiout, ByRef $preparedMidiHdr)
   $ret = DllCall("winmm.dll", "long", "midiOutLongMsg", "handle", $hmidiout, "ptr", DllStructGetPtr($preparedMidiHdr), "long", Number(DLLStructGetPtr($preparedMidiHdr,10)-DllStructGetPtr($preparedMidiHdr)))
   If @error Then Return SetError(@error,0,0)
   If $ret[0] Then Return SetError(-1,$ret[0],0)
   Return $ret[0]
...
;	}
;========================================================
Func _MidiInPrepareHeader ($hmidiin, $data, $bufferlength, $bytesrecorded, $user, $flags)
   ; MIDIHDR: BufferPtr,BufferLength,BytesRecorded,UserData,Flags,NextPtr,Reserved,Offset,Reserved[4] (+buffer)
   $struct = DllStructCreate("ptr;dword;dword;dword_ptr;dword;ptr;dword_ptr;dword;dword_ptr[4];byte["&$bufferlength+1&"]")
   DllStructSetData($struct, 1, DLLStructGetPtr($struct,10))
...
;~   DllStructSetData($struct, 6, $next)	; according to MSDN - do NOT use
   ...
   DllStructSetData($struct,10,$data)
   $ret = DllCall("winmm.dll", "long", "midiInPrepareHeader", "handle", $hmidiin, "ptr", DllStructGetPtr($struct), "long", Number(DLLStructGetPtr($struct,10)-DllStructGetPtr($struct)))
   If @error Then Return SetError(@error,0,0)
   If $ret[0] Then Return SetError(-1,$ret[0],0)
   Return $struct
...
;=======================================================
; Func _MidiInUnprepareHeader ($hmidiin, ByRef $preparedMidiHdr)
;
; parameters: handle to Midi In, structure (from _MidiInPrepareHeader)
;=======================================================

Func _MidiInUnprepareHeader ($hmidiin, ByRef $preparedMidiHdr)
   $ret = DllCall("winmm.dll", "long", "midiInUnprepareHeader", "handle", $hmidiin, "ptr", DllStructGetPtr($preparedMidiHdr), "long", Number(DLLStructGetPtr($preparedMidiHdr,10)-DllStructGetPtr($preparedMidiHdr)))
   If @error Then Return SetError(@error,0,0)
   If $ret[0] Then Return SetError(-1,$ret[0],0)
   Return $ret[0]
...
;~   DllStructSetData($struct, 6, $next)	; according to MSDN - do NOT use
   DllStructSetData($struct, 7, 0)
   DllStructSetData($struct,10,$data)
   $ret = DllCall("winmm.dll", "long", "midiOutPrepareHeader", "handle", $hmidiout, "ptr", DllStructGetPtr($struct), "long", Number(DLLStructGetPtr($struct,10)-DllStructGetPtr($struct)))
   If @error Then Return SetError(@error,0,0)
   If $ret[0] Then Return SetError(-1,$ret[0],0)
   Return $struct
...
;=======================================================
; Func _MidiOutUnprepareHeader ($hmidiout, ByRef $preparedMidiHdr)
;
; parameters: handle to Midi Out, structure (from _MidiOutPrepareHeader)
;=======================================================

Func _MidiOutUnprepareHeader ($hmidiout, ByRef $preparedMidiHdr)
   $ret = DllCall("winmm.dll", "long", "midiOutUnprepareHeader", "handle", $hmidiout, "ptr", DllStructGetPtr($preparedMidiHdr), "long", Number(DLLStructGetPtr($preparedMidiHdr,10)-DllStructGetPtr($preparedMidiHdr)))
   If @error Then Return SetError(@error,0,0)
   If $ret[0] Then Return SetError(-1,$ret[0],0)
   Return $ret[0]
...
Func _MidiInAddBuffer ($hmidiin, ByRef $preparedMidiHdr)
   $ret = DllCall("winmm.dll", "long", "midiInAddBuffer", "handle", $hmidiin, "ptr", DllStructGetPtr($preparedMidiHdr), "long", Number(DLLStructGetPtr($preparedMidiHdr,10)-DllStructGetPtr($preparedMidiHdr)))
   If @error Then Return SetError(@error,0,0)
   If $ret[0] Then Return SetError(-1,$ret[0],0)
   Return $ret[0]
...
;=======================================================
; Func _MidiStreamOut($hMidiStreamOut,ByRef $preparedMidiHdr)
;
...
Func _MidiStreamOut($hMidiStreamOut,ByRef $preparedMidiHdr)
   $ret = DLLCall("winmm.dll", "long", "midiStreamOut", "handle", $hMidiStreamOut, "ptr", DllStructGetPtr($preparedMidiHdr), "uint", Number(DLLStructGetPtr($preparedMidiHdr,10)-DllStructGetPtr($preparedMidiHdr)))
   If @error Then Return SetError(@error,0,0)
   If $ret[0] Then Return SetError(-1,$ret[0],0)
   Return $ret[0]
EndFunc
