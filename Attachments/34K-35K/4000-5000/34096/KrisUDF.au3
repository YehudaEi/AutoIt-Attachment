...
;				   5/4/2011  - Change the For loops to start at $i = 0 instead of $i = 1
; ===============================================================================================================================
Func _Array2DClearBlanks($Array,$2DSize = 2)
	...
	If $2DSize = 0 Then																			;If the user selected a 1 dimensional array, then
		...
		For $i = 0 to UBound($Array) - 1														;Start a For loop that will run for the size of the array
	...
	For $i = 0 To UBound ($Array) - 1															;Start the For loop that will run for the size of the array
...
;				   5/6/2011  - Added the StringRegExpReplace functionality to help error control
; ===============================================================================================================================
Func _TimeToTicksEx($Time, $Numeric = 0)
	...
	$Time = StringRegExpReplace($Time,"\s","")													;Remove any and all spaces from the Time string
...
; Modified.......: 4/25/2011 - Created, commented, added function header
;				   5/5/2011  - Shortened the timeout from 10000 to 500
; ===============================================================================================================================
Func _TCPConnectThenReceive($IP, $Port)
	...
	Until $sMessage <> "" Or $sTimeOut > 500																;Perform the next iteration of the Do loop until the Until condition is met
	TCPCloseSocket($Socket)																					;Close the TCP socket
	TCPShutdown()																							;Shut down the TCP protocol
	If $sTimeOut = 500 Then Return 3																		;If TimeOut occured, return 3
...
; Remarks .......: THIS FUNCTION IS FOR INTERNAL USE ONLY. NOT FOR USE OUTSIDE THIS UDF
; Modified.......: 4/25/2011 - Created, commented, added function header
;				   5/5/2011  - Added the TimeOut features
; ===============================================================================================================================
Func _TCPListenThenSend($IP, $Port, $Message = "Ack")
	Local $Socket, $ConnectedSocket = 0, $sTimeOut = 0														;Declare the local variabels for use in this function
	TCPStartup()																							;Start the TCP protocol
	...
	Do																										;Start a Do loop. This will repeat until the Until condition is met
		...
		$sTimeOut += 1																						;Increase the timeout counter by 1
	Until $ConnectedSocket >= 0	Or $sTimeOut > 500															;Perform the next iteration of the Do loop until the Until condition is met
	...
	If $sTimeOut = 500 Then Return 2																		;If TimeOut occured, return 3
