...
$SMTP_Port = "995"
$SSL_Exe_loc = "<Insert openssl location>"

Func _POP_Connect($POP_Server, $POP_Port, $SSL_Exe_loc, $Debug)
	$_POP_Debug = $Debug
	;Connect To Server Using OpenSSL
;~ 	Run($SSL_Exe_loc & " s_client genrsa", "", @SW_HIDE, $STDOUT_CHILD + $STDIN_CHILD + $STDERR_CHILD + $STDERR_MERGED)
	$_POP_RunPID = Run($SSL_Exe_loc & " s_client -crlf -ign_eof -connect " & $POP_Server & ":" & $POP_Port, "", @SW_HIDE, $STDOUT_CHILD + $STDIN_CHILD + $STDERR_CHILD + $STDERR_MERGED)
...
Func _POP_GetList()
	...
	_ArrayPop($Buffer)
	_ArrayPop($Buffer)
	$recCount = UBound($Buffer)
	SetExtended($recCount)
	_OpenSSL_IsRunning()
	Return $Buffer
...
Func _POP_GetMessage($id_num)
	...
	$_POP_Log &= $Buffer
	If _OpenSSL_ChkForCode($Buffer, @CRLF & ".") == 0 Then
		SetError(7) ;Couldn't read mail
		_OpenSSL_DebugToConsole(@CRLF & @CRLF & "Couldn't Get Message" & @CRLF)
		Return False
	EndIf
...
Func _POP_DelMessage($id_num)
	;Request List
	$Get = "DELE " & $id_num & @CRLF
	_OpenSSL_WriteBuffer($Get)
	$_POP_Log &= $Get
	;Get Responce
	$Buffer = _OpenSSL_ReadBuffer_Until(@CRLF & ".")
	$_POP_Log &= $Buffer
	If _OpenSSL_ChkForCode($Buffer, @CRLF & ".") == 0 Then
		SetError(8) ;Couldn't delete message
		_OpenSSL_DebugToConsole(@CRLF & @CRLF & "Couldn't delete Message" & @CRLF)
		Return False
	EndIf
	
	_OpenSSL_IsRunning()
	Return $Buffer
EndFunc   ;==>_POP_DelMessage

Func _POP_Logoff()
	If _OpenSSL_IsRunning() Then
		_OpenSSL_WriteBuffer("LOGOUT" & @CRLF)
		Sleep($SleepTime)
	EndIf
	Return True
EndFunc   ;==>_POP_Logoff
