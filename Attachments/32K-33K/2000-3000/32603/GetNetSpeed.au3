...
; ==================================================================================
Func GetNetSpeed()
    Local $DnBytes, $UpBytes, $ss, $dTemp, $uTemp
    Local $objWMIService = ObjGet("winmgmts:\\.\root\cimv2")
    If @error Then Return SetError(-1)

    Local $colItems = $objWMIService.ExecQuery("SELECT * FROM Win32_PerfRawData_Tcpip_NetworkInterface", "WQL", 0x10 + 0x20)
    ...
    If Not StringIsDigit($DnBytes) Then $DnBytes = 0
    ...
    $DnBytes = StringLeft($DnBytes / (TimerDiff($tGNS) / 1000), 8)
    $UpBytes = ($UpBytes - $UpStartBytes) / 1024 / 1024
    $UpBytes = StringLeft($UpBytes / (TimerDiff($tGNS) / 1000), 8)
    $DnStartBytes = $dTemp
    $UpStartBytes = $uTemp
