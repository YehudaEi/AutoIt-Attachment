#CS Winamp Library Info Header
	...
	;        Description:        38 Functions to handle Winamp Media Player (http://winamp.com).
	;                            This library is written based on the SDK that is provided on Winamp's homepage:
	...
	;
	;                           v1.4 [22.May.2009]
	;                              * The library is now compatible with AutoIt v3.3.0.0.
...
global const $Pan_Left=-127
global const $Pan_CENTRE=0
global const $Pan_RIGHT=127
global const $Pan_CURRENT=666
...
; Requirement(s):          None.
;
; Return Value(s):         On Success -  Returns 1 if song is currently playing, returns 3 if it's paused, and 0 if it's stopped.
;                          On Failure -  Return -1 and set @error to:
;                                                                   1 - Unable to find Winamp window, probably winamp isn't runing.
;                                                                   2 - SendMessage fail.
;
...
;
;=====================================================================
Func _WinampGetPan($h_Winamp_Wnd = 0)
Local $hWinamp = WinGetHandle($sWINAMP_CLASS)
	local $ret
	...
	$ret=DllCall("user32.dll", "int", "SendMessage", "hwnd", $hWinamp, "int", $WM_WA_IPC, "int", $Pan_CURRENT, "int", $IPC_SETPANNING)
	
	If @error Then Return SetError(2, 0, 0)
	
return $ret[0]
EndFunc   ;==>_WinampGetPan

;===============================================================================
;
; Function Name:  		   _WinampSetPan()
;
; Function Description:    Sets the specified pan position for a track.
;
; Parameter(s):            $h_Winamp_Wnd - [Optional] Winamp window handle to use, $pan (from -127 (left) to 127 (right). Centre is 0.)
;                                           (by default the handle is taken from $sWINAMP_CLASS).
;
; Requirement(s):          Winamp 2.x +.
;
...
;
;=====================================================================
Func _WinampSetPan($pan, $h_Winamp_Wnd=0)
Local $hWinamp = WinGetHandle($sWINAMP_CLASS)
	
	
	if ($pan < -127) or ($pan > 127) then Return SetError(3, 0, 0)
	If IsHWnd($h_Winamp_Wnd) Then $hWinamp = $h_Winamp_Wnd
	
	If Not IsHWnd($hWinamp) Then Return SetError(1, 0, 0)

	DllCall("user32.dll", "int", "SendMessage", "hwnd", $hWinamp, "int", $WM_WA_IPC, "int", $pan, "int", $IPC_SETPANNING)
	
	If @error Then Return SetError(2, 0, 0)
	
	return 0
EndFunc   ;==>_WinampSetPan

;===============================================================================
;
; Function Name:  		   _WinampGetEq()
;
; Function Description:    queries the status of the EQ.
;
; Parameter(s):            $h_Winamp_Wnd - [Optional] Winamp window handle to use
;                                           (by default the handle is taken from $sWINAMP_CLASS).
; $pos - The queried equaliser position. It can be set according to the following table:
; ** Value       Return
;** ------------------
...
;
;=====================================================================
Func _WinampGetEq($pos, $h_Winamp_Wnd=0)
Local $hWinamp = WinGetHandle($sWINAMP_CLASS)
	local $ret

  If ($pos < 0) or ($pos > 12) Then Return SetError(3, 0, 0)
	If IsHWnd($h_Winamp_Wnd) Then $hWinamp = $h_Winamp_Wnd
		If Not IsHWnd($hWinamp) Then Return SetError(1, 0, 0)

	$ret=DllCall("user32.dll", "int", "SendMessage", "hwnd", $hWinamp, "int", $WM_WA_IPC, "int", $pos, "int", $IPC_GETEQDATA)
	
	If @error Then Return SetError(2, 0, 0)
	
return $ret[0]
EndFunc   ;==>_WinampGetEq
...
; Parameter(s):            $h_Winamp_Wnd - [Optional] Winamp window handle to use
;                                           (by default the handle is taken from $sWINAMP_CLASS).
; $pos - The equaliser position.
; $val - The new value of the specified $pos. It can be set according to the following table:
; ** $Pos     $Val
;** ------------------
;** 0-9        The 10 bands of EQ data. 0-63 (+20db - -20db)
;** 10         The preamp value. 0-63 (+20db - -20db)
...
;
;=====================================================================
Func _WinampSetEq($pos, $val, $h_Winamp_Wnd=0)
Local $hWinamp = WinGetHandle($sWINAMP_CLASS)
  ...
  If ($pos < 0) or ($pos > 12) Then Return SetError(3, 0, 0)
	...
	DllCall("user32.dll", "int", "SendMessage", "hwnd", $hWinamp, "int", $WM_WA_IPC, "int", $pos, "int", $IPC_GETEQDATA)
	
	If @error Then Return SetError(2, 0, 0)
	
	DllCall("user32.dll", "int", "SendMessage", "hwnd", $hWinamp, "int", $WM_WA_IPC, "int", $val, "int", $IPC_SETEQDATA)
If @error Then Return SetError(2, 0, 0)

return 0
EndFunc   ;==>_WinampSetEq
