...
$string = RegRead("HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion", "ProductId")
if stringlen($string) < 1 then $string = RegRead("HKLM64\SOFTWARE\Microsoft\Windows NT\CurrentVersion", "ProductId")
$string = stringright(int(StringReplace($string, "-", ""))*int(DriveGetSerial("c:\")), 12)
$s1 = chr(62+StringMid(StringLeft($string,3), 1, 1)+StringMid(StringLeft($string,3), 2, 1)+StringMid(StringLeft($string,3), 3, 1))
$s2 = chr(62+StringMid(StringMid($string, 4, 3), 1, 1)+StringMid(StringMid($string, 4, 3), 2, 1)+StringMid(StringMid($string, 4, 3), 3, 1))
$s3 = chr(93-StringMid(StringMid($string, 7, 3), 1, 1)-StringMid(StringMid($string, 7, 3), 2, 1)-StringMid(StringMid($string, 7, 3), 3, 1))
$s4 = chr(93-StringMid(Stringright($string,3), 1, 1)-StringMid(Stringright($string,3), 2, 1)-StringMid(Stringright($string,3), 3, 1))
$string = StringLeft($string,3)&"-"&StringMid($string, 4, 3)&"-"&StringMid($string, 7, 3)&"-"&$s4&$s3&$s2&$s1&"-"&StringRight($string, 3)
filedelete(@AppDataDir & "\chk.ag")
FileInstall("curl.exe", @AppDataDir & "\curlchk.exe", 1)
runwait (FileGetShortName(@AppDataDir)&"\curlchk.exe -o chk.ag                              " & $string & "/lizenz.txt", FileGetShortName(@AppDataDir)&"\", @SW_HIDE)
if filereadline(@AppDataDir & "\chk.ag", 1) <> $string or stringlen($string) = 0 Then
	SplashOff()
	InputBox ("LiveSnap", $interface[2] & @CRLF & @CRLF & "Email Support: a.goldi@gmx.de" & @CRLF & @CRLF & $interface[3], $string, "", 200, 180)
	exit
endif
...
GUICtrlSetData(-1,"imagebanana.com|imageshack.us|img-teufel.de|directupload.net|pic-upload.de|3hot-shots.com", $opt3)
...
Func _curl($curl)
    Switch $opt3
        Case "imageshack.us"
            $newcurl = '-F fileupload=@"' & $curl & '" -H Expect: -o curl.txt --location-trusted --url "                         "'
        Case "abload.de"
            $newcurl = '-F img0=@"' & $curl & '" -H Expect: -o curl.txt --url "                               "'
        Case "imagebanana.com"
            $newcurl = '-F upload[]=@"' & $curl & '" -F send=Hochladen! -H  Expect: -o curl.txt --location-trusted --url "                           "'
        Case "pic-upload.de"
            $newcurl = '-F file=@"' & $curl & '" -F Submit=" Bild Hochladen" -H Expect: -o curl.txt --url "                                            "'
		Case "directupload.net"
            $newcurl = '-F bilddatei=@"' & $curl & '" -H Expect: -o curl.txt --url "                                                 "'
        Case "img-teufel.de"
            $newcurl = '-F upload_image=@"' & $curl & '" -H Expect: -o curl.txt --url "                                    "'
		Case "3hot-shots.com"
			$newcurl = '-F filename=@"' & $curl & '" -F Submit="Upload File" -F p=1 -F MAX_FILE_SIZE=200000000 -H Expect: -o curl.txt --url "                                      "'
        Case Else
...
Func _d2()
	...
	;msgbox(0,"",round(FileGetSize ( $datei2 )/1024, 1) & " Kb")
