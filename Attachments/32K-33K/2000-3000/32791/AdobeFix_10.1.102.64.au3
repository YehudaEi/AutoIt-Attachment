...
Func start($sPatchThis)
	If $sPatchThis Then
		If FileExists($sPatchThis) Then
			;MsgBox(0,'test message','File = ' & $sPatchThis & @CRLF &  'File Found.')
...
Func VersionIsGood_Openit($sDll, $sVer)
    ;MsgBox(0,'test message','File = ' & $sDLL & @CRLF & 'Version = ' & $sVer & @CRLF &  'test message.')
	Local $hDLL, $bFile
...
Func PatchIt(byref $bFile,$sDll,$sV,$nL,$nO,$bOrig,$bNew)
	...
	;MsgBox(0,'test message','replacement part = ' & $bTarget & @CRLF & 'test message.')
