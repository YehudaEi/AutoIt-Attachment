...
Func Run_Excel($iB1, $iC1)
    ...
    For $iE1 = $iB1 To $iC1
	   ...
	   $iV1 = ($iE1+($iClicks-1))
		...
		; Makes a value that cooresponds to how many lines I use in Excel with respect to the $iClicks value
		
		
			For $iZ1 = $iE1 To $iV1
			; Exits the loop when the policy numbers no longer match
			If $sPolicyNumber <> $sTempPolNumber Then ExitLoop
        ...
        If $iV1 > $iC1 Then ExitLoop
		; Next - in case the user enterd the lines wrongly, is there a policy number to read? 
		; read the policynumber cell on line $iV1 and exit if it is blank
        If _ExcelReadCell($oExcel, ($iV1+1), 2) = "" Then ExitLoop
