...
Dim $Font ="Arial Bold"
Dim $szDrive, $szPath, $szFName, $szExt
...
$Gui = GuiCreate("MSI Editor v1.1", 975, 571,(@DesktopWidth-975)/2, (@DesktopHeight-571)/2 , _ 
...
GUICtrlSetImage (-1, "objsel.dll",-3) ; Negative index !!
...
GUICtrlSetImage ($ListView, "objsel.dll",-3) ;Set Icons for records, negative index !!
_GUICtrlListView_SetColumnWidth ($listview, 0,130) ;Set Column with
...
GUICtrlSetState($ListView,$GUI_FOCUS) ;Set focus on the Object
...
Func OpenDB()
	...
	GuiSetState(@SW_LOCK)
	_GUICtrlTreeView_DeleteAll ($TablesNames)
	GuiSetState(@SW_UNLOCK)
	$sPath = FileOpenDialog("Select a MSI file...", @ScriptDir, "MSI (*.msi;*.msp)")
	_PathSplit($sPath, $szDrive, $szPath, $szFName, $szExt)
	If $sPath = "" Then Return ; Exit
		If $szExt = ".msp" Then
		$DB = $WI.OpenDatabase($sPath, $msiOpenDatabaseModeTransact+$msiOpenDatabaseModePatchFile)	
			Else
		$WISumInfo = $WI.SummaryInformation($sPath)
		$DB = $WI.OpenDatabase($sPath, $msiOpenDatabaseModeTransact)
	EndIf
...
Func ClearTable()
	...
	Else
		DeleteList()
    	If _GUICtrlTreeView_GetParentParam($TreeView) = 0 then
			$sSelTab = GUICtrlRead(GUICtrlRead($treeview),1)
		EndIf	
	
		
	If _GUICtrlTreeView_GetParentParam($TreeView) = 1 Then	
	...
	GuiSetState(@SW_LOCK)
	_GUICtrlTreeView_Delete ($treeview,$sSelTab)
	GuiSetState(@SW_UNLOCK)
	 EndIf
	EndIf
EndFunc
...
;--------------------------- Get Fields --------------------------------------
 ...
 $index = 0
 	For $i = 1 to 10
			_GUICtrlListView_SetColumn($ListView,$index,"")
			$index += 1
	Next
 $index = 0
				...
				While 1  
				  ...
				  If Not IsObj($RecT) Then ExitLoop
					...
					For $i = 1 to $Fields
						...
						_GUICtrlListView_SetColumn($ListView,$index,$TVData)
						$index += 1
					Next
				WEnd
			$ViewT.Close
...
Func GetCABList()
    ...
    $ViewC.execute()
		...
		Wend
			_GUICtrlListView_SetColumn($ListView,0,"Embedded Cabinet class ID")
		$ViewC.Close
		$RecC = ""
        $ViewC = ""
	EndIf
...
If $sPath = "" Then 
	...
	ElseIf StringInStr(GUICtrlRead($SQLInsert), "UPDATE") Then
		
	MsgBox(0,"Debug",StringStripWS(GUICtrlRead($SQLInsert), 3))
	$ViewT = $DB.OpenView(StringStripWS(GUICtrlRead($SQLInsert), 3))
			$ViewT.Execute()
			$DB.Commit
		MsgBox(0,"Update Status", "Finished ...")
	Else
		Msgbox (0,"Error","No SELECT or UPDATE entered, try again")
	Endif
EndIf	
EndFunc
