...
#Region Variable Dec
Global $ProcessHandle, $address[10001], $ValidMemory[500][2]
#EndRegion
...
Func SearchData($Data);i think needs more work
    $n = 0
    $n2 = 0
    _GUICtrlListView_BeginUpdate($Return_ListView)
    Do
        $n = $n + 1
        $Memory_Read = _MemoryRead($ValidMemory[$n][0], $ProcessHandle, 'byte['&$ValidMemory[$n][1]&']');reads  bytes
        $String_Length = StringLen("hi")
        $i = 1
        $p = 0
        While 1
            $search = StringInStr($Memory_Read, $Data, 1, 1,$i)
            If $search <> 0 Then
                $p = $p + 1
                $address[0] = $p;#found
                $address[$p] = Hex($ValidMemory[$n][0] +(($search - 1)/2 -1));stores found address
                $i = $search + $String_Length; new start posistion
                GUICtrlCreateListViewItem ( $address[$p] &"|"& $Search_Value, $Return_ListView )
            Else
                ExitLoop
            EndIf
        WEnd
        $Progress = ($ValidMemory[$n][0] / 4294967295) * 200
        GUICtrlSetData($ProgressBar, $Progress)
    Until $n = $ValidMemory[0][0]
    _GUICtrlListView_EndUpdate($Return_ListView)
EndFunc
