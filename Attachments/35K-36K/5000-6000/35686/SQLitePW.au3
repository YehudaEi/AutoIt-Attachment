...
#comments-start
	...
	02.11.11	Added _SQLite_Create_PW
	02.11.11	Added _SQLite_Password
	02.11.11	Changed _SQLite_open() Password Add
...
;				   $sPassword - The Password That must be given/Create
...
; Author ........: piccaso (Fida Florian)
; Modified.......: jchd, jpm, bartw
; ===============================================================================================================================
Func _SQLite_Open($sDatabase_Filename = Default,$sPassword = Default, $iAccessMode = Default, $iEncoding = Default)
	...
	EndIf
	if not IsKeyword($sPassword) Then
		If Not $OldBase Then
			_SQLite_Exec($avRval[2], "pragma rekey = "&_SQLite_Escape($sPassword))
		Else
			_SQLite_Exec($avRval[2], "pragma key = "&_SQLite_Escape($sPassword))
		endif
	endif
...
; #FUNCTION# ====================================================================================================================
; Name...........: _SQLite_Create_PW
; Description ...: Sets password
; Syntax.........: _SQLite_Create_PW($hDB, $sPassword)
; Parameters ....: $hDB - An Open Database, Use -1 To use Last Opened Database
;				   $sPassword - The Password That must be added
; Return values .: On Success - Returns $SQLITE_OK
;                  On Failure - Return Value can be compared against $SQLITE_* Constants
;                   @error Value(s):	-1 - SQLite Reported an Error (Check Return value)
;					 1 - Error Calling SQLite API 'sqlite3_exec'
...
; Modified.......:
; ===============================================================================================================================
Func _SQLite_Create_PW($hDB, $sPassword)

$iPW =_SQLite_Exec($hDB, "pragma rekey = "&_SQLite_Escape($sPassword))
...
; #FUNCTION# ====================================================================================================================
; Name...........: _SQLite_Password
; Description ...: Puts in the password
; Syntax.........: _SQLite_Password($hDB, $sPassword)
; Parameters ....: $hDB - An Open Database, Use -1 To use Last Opened Database
;				   $sPassword - The Password That must be given
; Return values .: On Success - Returns $SQLITE_OK
;                  On Failure - Return Value can be compared against $SQLITE_* Constants
;                   @error Value(s):	-1 - SQLite Reported an Error (Check Return value)
;					 1 - Error Calling SQLite API 'sqlite3_exec'
...
; Modified.......:
; ===============================================================================================================================
Func _SQLite_Password($hDB, $sPassword)

$iPW = _SQLite_Exec($hDB, "pragma key = "&_SQLite_Escape($sPassword))
