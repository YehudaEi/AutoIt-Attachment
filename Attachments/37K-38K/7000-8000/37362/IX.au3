...
Global $CurrentPlayerTurn = $Green

; Indicate where the buttons for each team will go. 10 is valid if red starts. 772 for green.
...
$hGUI = GUICreate("IX ver 0.02", 68 * 9 + 300, 68 * 9)
InitializeBoard()
GUISetState()
DrawBoard()

$hRedCount  = GUICtrlCreateButton("Count: 64", 10, 572, 130, 30)
$hGreenCount  = GUICtrlCreateButton("Count: 64", 772, 572, 130, 30)
...
Func CheckPiece($piece)
	ConsoleWrite("@@debug CheckPiece(" & $piece & ")" & @CRLF)

	Return BitAND($piece, $CurrentPlayerTurn)
EndFunc

Func OnExit()
	ConsoleWrite("@@debug OnExit()" & @CRLF)
	OkayBeep()
...
Func WipeColor()
	ConsoleWrite("@@debug WipeColor()" & @CRLF)
	For $x = 0 to 8
		For $y = 0 to 8
			$Color[$x][$y] = 0
		Next
...
Func DrawBoard()
	ConsoleWrite("@@debug DrawBoard()" & @CRLF)
...
Func CalculateBoard()
	ConsoleWrite("@@debug CalculateBoard()" & @CRLF)
	...
	If $RedCalc = 16 Then
		Msgbox(48, "Game Over", "Green Wins")
		Exit
	EndIf
	If $GreenCalc = 16 Then
		MsgBox(48, "Game Over", "Red Wins")
		Exit
	EndIf
	If $RedCalc = 17 And $GreenCalc = 17 Then
		MsgBox(48, "Game Over", "Draw")
		Exit
	EndIf
	Return 1
EndFunc
...
Func InitializeBoard()
	ConsoleWrite("@@debug InitializeBoard()" & @CRLF)
...
Func SetUndo()
	ConsoleWrite("@@debug SetUndo()" & @CRLF)

	$hSet  = GUICtrlCreateButton("Set", $Column, 10, 130, 30)
	$hUndo = GUICtrlCreateButton("Undo", $Column, 50, 130, 30)
...
Func DetermineAction($fore, $side)
	ConsoleWrite("@@debug DetermineAction(" & $fore & ", " & $side & ")" & @CRLF)
...
Func Tribe($fore, $side)
	ConsoleWrite("@@debug Tribe(" & $fore & ", " & $side & ")" & @CRLF)

	ValidBeep()
	$Color[$fore][$side] = $Yellow
	$hSplit  = GUICtrlCreateButton("Split", $Column, 10, 130, 30)
	If $fore = 0 Or $fore = 8 Or $side = 0 Or $side = 8 Then
		$hRemove = GUICtrlCreateButton("Remove", $Column, 50, 130, 30)
		$CancelLine = 90
	Else
		$hRemove = GUICtrlCreateDummy()
		$CancelLine = 50
	EndIf
	$hCancel = GUICtrlCreateButton("Cancel", $Column, $CancelLine, 130, 30)
	...
	While True
		...
		If $msg = $hSplit Then
			...
			If $fore > 0 And $side < 8 Then $Board[$fore-1][$side+1] = $Clan + $CurrentPlayerTurn
			If $fore < 8 And $side > 0 Then $Board[$fore+1][$side-1] = $Clan + $CurrentPlayerTurn
			If $fore < 8 And $side < 8 Then $Board[$fore+1][$side+1] = $Clan + $CurrentPlayerTurn
			DrawBoard()
			GUICtrlDelete($hSplit)
			GUICtrlDelete($hRemove)
			GUICtrlDelete($hCancel)
...
Func ValidateTribeMove($baseF, $baseS, $newF, $newS)
	ConsoleWrite("@@debug ValidateTribeMove(" & $baseF & ", " & $baseS & ", " & $newF & ", " & $newS & ")" & @CRLF)
...
Func Clan($fore, $side)
	ConsoleWrite("@@debug Clan(" & $fore & ", " & $side & ")" & @CRLF)
	...
	While True
		...
		If $msg = $hSplit Then
			...
			If $fore < 8 Then $Board[$fore+1][$side] = HunterAdjust($fore+1, $side, $HunterE) + $CurrentPlayerTurn
			If $side < 8 Then $Board[$fore][$side+1] = HunterAdjust($fore, $side+1, $HunterS) + $CurrentPlayerTurn
			DrawBoard()
			GUICtrlDelete($hSplit)
			GUICtrlDelete($hRemove)
			GUICtrlDelete($hCancel)
...
Func ValidateClanMove($baseF, $baseS, $newF, $newS)
	ConsoleWrite("@@debug ValidateClanMove(" & $baseF & ", " & $baseS & ", " & $newF & ", " & $newS & ")" & @CRLF)
...
Func HunterAdjust($fore, $side, $newpiece)
	...
	ConsoleWrite("@@debug HunterAdjust(" & $fore & ", " & $side & "," & $newpiece & ")" & @CRLF)
...
Func IsRemoveHunterValid($fore, $side)
	ConsoleWrite("@@debug IsRemoveHunterValid(" & $fore & ", " & $side & ")")
	...
	Switch $piece
		Case $HunterN
			...
			If $fore = 8 Then Return 1
			Return 0
		Case $HunterS
			If $side = 8 Then Return 1
			If $fore = 0 Then Return 1
			If $fore = 8 Then Return 1
		Case $HunterW
			If $fore = 0 Then Return 1
...
Func Hunter($fore, $side)
	...
	ConsoleWrite("@@debug Hunter(" & $fore & ", " & $side & ")")

	ValidBeep()
	$Color[$fore][$side] = $Yellow
	DrawBoard()
	$hLeft90  = GUICtrlCreateButton("Rotate 90 Degrees" & @CRLF & "Counter Clockwise", $Column, 10, 130, 40, $BS_MULTILINE)
	$hLeft45  = GUICtrlCreateButton("Rotate 45 Degrees" & @CRLF & "Counter Clockwise", $Column, 50, 130, 40, $BS_MULTILINE)
	$hRight45 = GUICtrlCreateButton("Rotate 45 Degrees" & @CRLF & "Clockwise", $Column, 90, 130, 40, $BS_MULTILINE)
	$hRight90 = GUICtrlCreateButton("Rotate 90 Degrees" & @CRLF & "Clockwise", $Column, 130, 130, 40, $BS_MULTILINE)
	If IsRemoveHunterValid($fore, $side) Then
		$hRemove = GUICtrlCreateButton("Remove", $Column, 170, 130, 30)
		$CancelLine = 200
	Else
		$hRemove = GUICtrlCreateDummy()
		$CancelLine = 170
			...
			For $afore = 0 to 8
				If $msg = $hBoard[$afore][$aside] Then
					...
					If ValidateHunterMove($fore, $side, $afore, $aside) Then
						...
						$Color[$fore][$side] = $Black
						DrawBoard()
						If CalculateBoard() Then SetUndo()
					Return
					Endif
					BadBeep()
				EndIf
...
Func TurnHunter($fore, $side, $mode)
	ConsoleWrite("@@debug TurnHunter(" & $fore & ", " & $side & "," & $mode & ")")
...
Func ValidateHunterMove($baseF, $baseS, $newF, $newS)
	ConsoleWrite("@@debug ValidateHunterMove(" & $baseF & ", " & $baseS & ", " & $newF & ", " & $newS & ")" & @CRLF)
