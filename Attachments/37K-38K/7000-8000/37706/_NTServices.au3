...
;=============================================================================
; 31/05/2012: some bugfixes in _ServiceStatus by Kasty. Now it doesn't ask for more
; privileges than actually needed, allowing it to work for processes that
; don't hold administrator or system rights. Also, when a service is not installed,
; it returns an empty string, rather than a number (makes comparison logic much easier).
;=============================================================================
...
Func _ServiceStatus($v_ServiceName, $v_ComputerName = "")
	...
	$_hSManager = __OpenSCManager($v_ComputerName, $SERVICES_ACTIVE_DATABASE, $SC_MANAGER_ENUMERATE_SERVICE)
    If $_hSManager <> 0 Then
        $_hService = __OpenService($_hSManager, $v_ServiceName, $SERVICE_QUERY_STATUS)
        If $_hService <> 0 Then
