...
If FileExists("Reflex.ini") = 0 Then
	...
	IniWrite("Reflex.ini", "GUI", "Rows", 9)
EndIf
...
$hGUI = GUICreate("Reflex - ver 1.4", ($WidthX + 1) * 64, ($WidthY + 2) * 64)
GUISetFont(22, 800, 0, "Courier New")
$hDisplay  = GUICtrlCreateLabel("" , 10, 3,  1000, 28)
$hDisplay2 = GUICtrlCreateLabel("" , 10, 31, 1000, 28)
...
Func StandardMove()
	...
	While _Timer_Diff($StartTime) < (1010 - ($Level * 20))
        ReadyMovement()
	WEnd
	If $ForceFieldMode Then Return
	StopAllSounds()
...
Func HighScore()
	...
	$StatID = GUICreate("High Scores", 220, 475, -1, -1, BitOR($WS_EX_TOPMOST, 0), -1, $hGUI)
