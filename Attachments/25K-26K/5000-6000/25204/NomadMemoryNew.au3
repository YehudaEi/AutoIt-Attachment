...
Func _MemoryPointerRead ($iv_Address, $ah_Handle, $av_Offset, $sv_Type = 'dword')
	...
	If Not IsArray($ah_Handle) Then Return SetError(2, 0, 0)
...
;~ 	ConsoleWrite("addr: " & Hex($iv_Data[1]) & @CR)
	
	For $i = 0 To $iv_PointerCount
		If $i = $iv_PointerCount Then $v_Buffer = DllStructCreate($sv_Type)
		$iv_Address = '0x' & hex($iv_Data[1] + $av_Offset[$i])
;~ 		ConsoleWrite("Add+Off: " & $iv_Address & @CR)
		DllCall($ah_Handle[0], 'int', 'ReadProcessMemory', 'int', $ah_Handle[1], 'int', $iv_Address, 'ptr', DllStructGetPtr($v_Buffer), 'int', DllStructGetSize($v_Buffer), 'int', '')
		If @Error Then Return SetError(7, 0, 0)
		
		$iv_Data[1] = DllStructGetData($v_Buffer, 1)
;~ 		ConsoleWrite("data: " & Hex($iv_Data[1]) & @CR)
	Next
