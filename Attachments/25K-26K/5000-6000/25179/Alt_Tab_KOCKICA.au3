...
Global $current_windows, $new_handle, $win_hwnd_count_x = 0, $win_hwnd_count2 = 1
				...
				Case $HOTKEY
                    If BitAND($iFlags, $LLKHF_ALTDOWN) Then
						if $lock_tab = false then
							...
							Local $move = $win_hwnd_count
							if $move = $current_windows[0][0] then $move = 0
;~ 							$new_handle_x[1] = $new_handle
							...
							for $i = 1 to $last_count
								$win_hwnd_count_x = $win_hwnd_count2+$i
							...
							Local $hwnd_count_previous = $win_hwnd_count2-1
...
;~ 								ControlMove($form12, "", $gui_graph, $move*50+17, 384)
								...
								Next
								_set_icon(0, $new_handle_x[0])
								ControlMove($form12, "", $gui_graph, ($move)*50+27, 384)
