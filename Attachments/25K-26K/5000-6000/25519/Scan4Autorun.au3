...
$Form1 = GUICreate("Autorun.inf Scanner //", 635, 444, 193, 125)
$Edit1 = GUICtrlCreateEdit("", 8, 40, 617, 393,BitOR($ES_AUTOVSCROLL,$ES_READONLY,$WS_VSCROLL))
	...
	If Not @error Then
		For $i = 1 To $var[0]
			...
			If FileExists($path) Then
				GUICtrlAddData($Edit1, "--------------------------------------------------" & @CRLF)
				GUICtrlAddData($Edit1, " Autorun.inf found @ " & $path  & @CRLF)
				$autorun = FileRead($path)
				GUICtrlAddData($Edit1, $autorun)
				GUICtrlAddData($Edit1, @CRLF & @CRLF)
				If StringInStr(FileGetAttrib($path), "D") Then
					$FolderContent = _FileListToArray($path)
					GUICtrlAddData($Edit1, _ArrayToString ($FolderContent) & @CRLF & @CRLF)
					GUICtrlAddData($Edit1, "--------------------------------------------------" & @CRLF)
				EndIf
			Else
