...
Func _AddRecord($oConnectionObj, $sTable, $vRow, $vValue = "")
	If IsObj($oConnectionObj) Then
		...
		If Not IsArray($vRow) And Not IsArray($vValue) And Not IsInt($vValue) Then
			...
			;$oConnectionobj.execute ("INSERT INTO " & $sTable & " (" & $vRow & "," & $vRow2 & "," &  $vRow3 & ") VALUES (" & $vValue & "," & $vValue2 & "," & $vValue3 & ")" )
			$oConnectionobj.execute ("INSERT INTO users(name,email,password) VALUES('ernesto jr','email@test.com','password')")
			return 1
...
Func _READ($oConnectionObj, $sTable, $sColumn)
	If IsObj($oConnectionObj) Then
		;SHOW COLUMNS FROM mytable FROM mydb
		$quer = $oConnectionObj.execute ("SHOW COLUMNS FROM " & "users" & " FROM " & 'database')
		;$quer = $oConnectionObj.execute ("SHOW COLUMNS FROM " & "users" & " FROM " & '')
		With $quer
			$i = 0
			While Not .eof
				If .fields (1).value = $sColumn Then
					$ret = .fields (1).value
				EndIf
				.MoveNext
			WEnd
