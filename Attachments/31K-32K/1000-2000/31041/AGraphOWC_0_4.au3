...
Global $oChartSpace, $oChart, $oSeries, $oSeries_0, $oSeries_1 
...
$oSeries_0 = $oChartSpace.Charts(0).SeriesCollection.Add(1)
$oSeries_1 = $oChartSpace.Charts(0).SeriesCollection.Add(2)
...
If StringInStr($GraphTypeName, "Pie") or StringInStr($GraphTypeName, "Doughnut") then 
 ...
 With $oChart
  ...
  if StringInStr($GraphTypeName, "Scatter") or _
    $GraphTypeName = "Column3D" or $GraphTypeName = "ColumnClustered3D" or $GraphTypeName = "ColumnStacked3D" or $GraphTypeName = "ColumnStacked1003D" or _ 
    $GraphTypeName = "Bar3D" or $GraphTypeName = "BarClustered3D" or $GraphTypeName = "BarStacked3D" or $GraphTypeName = "BarStacked1003D" or _
    $GraphTypeName = "Line3D" or $GraphTypeName = "LineOverlapped3D" or $GraphTypeName = "LineStacked3D" or $GraphTypeName = "LineStacked1003D" then
   .Axes(0).Title.Caption = "Sin(X/15)"
   .Axes(1).Title.Caption = "X"
  endif
 EndWith
endif
...
local $X, $Y, $YMin, $YMax, $YAbs, $Z
 ...
 $oChartSpace.Charts(0).SeriesCollection.Delete(2)
 $oChartSpace.Charts(0).SeriesCollection.Delete(1)
 ...
 If StringInStr($GraphTypeName, "Pie") then 
  ...
  If StringInStr($GraphTypeName, "Scatter") then 
   $oSeries.SetData ($chDimSeriesNames, $chDataLiteral, "Sin(X/15)")
   ...
   $oSeries_0.SetData ($chDimSeriesNames, $chDataLiteral, "YMin")
   $oSeries_0.SetData($chDimXValues, $chDataLiteral, StringTrimRight($X,1))
   $oSeries_0.SetData($chDimYValues, $chDataLiteral, StringTrimRight($YMin,1))
   $oSeries_0.Marker.Style = Random(1,9,1)

   $oSeries_1.SetData ($chDimSeriesNames, $chDataLiteral, "YMax")
   $oSeries_1.SetData($chDimXValues, $chDataLiteral, StringTrimRight($X,1))
   $oSeries_1.SetData($chDimYValues, $chDataLiteral, StringTrimRight($YMin,1))
   $oSeries_1.Marker.Style = Random(1,9,1)

  elseif StringInStr($GraphTypeName, "Polar") then 
   $oSeries.SetData ($chDimSeriesNames, $chDataLiteral, "Abs(Sin(X/15))")
   $oSeries.SetData($chDimThetaValues, $chDataLiteral, StringTrimRight($X,1))
   $oSeries.SetData($chDimRValues, $chDataLiteral, StringTrimRight($YAbs,1))

   $oSeries_0.SetData ($chDimSeriesNames, $chDataLiteral, "YMax")
   $oSeries_0.SetData($chDimThetaValues, $chDataLiteral, StringTrimRight($X,1))
   $oSeries_0.SetData($chDimRValues, $chDataLiteral, StringTrimRight($YMax,1))
   $oSeries_0.Marker.Style = Random(1,9,1)

   $oSeries_1.SetData ($chDimSeriesNames, $chDataLiteral, "YMin")
   $oSeries_1.SetData($chDimThetaValues, $chDataLiteral, StringTrimRight($X,1))
   $oSeries_1.SetData($chDimRValues, $chDataLiteral, StringTrimRight($YMin,1))
   $oSeries_1.Marker.Style = Random(1,9,1)

  elseif StringInStr($GraphTypeName, "Radar") then 
   $oSeries.SetData($chDimSeriesNames, $chDataLiteral, "Abs(Sin(X/15))")
   $oSeries.SetData($chDimValues, $chDataLiteral, StringTrimRight($X,1))
   $oSeries.SetData($chDimValues, $chDataLiteral, StringTrimRight($YAbs,1))
   $oSeries.Marker.Style = Random(1,9,1)

   $oSeries_0.SetData($chDimSeriesNames, $chDataLiteral, "YMax")
   $oSeries_0.SetData($chDimValues, $chDataLiteral, StringTrimRight($X,1))
   $oSeries_0.SetData($chDimValues, $chDataLiteral, StringTrimRight($YMax,1))
   $oSeries_0.Marker.Style = Random(1,9,1)

   $oSeries_1.SetData($chDimSeriesNames, $chDataLiteral, "YMin")
   $oSeries_1.SetData($chDimValues, $chDataLiteral, StringTrimRight($X,1))
   $oSeries_1.SetData($chDimValues, $chDataLiteral, StringTrimRight($YMin,1))
   $oSeries_1.Marker.Style = Random(1,9,1)
  ...
  elseif StringInStr($GraphTypeName, "Line") or StringInStr($GraphTypeName, "Area") then 
   ...
   $oSeries_0.SetData ($chDimSeriesNames, $chDataLiteral, "YMin")
   $oSeries_0.SetData($chDimCategories, $chDataLiteral, StringTrimRight($X,1))
   $oSeries_0.SetData($chDimValues, $chDataLiteral, StringTrimRight($X,1))
   $oSeries_0.SetData($chDimValues, $chDataLiteral, StringTrimRight($YMin,1))
   $oSeries_0.Marker.Style = Random(1,9,1)

   $oSeries_1.SetData ($chDimSeriesNames, $chDataLiteral, "YMax")
   $oSeries_1.SetData($chDimCategories, $chDataLiteral, StringTrimRight($X,1))
   $oSeries_1.SetData($chDimValues, $chDataLiteral, StringTrimRight($X,1))
   $oSeries_1.SetData($chDimValues, $chDataLiteral, StringTrimRight($YMax,1))
   $oSeries_1.Marker.Style = Random(1,9,1)
