...
Global $StartDialog = IniRead($sIniFile, "Options", "StartDialog", "0")
Global $iWidth = IniRead($sIniFile, "Options", "Width", "800"), $iHeight = IniRead($sIniFile, "Options", "Height", "600")
...
GUICtrlCreateMenuItem("", $hMenu)
;===== Recent Files ========
Global $hMenu7 = GUICtrlCreateMenu("Recen&t Files", $hMenu)
Global $NrFisRecente = IniReadSection($sIniFile, "RecentFiles")
If @error = 1 Then
	ConsoleWrite("Section does not exists!")
Else
	For $i = 1 To $NrFisRecente[0][0]
		GUICtrlCreateMenuItem($NrFisRecente[$i][1], $hMenu7)
		GUICtrlSetOnEvent(-1, '_OpenRecent')
	Next
EndIf
;===== End Recent Files
GUICtrlCreateMenuItem("", $hMenu)
...
Func _OpenFile()
	...
	_ShowFile()
	For $i = 1 To 10;let's say we store 10 files
		If IniRead($sIniFile, "RecentFiles", "File" & $i, $FileName) = $FileName Then
			IniWrite($sIniFile, "RecentFiles", "File" & $i, $FileName)
			ExitLoop
		EndIf
	Next
EndFunc   ;==>_OpenFile
...
Func _OpenRecent()
	;added by taietel
	;open 10 most recent files
	Local $ctrlMenu = @GUI_CtrlId
	Local $sRFile = IniRead($sIniFile, "RecentFiles", "File" & $ctrlMenu - 11, "")
	$sFileName = $sRFile
	_GUICtrlListView_DeleteAll($hLV)
	_ShowFile()
EndFunc   ;==>_OpenRecent
